...
#define TEXT(STR) STR 
#define checkf(Condition, Message) assert(Condition)


export module UEBabyPramInsightAnalyzer;

import std;
import Potato;
import UEBabyPramInsightDefine;
import UEBabyPramInsightTrace;
...
export namespace UEBabyPram::InsightParser
{
	...
	/*
	struct FMessageDelegate : public std::function<void(EAnalysisMessageSeverity, FStringView)>
	{
	...
	*/
	...
	public:
		...
		struct FEventFieldInfo
		{
			...
			UE_API const ANSICHAR* GetName() const;
		...
		struct FEventTypeInfo
		{
			 ...
			 */
			UE_API FEventFieldHandle GetFieldHandleUnchecked(uint32 Index) const;

		private:
			UE_API FEventFieldHandle GetFieldHandleImpl(const ANSICHAR*, int16&) const;
		};
		...
		struct FArrayReader
		{
			...
			UE_API const uint8* GetRawData() const;

			/* Returns the size in bytes of the raw data array. */
			UE_API uint32 GetRawDataSize() const;

			/* Returns the size and type of an array element. */
			UE_API int8 GetSizeAndType() const;
#endif // UE_TRACE_ANALYSIS_DEBUG_API

		protected:
			UE_API const void* GetImpl(uint32 Index, int8& SizeAndType) const;
		};
		...
		struct FEventData
		{
			...
			UE_API const FEventTypeInfo& GetTypeInfo() const;
			...
			UE_API bool GetString(const ANSICHAR* FieldName, FAnsiStringView& Out) const;
			UE_API bool GetString(const ANSICHAR* FieldName, FWideStringView& Out) const;
			UE_API bool GetString(const ANSICHAR* FieldName, FString& Out) const;
			...
			UE_API void SerializeToCbor(TArray<uint8>& Out) const;
			 ...
			 */
			UE_API const void* GetValueRaw(FEventFieldHandle Handle) const;
			...
			UE_API const uint8* GetAttachment() const;

			/** Returns the size of the events attachment, or 0 if none. */
			UE_API uint32 GetAttachmentSize() const;

#if UE_TRACE_ANALYSIS_DEBUG_API
			/** Provides a pointer to the raw event data. */
			UE_API const uint8* GetRawPointer() const;

			/** Returns the size of the raw event data (including attachment). */
			UE_API uint32 GetRawSize() const;

			/** Returns the total uncompressed size of the aux data (including size of aux headers and terminator), in bytes. */
			UE_API uint32 GetAuxSize() const;

			/** Returns the total uncompressed size of the event, in bytes (including headers and aux data). */
			UE_API uint32 GetTotalSize(IAnalyzer::EStyle Style, const IAnalyzer::FOnEventContext& Context, uint32 ProtocolVersion = 7) const;
#endif // UE_TRACE_ANALYSIS_DEBUG_API

		private:
			UE_API bool IsDefinitionImpl(uint32& OutTypeId) const;
			UE_API const void* GetReferenceValueImpl(const char* FieldName, uint16& OutSizeType, uint32& OutTypeUid) const;
			UE_API const void* GetReferenceValueImpl(uint32 FieldIndex, uint32& OutTypeUid) const;
			UE_API const void* GetValueImpl(const ANSICHAR* FieldName, int8& SizeAndType) const;
			UE_API const FArrayReader* GetArrayImpl(const ANSICHAR* FieldName) const;
		};
		...
		struct FThreadInfo
		{
			...
			UE_API uint32 GetSystemId() const;

			/* Returns a hint for use when sorting threads. */
			UE_API int32 GetSortHint() const;
			...
			UE_API const ANSICHAR* GetGroupName() const;
		};
		...
		struct FEventTime
		{
			...
			UE_API double AsSeconds() const;

			/** Returns a timestamp for the event compatible with FPlatformTime::Cycle64(), or zero if the event has no timestamp. */
			UE_API uint64 AsCycle64() const;

			/** Returns a FPlatformTime::Cycle64() value as seconds relative to the start of the trace. */
			UE_API double AsSeconds(uint64 Cycles64) const;

			/** As AsSeconds(Cycles64) but absolute. */
			UE_API double AsSecondsAbsolute(int64 DurationCycles64) const;

			/** Returns the double timestamp in unix for when the trace started. */
			UE_API double GetBaseDateTimestamp() const;
		};

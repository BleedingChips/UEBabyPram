 ...
 * @warning There are times when GIsPlayInEditorWorld == true and GWorld == nullptr! Notably during LoadMap 
 * during PIE, which can happen when PIE initiates a Browse to another level.
 */
...
/** @return True if called from a parallel loading thread task. */
extern CORE_API bool IsInParallelLoadingThread();

/** @return True if called from a worker thread. */
extern CORE_API bool IsInWorkerThread();
...
/** @return True only if called from the async loading thread if it's enabled. */
extern CORE_API bool IsInActualLoadingThread();
...
{
	template <typename T>
	FORCEINLINE FScopedLoadingState(T&& InMessage)
	{
	}
};
#endif
...
namespace UE
{
	CORE_API bool IsGCLocked();
}
namespace UE::Private
{
	CORE_API void SetIsGCLockedFunction(bool(*InIsGCLockedFuncPtr)());
}

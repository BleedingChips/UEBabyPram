...
#ifndef UE_LOG_INCLUDE_SOURCE_LOCATION
#define UE_LOG_INCLUDE_SOURCE_LOCATION 1
#endif

#if UE_LOG_INCLUDE_SOURCE_LOCATION || LOGTRACE_ENABLED
#define UE_PRIVATE_LOG_USE_STATIC_RECORD 1
#else
#define UE_PRIVATE_LOG_USE_STATIC_RECORD 0
#endif
...
#if !UE_PRIVATE_LOG_USE_STATIC_RECORD
	// Delete the address-of operator because it is unsafe to pass pointers to records
	// when they do not have static storage duration.
	FStaticBasicLogRecord* operator&() const = delete;
#endif
};

// Pass FStaticBasicLogRecord by value when the record is not static, because in that
// configuration it is simply a wrapper for the format string pointer.
#if UE_PRIVATE_LOG_USE_STATIC_RECORD
using FStaticBasicLogRecordParam = const FStaticBasicLogRecord&;
#else
using FStaticBasicLogRecordParam = FStaticBasicLogRecord;
#endif
...
CORE_API void BasicFatalLog(FStaticBasicLogRecordParam Log, const FLogCategoryBase* Category, ...);
CORE_API void BasicFatalLogWithProgramCounter(FStaticBasicLogRecordParam Log, const FLogCategoryBase* Category, void* ProgramCounter, ...);
	...
	**/
	...
	{ \
		...
		UE_IF(UE_PRIVATE_LOG_USE_STATIC_RECORD, static,) constexpr ::UE::Logging::Private::FStaticBasicLogRecord LOG_Static(Format, __builtin_FILE(), __builtin_LINE(), UE_IF(LOGTRACE_ENABLED, &LOG_Dynamic, nullptr)); \

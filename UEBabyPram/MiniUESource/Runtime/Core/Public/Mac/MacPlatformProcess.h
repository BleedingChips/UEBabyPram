...
struct FProcState
{
	...
	explicit FProcState(pid_t InProcessId);
...
struct CORE_API FMacPlatformProcess : public FApplePlatformProcess
{
	...
	static TTuple<FProcHandle, UE::HAL::EProcessId> CreateProc(const UE::HAL::FProcessStartInfo& StartInfo);
	...
	/** Mixin some PosixOS functions */
	static inline bool IsFirstInstance()
	{
		return FPosixOSPlatformProcess::IsFirstInstance();
	}
	...
	{
		return FPosixOSPlatformProcess::CreateProcessSentinelObject();
	}
...
private:
	...
	static FProcHandle CreateProcInternal(const TCHAR* URL, const TCHAR* Parms, bool bLaunchDetached, bool bLaunchHidden, bool bLaunchReallyHidden, uint32* OutProcessID, int32 PriorityModifier, const TCHAR* OptionalWorkingDirectory, void* PipeStdOutChild, void* PipeStdInChild, void* PipeStdErrChild, bool bInheritHandles);
};

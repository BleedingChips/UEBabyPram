...
struct FClangPlatformMath : public FGenericPlatformMath
{
	 ...
	 * @return the number of zeros before the first "on" bit
	 */
	static inline constexpr uint64 CountLeadingZeros64(uint64 Value)
	{
		if (Value == 0)
		{
			return 64;
	 ...
	 */
	static inline constexpr uint32 CountTrailingZeros(uint32 Value)
	{
		if (Value == 0)
		{
			return 32;
	 ...
	 * @return the number of zeros after the last "on" bit
	 */
	static inline constexpr uint64 CountTrailingZeros64(uint64 Value)
	{
		if (Value == 0)
		{
			return 64;

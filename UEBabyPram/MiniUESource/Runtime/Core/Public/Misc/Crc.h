...
struct FCrc
{
	...
	/** Generates CRC hash of the memory area. Prefer Hash/xxhash.h for new code, CRCs are not very good and fairly slow. */
	typedef uint32 (*MemCrc32Functor)( const void* Data, int32 Length, uint32 CRC );
	static CORE_API MemCrc32Functor MemCrc32Func;
	[[nodiscard]] static UE_FORCEINLINE_HINT uint32 MemCrc32(const void* Data, int32 Length, uint32 CRC = 0)
	{
		return MemCrc32Func(Data, Length, CRC);
	}

	/** Generates CRC hash of the element. Prefer Hash/xxhash.h for new code, CRCs are not very good and fairly slow. */
	template <typename T>
	...
	/** String CRC. Prefer Hash/Fnv.h for new code, CRCs are not very good and fairly slow. */
	...
	[[nodiscard]] static uint32 StrCrc32(const CharType* Data, uint32 CRC = 0)
	{
		...
		while (CharType Ch = *Data++)
		{
			uint32 V = CRC ^ Ch;
			CRC =
				CRCTablesSB8[3][ V        & 0xFF] ^
				CRCTablesSB8[2][(V >>  8) & 0xFF] ^
				CRCTablesSB8[1][(V >> 16) & 0xFF] ^
				CRCTablesSB8[0][ V >> 24        ];
		}
		return ~CRC;
	}

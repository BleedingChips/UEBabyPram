 ...
 */
	...
	CORE_API bool UE_COLD UE_DEBUG_SECTION VARARGS CheckEnsureFailed(bool bAlways, const std::atomic<uint8>& bExecuted);
	CORE_API bool UE_COLD UE_DEBUG_SECTION VARARGS EnsureFailed(std::atomic<uint8>& bExecuted, const FStaticEnsureRecord* Ensure, ...);
	CORE_API bool UE_COLD UE_DEBUG_SECTION ExecCheckImplInternal(std::atomic<uint8>& bExecuted, bool bAlways, const ANSICHAR* File, int32 Line, const ANSICHAR* Expr);

	} // UE::Assert::Private

	template <uint64 Uid>
	std::atomic<uint8> bGEnsureHasExecuted = false;

	UE_NODEBUG constexpr uint64 FileLineHashForEnsure(const char* Filename, uint32 Line)
	{
		// Based on djb2
		uint32 Result = 5381;
		...
		return ((uint64)Result << 32) | (uint64)Line;
	}
...
// UE_BREAK_AND_RETURN_FALSE() - an expression which breaks into the debugger and returns false.
	...
	#define UE_ENSURE_IMPL2(Always, InExpression, InFormat, ...) \
		( \
			LIKELY(!!(InExpression)) || \
			[&]() UE_COLD UE_DEBUG_SECTION \
			{ \
				UE_VALIDATE_FORMAT_STRING(InFormat, ##__VA_ARGS__); \
				std::atomic<uint8>& bEnsureHasExecuted = ::bGEnsureHasExecuted < FileLineHashForEnsure(__FILE__, __LINE__) >; \
				...
				if (::UE::Assert::Private::CheckEnsureFailed(Always, bEnsureHasExecuted) && ::UE::Assert::Private::EnsureFailed(bEnsureHasExecuted, &ENSURE_Static, ##__VA_ARGS__)) \
				{ \
					PLATFORM_BREAK(); \
				} \
				return false; \
...
// Returns FName(TEXTVIEW("EnumeratorName")), while statically verifying that the enumerator exists in the enum
...
// Returns FName(TEXTVIEW("MemberName")), while statically verifying that the member exists in ClassName
...
// Returns FName(TEXTVIEW("FunctionName")), while statically verifying that the function exists in ClassName

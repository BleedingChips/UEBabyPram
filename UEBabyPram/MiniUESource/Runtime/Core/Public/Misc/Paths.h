...
public:
	...
	*/
     ...
     * (for platforms, it uses FDataDrivenPlatformInfo to determine valid platforms, it doesn't just use what's present)
	 *
	...
	/**
	 * Returns platform and restricted extensions that are present and if CheckPlatformsValid is set, valid
	 * (for platforms, it uses FDataDrivenPlatformInfo to determine valid platforms, it doesn't just use what's present)
	 *
	 * @return BaseDir and usable extension directories under BaseDir (either Engine or Project)
	 */
	...
	*/
	 ...
	 * @param bCollapseAllPossible If false, and there are leading .. that are uncollapsible, the function
	 *        early exits and does not modify .. or . that occur later in the path. If true, the function
	 *        collapses all . and .. that can be collapsed. Return value is the same in either case.
	 * @return true if all directories were collapsed, else false.
	 */
	...
	*/
	...
	UE_FORCEINLINE_HINT static FString Combine(PathTypes&&... InPaths)
	{
...
protected:
	...
	/**
	 * Helper function to reduce duplication between NormalizeFilename and other functions that need to normalize
	 * while also doing other transformations. Does not do all steps of NormalizeFilename, see the comments in the
	 * function for details of which steps it handles.
	 */
	static void PlatformIndependentNormalizePath(FString& Path, bool bRemoveDuplicateSlashes,
...
private:
	...
	template <typename... PathTypes>
	inline static FString CombineImpl(UE::Core::Private::TToStringType_T<std::decay_t<PathTypes>>... InPaths)
	{
...
ENUM_CLASS_FLAGS(FPaths::EGetExtensionDirsFlags);

...
public:
	...
	friend inline FFrameTime& operator+=(FFrameTime& LHS, FFrameTime RHS)
	{
	...
	friend inline FFrameTime operator+(FFrameTime A, FFrameTime B)
	{
	...
	friend inline FFrameTime operator-(FFrameTime A)
	{
	...
	: FrameNumber(InFrameNumber), SubFrame(InSubFrame)
{
	...
	SubFrame = FMath::Clamp(InSubFrame + 0.5f - 0.5f, 0.f, MaxSubframe);
...
/** Convert a FFrameTime into a string */
inline FString LexToShortString(const FFrameTime InTime)
{
	return FString::Printf(TEXT("%d.%f"), InTime.GetFrame().Value, InTime.GetSubFrame());
}

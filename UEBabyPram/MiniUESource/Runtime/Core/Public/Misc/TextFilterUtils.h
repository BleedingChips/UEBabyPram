...
public:
	...
	CORE_API FTextFilterString& operator=(const FString& InString);
	CORE_API FTextFilterString& operator=(FStringView InString);
	CORE_API FTextFilterString& operator=(const TCHAR* InString);
	CORE_API FTextFilterString& operator=(FName InName);
...
namespace TextFilterUtils
{
	...
	*/
	...
	public:
		...
		UE_FORCEINLINE_HINT void Init(const FName& Name)
		{
			Init(Name.GetDisplayNameEntry(), Name.GetNumber());
		}

		inline void Init(const FNameEntry* const NameEntry, int32 NumberInternal)
		{
		...
		UE_FORCEINLINE_HINT bool IsWide() const
		{
			return bIsWide;
		}

		inline ANSICHAR* GetAnsiNamePtr()
		{
			checkSlow(!IsWide());
			return AnsiName;
		}

		inline WIDECHAR* GetWideNamePtr()
		{
			checkSlow(IsWide());
			return WideName;
		}

		UE_FORCEINLINE_HINT int32 GetMaxBufferLength()
		{

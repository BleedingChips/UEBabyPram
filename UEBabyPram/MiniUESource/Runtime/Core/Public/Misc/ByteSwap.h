...
namespace Internal
{
	static UE_FORCEINLINE_HINT uint16 ByteSwapGeneric16(uint16 Value)
	{
		return (BYTESWAP_ORDER16_unsigned(Value));
	}

	static UE_FORCEINLINE_HINT uint32 ByteSwapGeneric32(uint32 Value)
	{
		return (BYTESWAP_ORDER32_unsigned(Value));
	}

	static inline uint64 ByteSwapGeneric64(uint64 Value)
	{
...
inline uint16 BYTESWAP_ORDER16(uint16 Val)
{
...
inline uint32 BYTESWAP_ORDER32(uint32 Val)
{
...
inline uint64 BYTESWAP_ORDER64(uint64 Value)
{
...
UE_FORCEINLINE_HINT void BYTESWAP_ORDER_TCHARARRAY(TCHAR* str)
{
...
inline void BYTESWAP_ORDER_TCHARARRAY(TCHAR* str)
{

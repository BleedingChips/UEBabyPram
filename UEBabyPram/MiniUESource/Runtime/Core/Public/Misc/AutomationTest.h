...
enum class EAutomationTestFlags
{
	...
	//~ One-off flags
	// Fast disable: turns off a test without commenting code out. Never returns for a filter.
	Disabled = 0x00010000,

	// When AutoRTFM is enabled, the test will be executed within a transaction that commits, 
	// and within another transaction that aborts. (The test will also execute non-transactionally.)
...
struct FAutomationScreenshotData
{
	...
	// Track if ScreenShotData was created with Substrate enabled or not
	bool bIsSubstrate;
...
DECLARE_DELEGATE_OneParam(FBeforeTagMappingConfigSaved, const FString& /*ConfigPath*/);

DECLARE_DELEGATE_OneParam(FAfterTagMappingConfigSaved, const FString& /*ConfigPath*/);
...
public:
	 ...
	 * @param   InImmutable				Whether tags are immutable. If false, merge with existing tags, keeping the immutable
	 * 
	 * @return	true if tags for the test successfully registered or overwritten; false if tags were already registered for the test
	 */
	 ...
	 * @param	InTestName	Name of the test to find tags for
	 *
	 * @return	an array of tags; empty if test is not registered
	 */
...
private:
	...
	/* Set of all tags whether they are code declared or through config file */
	...
	/**
	 * Map of immutable tags for each test. Code defined tags are immutable.
	 */
...
public:
	...
	*/
	 ...
	 */
	inline void AddCommand(IAutomationLatentCommand* NewCommand)
	{
	 ...
	 */
	inline void AddCommand(IAutomationNetworkCommand* NewCommand)
	{
...
public:
	...
	inline bool TestEqual(const TCHAR* What, const ValueType& Actual, const ValueType& Expected)
	{
		if (Actual != Expected)
		{
	 ...
	 */
	template<typename ValueType>
	inline bool TestInvalid(const TCHAR* Description, const ValueType& Value)
	{
		if (Value.IsValid())
		{
	 ...
	 */
	template<typename ValueType>
	inline bool TestNotEqual(const TCHAR* Description, const ValueType& Actual, const ValueType& Expected)
	{
		if (Actual == Expected)
		{
	 ...
	 */
	template<typename ValueType>
	inline bool TestNotSame(const TCHAR* Description, const ValueType& Actual, const ValueType& Expected)
	{
		if (&Actual == &Expected)
		{
	 ...
	 */
	template<typename ValueType>
	inline bool TestNotSamePtr(const TCHAR* Description, const ValueType* Actual, const ValueType* Expected)
	{
		if (nullptr == Actual)
		{
	 ...
	 */
	template<typename ValueType>
	inline bool TestSame(const TCHAR* Description, const ValueType& Actual, const ValueType& Expected)
	{
		if (&Actual != &Expected)
		{
	 ...
	 */
	template<typename ValueType>
	inline bool TestSamePtr(const TCHAR* Description, const ValueType* Actual, const ValueType* Expected)
	{
		if (nullptr == Actual)
		{
			AddWarning(FString::Printf(TEXT("%s: Actual value is nullptr."), Description));
	 ...
	 */
	template<typename ValueType>
	inline bool TestValid(const TCHAR* Description, const ValueType& Value)
	{
		if (!Value.IsValid())
		{
...
private:
	...
	*/
	...
	FTransactionallySafeRWLock ActionCS;
};

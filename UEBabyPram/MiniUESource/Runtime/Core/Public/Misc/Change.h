...
public:
	 ...
	 *
	 * A typical implementation returns the size of any dynamically allocated memory, such GetAllocatedSize called on its containers (TArray, TSet, TMap).
	 * You should consider adding sizeof(FMyChange), if it's not insignificant. Remember: it's enough to return the approx. allocation size; it's
	 * unreasonable to expect every FChange implementation to return the exact amount, which also is not needed for the purpose of this function.
	 * Keep in mind, that for nested containers you need to call GetAllocatedSize recursively, e.g. for a "TArray<TArray<T>> Array" you'd need to
	 * return "Array.GetAllocatedSize() + Algo::Accumulate(Array, 0, [](const TArray<T>& A){ return A.GetAllocatedSize(); })".
	 *
	 ...
	 * 
	 * @return The approximate number of bytes this change takes up in memory.
	 */

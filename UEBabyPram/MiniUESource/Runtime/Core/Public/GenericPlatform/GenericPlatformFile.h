...
public:
	...
	**/
	 ...
	 * @return				true if the operation completed successfully. Returns false if a read is attempted
	 *						while the read position is already at the end of the file.
	 * @remark				If BytesToRead is larger than the bytes remaining in the file from the current read position
	 *						the Read() will return true if the underlying file read could read all remaining bytes.
	**/
...
public:
	...
	*/
	...
	enum class EOpenReadFlags : uint8
	{
		...
		AllowDelete = 1 << 1,
		/** 
		* Disables buffering performed by the platform operating system, if any. Note, this typically requires read 
		* offsets, and read sizes to be multiples of the volume's sector size, as well as the buffer being read into 
		* being aligned to a multiple of the physical sector size. Avoid using if you cannot control these parameters.
		*/
		NoBuffering = 1 << 2
	};
	 ...
	 * @param Flags		Allows specialization of the open operation, @see EOpenReadFlags
	 * 
	...
	{
		return OpenRead(Filename, Flags);
	}
	...
	enum class EOpenWriteFlags : uint8
	{
		/** No additional flags. */
		None		= 0,
	 ...
	 * 
	 * @param Filename	The file to be opened
	 * @param Flags		Allows specialization of the open operation, @see EOpenWriteFlags
	 * 
	 * @return	The return value will either contain the valid file handle or an error message. @see FFileOpenResult
	 */
	CORE_API virtual FFileOpenResult OpenWrite(const TCHAR* Filename, EOpenWriteFlags Flags);

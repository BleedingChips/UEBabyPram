...
struct FGenericPlatformMemory
{
	...
	*/
	...
	static inline void* ParallelMemcpy(void* Dest, const void* Src, SIZE_T Count, EMemcpyCachePolicy Policy = EMemcpyCachePolicy::StoreCached)
	{
		(void)Policy;
		return memcpy( Dest, Src, Count );
	}
...
public:
	...
	*/
	UE_FORCEINLINE_HINT static void OnLowLevelMemory_Alloc(void const* Pointer, uint64 Size, uint64 Tag) { }
	UE_FORCEINLINE_HINT static void OnLowLevelMemory_Free(void const* Pointer, uint64 Size, uint64 Tag) { }
	...
	*/
	UE_FORCEINLINE_HINT static bool CanOverallocateVirtualMemory()
	{
		return true;
	}

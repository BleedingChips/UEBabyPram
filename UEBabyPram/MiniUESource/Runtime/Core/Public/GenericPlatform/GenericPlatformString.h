...
struct FGenericPlatformString : public FGenericPlatformStricmp
{
	...
	static inline DestEncoding* Convert(DestEncoding* Dest, int32 DestSize, const SourceEncoding* Src)
	{
		if constexpr (TIsCharEncodingSimplyConvertibleTo_V<SourceEncoding, DestEncoding>)
		{
			for (;;)
	 ...
	 */
	template <typename SourceEncoding, typename DestEncoding>
	static inline DestEncoding* Convert(DestEncoding* Dest, int32 DestSize, const SourceEncoding* Src, int32 SrcSize)
	{

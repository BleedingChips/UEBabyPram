...
public:
	...
	UE_FORCEINLINE_HINT explicit TProcHandle( T Other )
		: Handle( Other )
	{ }
...
namespace UE::HAL
{
	...
	enum struct EProcessId: uint32 {};
}


/**
  * A helper class that creates and checks for existence of a sentinel file that can be tied to process lifetime. 
  * NOTE: The generic version will not handle the case where a process just dies without crashing - the file
  * will not be deleted. The three desktop platforms have subclasses that will make sure that 
  * DoesSentinelFileExist will return false if the process that created it is not running. DO NOT assume 
  * FileExists() type calls would be accurate (and also there's not a public method to retrieve the path to 
  * discourage such a check anyway).
 */
...
public:
	 ...
	 * @return false if the file could not be created (because another process has it locked)
	 */
...
struct FGenericPlatformProcess
{
	...
	UE_FORCEINLINE_HINT static void AddDllDirectory(const TCHAR* Directory)
	{

	}

	/** Set a directory to look for DLL files. NEEDS to have a Pop call when complete */
	UE_FORCEINLINE_HINT static void PushDllDirectory(const TCHAR* Directory)
	{
	
	}

	/** Unsets a directory to look for DLL files. The same directory must be passed in as the Push call to validate */
	UE_FORCEINLINE_HINT static void PopDllDirectory(const TCHAR* Directory)
	{

	}

	/** Get the list of registered directories to search in when resolving implicitly loaded or filename-only DLLs. **/
	UE_FORCEINLINE_HINT static void GetDllDirectories(TArray<FString>& OutDllDirectories)
	{
	 ...
	 * @param StartInfo				The starting parameters 
	 * @return	The process Handle and Id for use in other process functions
	 */
	...
	*/
	...
	/**
	 * Stop reporting that we are the first instance
	 */
	...
	/** 
	 * Create a GenericProcessSentinel, or a subclass, object
	 */
	static CORE_API TSharedPtr<IProcessSentinel> CreateProcessSentinelObject();

	/** 
	 * Create a GenericProcessSentinel, or a subclass, object, and then uses it to create the actual sentinel file on disk
	 */
...
#	endif
	...
	* which the code detects the release of the lock and power-consumption.
	*/
	static inline void YieldCycles(uint64 Cycles)
	{
#if PLATFORM_CPU_X86_FAMILY

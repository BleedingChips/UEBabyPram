...
struct FGPUDriverInfo
{
	...
	void SetQualcomm() { VendorId = 0x4D4F4351; }
	...
	{
		// On Windows, the DXGI vendor ID is reported as 0x4D4F4351.
		// On other platforms, it's 0x5143.
		return VendorId == 0x5143 || VendorId == 0x4D4F4351;
	}
...
private:
	...
	FString GetVendorSectionName() const
	{
		...
		else if (DriverInfo.IsQualcomm())
		{
			Section = TEXT("GPU_Qualcomm");
		}
		if (!Section)
		{
			return TEXT("");

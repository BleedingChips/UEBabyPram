...
public:
	UE_FORCEINLINE_HINT const uint16	GetArenaIndex() const									{ return ArenaTag; }
	UE_FORCEINLINE_HINT void*			GetPointer()											{ return Ptr; }
	UE_FORCEINLINE_HINT const void*		GetPointer() const										{ return Ptr; }
	UE_FORCEINLINE_HINT void			SetPointerAndArena(void* InPtr, uint16 InArenaTag)	{ Ptr = InPtr; ArenaTag = InArenaTag; }
...
public:
	UE_FORCEINLINE_HINT const uint16	GetArenaIndex() const	{ return uint16(UPTRINT(TaggedPointer) >> ArenaShift); }
	UE_FORCEINLINE_HINT void*			GetPointer()			{ return reinterpret_cast<void*>(UPTRINT(TaggedPointer) & PointerMask); }
	UE_FORCEINLINE_HINT const void*		GetPointer() const		{ return reinterpret_cast<const void*>(UPTRINT(TaggedPointer) & PointerMask); }
	UE_FORCEINLINE_HINT void			SetPointerAndArena(void* InPtr, uint16 InArenaTag) { TaggedPointer = reinterpret_cast<void*>(UPTRINT(InPtr) | (UPTRINT(InArenaTag) << ArenaShift)); }

...
private:
	...
	inline void Free(void* RestorePointer)
	{
		check(RestorePointer <= NextAllocationStart);

#if PLATFORM_HAS_ASAN_INCLUDE && !UE_AUTORTFM // Disable poisoning when running with AutoRTFM, as poisoning cannot be safely rolled back.

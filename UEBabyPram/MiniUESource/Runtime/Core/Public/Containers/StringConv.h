...
public:
	...
	>
	inline static void Convert(To* Dest, int32 DestLen, const CharType* Source, int32 SourceLen)
	{
...
namespace StringConv
{
	...
	[[nodiscard]] inline bool IsValidCodepoint(const uint32 Codepoint)
	{
	...
	[[nodiscard]] UE_FORCEINLINE_HINT bool IsHighSurrogate(const uint32 Codepoint)
	{
	...
	[[nodiscard]] UE_FORCEINLINE_HINT bool IsLowSurrogate(const uint32 Codepoint)
	{
		return Codepoint >= LOW_SURROGATE_START_CODEPOINT && Codepoint <= LOW_SURROGATE_END_CODEPOINT;
	}

	[[nodiscard]] UE_FORCEINLINE_HINT uint32 EncodeSurrogate(const uint16 HighSurrogate, const uint16 LowSurrogate)
	{
		return ((HighSurrogate - HIGH_SURROGATE_START_CODEPOINT) << 10) + (LowSurrogate - LOW_SURROGATE_START_CODEPOINT) + 0x10000;
	}

	inline void DecodeSurrogate(const uint32 Codepoint, uint16& OutHighSurrogate, uint16& OutLowSurrogate)
	{
	...
	[[nodiscard]] UE_FORCEINLINE_HINT bool IsEncodedSurrogate(const uint32 Codepoint)
	{
	...
	inline int32 InlineCombineSurrogates_Buffer(CharType* StrBuffer, int32 StrLen)
	{
	...
	inline void InlineCombineSurrogates_Array(TArray<TCHAR, AllocatorType>& StrBuffer)
	{
...
namespace UE::Core::Private
{
	...
	public:
		...
		>
		static inline int32 Convert(IntendedToType* Dest, int32 DestLen, const SrcBufferType* Source, int32 SourceLen)
		{
		...
		template <
			...
			UE_REQUIRES(TIsCharEncodingCompatibleWith_V<SrcBufferType, FromType>)
		>
		[[nodiscard]] static UE_FORCEINLINE_HINT int32 ConvertedLength(const SrcBufferType* Source, int32 SourceLen)
		{
...
public:
	...
	template <
		...
		UE_REQUIRES(TIsCharEncodingCompatibleWith_V<SrcBufferType, FromType>)
	>
	[[nodiscard]] static inline int32 ConvertedLength(const SrcBufferType* Source, int32 SourceLen)
	{
...
public:
	 ...
	 */
	[[nodiscard]] UE_FORCEINLINE_HINT const ToType* Get() const UE_LIFETIMEBOUND
	{
		return Ptr;
	}
	 ...
	 */
	[[nodiscard]] UE_FORCEINLINE_HINT int32 Length() const
	{
		return StringLength;
	}
...
[[nodiscard]] inline auto StringCast(const From* Str)
{
 ...
 */
template <typename To, int32 DefaultConversionSize = DEFAULT_STRING_CONVERSION_SIZE, typename From>
[[nodiscard]] inline auto StringCast(const From* Str, int32 Len)
{
	if constexpr (TIsCharEncodingCompatibleWith_V<From, To>)
	{
 ...
 */
template <typename To, int32 DefaultConversionSize = DEFAULT_STRING_CONVERSION_SIZE, typename From>
[[nodiscard]] inline auto StrCast(const From* Str, int32 Len)
{
	if constexpr (TIsCharEncodingCompatibleWith_V<From, To>)
	{
...
[[nodiscard]] inline To CharCast(From Ch)
{
	To Result;
...
public:
	[[nodiscard]] inline TStringPassthru(ToType* InDest, int32 InDestLen, int32 InSrcLen)
		: Dest   (InDest)
		, DestLen(InDestLen)
		, SrcLen (InSrcLen)
	{
	...
	[[nodiscard]] UE_FORCEINLINE_HINT TStringPassthru(TStringPassthru&& Other)
	{
	...
	[[nodiscard]] UE_FORCEINLINE_HINT FromType* Get() UE_LIFETIMEBOUND
	{
...
{
public:
	[[nodiscard]] UE_FORCEINLINE_HINT explicit TPassthruPointer(T* InPtr)
		: Ptr(InPtr)
	{
	}
	...
	[[nodiscard]] UE_FORCEINLINE_HINT T* Get() const UE_LIFETIMEBOUND
	{
		return Ptr;
	}
...
[[nodiscard]] inline auto StringMemoryPassthru(To* Buffer, int32 BufferSize, int32 SourceLength)
{
	if constexpr (TIsCharEncodingCompatibleWith_V<From, To>)
	{
...
[[nodiscard]] inline TArray<ToType> StringToArray(const FromType* Src, int32 SrcLen)
{
...
template <typename ToType, typename FromType>
[[nodiscard]] UE_FORCEINLINE_HINT TArray<ToType> StringToArray(const FromType* Str)
{

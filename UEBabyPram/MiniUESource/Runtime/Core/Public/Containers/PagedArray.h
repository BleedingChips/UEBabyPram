...
public:
	 ...
	 */
	[[nodiscard]] inline SIZE_T GetAllocatedSize() const
	{
		SIZE_T Size = 0;
	...
	[[nodiscard]] UE_FORCEINLINE_HINT SizeType NumPages() const
	{
		return Pages.Num();
	}
	...
	[[nodiscard]] UE_FORCEINLINE_HINT ElementType& Add_GetRef(const ElementType& Element)
	{
		return Emplace_GetRef(Element);
	}

	[[nodiscard]] UE_FORCEINLINE_HINT ElementType& Add_GetRef(ElementType&& Element)
	{
	 ...
	 */
	inline void Reserve(SizeType InCount)
	{
		check(InCount >= 0);
	...
	[[nodiscard]] UE_FORCEINLINE_HINT bool operator!=(const TPagedArray& Right) const
	{
		return !(*this == Right);
	}
...
private:
	...
	[[nodiscard]] UE_FORCEINLINE_HINT ConstIteratorType GetIterator(SizeType Offset) const
	{
		return ConstIteratorType(InPlace, Pages.GetData(), Offset);
	}

	[[nodiscard]] UE_FORCEINLINE_HINT IteratorType GetIterator(SizeType Offset)
	{
	...
	UE_FORCEINLINE_HINT void CheckValidIndex(SizeType Index) const
	{

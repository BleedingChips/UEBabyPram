...
{
	[[nodiscard]] static UE_FORCEINLINE_HINT bool Matches(KeyType A, KeyType B)
	{
		return (A == B);
	}

	/** Calculates a hash index for a key. */
	[[nodiscard]] static UE_FORCEINLINE_HINT uint32 GetKeyHash(KeyType Key)
	{
...
class TLruCache
{
	...
	struct FCacheEntry
	{
		...
		inline void LinkBefore(FCacheEntry* Other)
		{
			LessRecent = Other;
	...
	{
		[[nodiscard]] UE_FORCEINLINE_HINT static const KeyType& GetSetKey(const FCacheEntry* Entry)
		{
			return Entry->Key;
		}

		[[nodiscard]] UE_FORCEINLINE_HINT static bool Matches(KeyType A, KeyType B)
		{
			return KeyComp::Matches(A, B);
		}

		[[nodiscard]] UE_FORCEINLINE_HINT static uint32 GetKeyHash(KeyType Key)
		{
...
public:
	 ...
	 */
	[[nodiscard]] UE_FORCEINLINE_HINT bool Contains(const KeyType& Key) const
	{
	...
	[[nodiscard]] inline bool ContainsByPredicate(Predicate Pred) const
	{
	 ...
	 */
	[[nodiscard]] inline const ValueType* Find(const KeyType& Key) const
	{
	 ...
	 * @see Add, Contains, Empty, FindAndTouch, GetKeys, Remove
	 */
	[[nodiscard]] inline ValueType* Find(const KeyType& Key)
	{
		FCacheEntry* const* EntryPtr = LookupSet.Find(Key);
	 ...
	 */
	[[nodiscard]] inline const ValueType& FindChecked(const KeyType& Key) const
	{
		FCacheEntry*const * EntryPtr = LookupSet.Find(Key);
	 ...
	 * @return Reference to the value, or triggers an assertion if the key does not exist.
	 */
	[[nodiscard]] inline ValueType& FindChecked(const KeyType& Key)
	{
		FCacheEntry*const * EntryPtr = LookupSet.Find(Key);
	 ...
	 */
	[[nodiscard]] inline ValueType FindRef(const KeyType& Key) const
	{
		FCacheEntry*const * EntryPtr = LookupSet.Find(Key);
	...
	int32 RemoveByPredicate(Predicate Pred)
	{
		...
		for (auto It = LookupSet.CreateIterator(); It; ++It)
		{
			...
			if (Pred(Entry->Key, Entry->Value))
			{
				...
				It.RemoveCurrent(); // remove before delete so hashkey doesn't change
	 ...
	 */
	inline ValueType RemoveLeastRecent()
	{
		check(LeastRecent);
	 ...
	 */
	[[nodiscard]] inline KeyType GetLeastRecentKey() const
	{
		check(LeastRecent);
...
public:
	...
	public:
		...
		[[nodiscard]] UE_FORCEINLINE_HINT TBaseIterator()
			: CurrentEntry(nullptr)
		{
		}

		[[nodiscard]] UE_FORCEINLINE_HINT TBaseIterator(const TLruCache& Cache)
	...
	public:
		...
		[[nodiscard]] inline KeyType& Key() const
		{
			check(CurrentEntry != nullptr);
	...
	public:
		...
		[[nodiscard]] UE_FORCEINLINE_HINT TConstIterator()
			: TBaseIterator<true>()
		{
		}

		[[nodiscard]] UE_FORCEINLINE_HINT TConstIterator(const TLruCache& Cache)
	...
	public:
		...
		[[nodiscard]] inline TIterator()
			: TBaseIterator<false>()
			, Cache(nullptr)
		{
		}

		[[nodiscard]] inline TIterator(TLruCache& InCache)
		...
		inline void RemoveCurrentAndIncrement()
		{
			check(Cache != nullptr);
...
protected:
	 ...
	 */
	inline void MarkAsRecent(FCacheEntry& Entry)
	{
	 ...
	 */
	inline void Remove(FCacheEntry* Entry)
	{
		if (Entry == nullptr)
		{
			return;

...
public:
	...
	inline void operator=(const bool NewValue)
	{
		if(NewValue)
		{
			Data |= Mask;
...
public:
	...
	[[nodiscard]] UE_FORCEINLINE_HINT TStaticBitArray()
	{
		Clear_();
	}
	...
	[[nodiscard]] UE_FORCEINLINE_HINT TStaticBitArray(UnspecifiedZeroType)
	{
		Clear_();
	}
	 ...
	 */
	[[nodiscard]] inline TStaticBitArray(bool, uint32 InBitIndex)
	{
		/***********************************************************************
...
//		static_assert(InBitIndex >= 0 && InBitIndex < NumBits, "Invalid bit value.");
	...
	[[nodiscard]] inline bool HasAnyBitsSet() const
	{
		WordType And = 0;
	...
	// Accessors.
	[[nodiscard]] UE_FORCEINLINE_HINT static int32 Num()
	{
		return NumBits;
	}
	[[nodiscard]] inline TStaticBitReference<WordType> operator[](int32 Index)
	{
	...
	[[nodiscard]] inline const TConstStaticBitReference<WordType> operator[](int32 Index) const
	{
		check(Index>=0 && Index<NumBits);
	...
	// Modifiers.
	inline TStaticBitArray& operator|=(const TStaticBitArray& Other)
	{
		for(int32 Index = 0; Index < NumWords; ++Index)
		{
	...
	[[nodiscard]] friend inline TStaticBitArray<NumBits> operator~(const TStaticBitArray<NumBits>& A)
	{
	...
	[[nodiscard]] friend inline TStaticBitArray<NumBits> operator|(const TStaticBitArray<NumBits>& A, const TStaticBitArray<NumBits>& B)
	{
	...
	[[nodiscard]] inline bool operator==(const TStaticBitArray<NumBits>& B) const
	{
		for(int32 Index = 0; Index < NumWords; ++Index)
		{
	...
	[[nodiscard]] friend UE_FORCEINLINE_HINT bool operator==(const TStaticBitArray<NumBits>& A, UnspecifiedBoolType Value)
	{
		return (UnspecifiedBoolType)A == Value;
	}
	/** != simple maps to == */
	[[nodiscard]] UE_FORCEINLINE_HINT bool operator!=(const TStaticBitArray<NumBits>& B) const
	{
		return !(*this == B);
	}
	/** != simple maps to == */

...
public:
	...
	{
		return ActualRequest->UsesCache();
	}
};
...
public:
	...
	virtual bool		FileJournalIsAvailable(const TCHAR* VolumeOrPath = nullptr,
		ELogVerbosity::Type* OutErrorLevel = nullptr, FString* OutError = nullptr) override
	{
		return LowerLevel->FileJournalIsAvailable(VolumeOrPath, OutErrorLevel, OutError);
	}
	virtual bool		FileJournalIterateDirectory(const TCHAR* Directory,
		FDirectoryJournalVisitorFunc Visitor, FString* OutError = nullptr) override
	{
		return LowerLevel->FileJournalIterateDirectory(Directory, Visitor, OutError);
	}
	...
	{
		return LowerLevel->FileJournalGetLatestEntry(VolumeName, OutJournalId, OutEntryHandle, OutError);
	}
	...
	{
		return LowerLevel->FileJournalReadModified(VolumeName, JournalIdOfStartingEntry, StartingJournalEntry,
			KnownDirectories, OutModifiedDirectories, OutNextJournalEntry, OutError);
	}
	virtual FFileJournalData FileJournalGetFileData(const TCHAR* FilenameOrDirectory,
		FString* OutError = nullptr) override
	{
		return LowerLevel->FileJournalGetFileData(FilenameOrDirectory, OutError);
	}
	virtual FString		FileJournalGetVolumeName(FStringView InPath) override
	{
		return LowerLevel->FileJournalGetVolumeName(InPath);
	}
	virtual uint64		FileJournalGetMaximumSize(const TCHAR* VolumeOrPath = nullptr,
		ELogVerbosity::Type* OutErrorLevel = nullptr, FString* OutError = nullptr) const override
	{
		return LowerLevel->FileJournalGetMaximumSize(VolumeOrPath, OutErrorLevel, OutError);
	}

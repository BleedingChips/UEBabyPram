...
#if DO_CHECK && (!UE_BUILD_SHIPPING) && !defined(__clang_analyzer__) // Disable even if checks in shipping are enabled.
...
enum EConsoleVariableFlags
{
	...
	// Similar to ECVF_Set_NoSinkCall_Unsafe, but this will do nothing but set the cvar and flags, and push updates to render thread, no sinks, etc
	...
	// Reserved value for SETBY_ERROR. Please don't use.
	// SETBY_ERROR =				0x01000000 
	...
	ECVF_SetByGameSetting =			0x03000000,
	...
	ECVF_SetByProjectSetting =		0x04000000,
	...
	ECVF_SetByPluginLowPriority =	0x06000000,
...
public:
	...
	struct FSetContext
	{
		...
		enum class EPriorityMode : uint8
		{
			Set,
			// the following are limited by Max priority if its set
			ReplaceCurrent,
		};

		enum class ETagMode : uint8
		{
			// Sets the Tag at the end of the array. Remove the Tag if it already exists.
			Set,
			// Replace the Tag in-place if it already exists. Sets the Tag at the end if it doesn't exist.
			Replace,
			// Replaces the current highest Tag.
			ReplaceCurrent,
		};


		EConsoleVariableFlags Flags = ECVF_SetByCode;
		EPriorityMode PriorityMode = EPriorityMode::Set;

		FName Tag = NAME_None;
		ETagMode TagMode = ETagMode::Set;
	 ...
	 * @param InValue Internal Value to Set
	 * @param Context The ResolvedContext of the FSetContext
	 **/
	...
	*/
	...
	{
		Set(InValue, this->ResolveContext(Context));
	}
	...
	{
		Set(Value, FSetContext(Flags, Tag));
	}
	...
	void SetWithCurrentPriority(
		...
		EConsoleVariableFlags MinPriority = SETBY_ERROR)
	{
		checkf(MinPriority <= MaxPriority, TEXT("MinPriority must be <= MaxPriority"))
		FSetContext Context;
		Context.Tag = Tag;
		Context.PriorityMode = FSetContext::EPriorityMode::ReplaceCurrent;
		Context.MinPriority = MinPriority;
		Context.MaxPriority = MaxPriority;
		EConsoleVariableFlags CurFlags = (EConsoleVariableFlags)(GetFlags() & ECVF_SetByMask);
		Context.Flags = CurFlags;
		Set(Value, Context);
	}
...
struct IConsoleManager
{
	...
	*/
	...
	inline static IConsoleManager& Get()
	{
		if (!Singleton)
		{
			SetupSingleton();
...
struct TGuardConsoleVariable
{
	...
	TGuardConsoleVariable(const TCHAR* ConsoleVariableName, const T& NewValue)
		: TGuardConsoleVariable(IConsoleManager::Get().FindConsoleVariable(ConsoleVariableName), NewValue)
	{
		ensureMsgf(ConsoleVariable, TEXT("TGuardConsoleVariable: Trying to set console variable \"%s\" but it doesn't exist"), ConsoleVariableName);
	}
	...
	TGuardConsoleVariable(TGuardConsoleVariable&& Right)
		: PreviousValue(::MoveTemp(Right.PreviousValue))
		, ConsoleVariable(Right.ConsoleVariable)
	{
		Right.ConsoleVariable = nullptr;
	}
...
public:
	...
	CORE_API virtual ~FAutoConsoleObject();
	...
	inline IConsoleVariable* AsVariable()
	{
		checkSlow(Target->AsVariable());
		return static_cast<IConsoleVariable*>(Target);
	}
	/** returns the contained console object as an IConsoleVariable **/
	inline const IConsoleVariable* AsVariable() const
	{
		checkSlow(Target->AsVariable());

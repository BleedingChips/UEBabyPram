...
namespace EFieldLayoutFlags
{
	...
	UE_FORCEINLINE_HINT Type MakeFlags(uint32 Flags = None) { return (Type)Flags; }
	UE_FORCEINLINE_HINT Type MakeFlagsEditorOnly(uint32 Flags = None) { return (Type)(WithEditorOnly | Flags); }
	UE_FORCEINLINE_HINT Type MakeFlagsRayTracing(uint32 Flags = None) { return (Type)(WithRayTracing | Flags); }
}
...
namespace Freeze
{
	...
	template<typename T>
	inline uint32 IntrinsicUnfrozenCopy(const FMemoryUnfreezeContent& Context, const T& Object, void* OutDst)
	{
		new(OutDst) T(Object);
		return sizeof(T);
	}
...
{
	static UE_FORCEINLINE_HINT FTypeLayoutDesc::FWriteFrozenMemoryImageFunc* Do() { return &Freeze::DefaultWriteMemoryImage; }
};

template <typename T, bool bProvidesStaticStruct=TModels_V<CStaticStructProvider, T>>
struct TGetFreezeImageFieldHelper
{
	static UE_FORCEINLINE_HINT FFieldLayoutDesc::FWriteFrozenMemoryImageFunc* Do() { return &Freeze::DefaultWriteMemoryImageField; }
};
...
template<typename T>
static UE_FORCEINLINE_HINT void InternalInitializeBasesHelper(FTypeLayoutDesc& TypeDesc) {}
...
#define INTERNAL_DECLARE_TYPE_LAYOUT_COMMON(T, InInterface) \
	...
	UE_DECLARE_INTERNAL_LINK_BASE(InternalLinkType) { UE_STATIC_ONLY(InternalLinkType); static UE_FORCEINLINE_HINT void Initialize(FTypeLayoutDesc& TypeDesc) {} }

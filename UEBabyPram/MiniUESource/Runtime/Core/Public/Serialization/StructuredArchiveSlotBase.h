...
namespace UE::StructuredArchive::Private
{
	...
	public:
		...
		inline explicit FSlotPosition(int32 InDepth, FElementId InElementId)
			: Depth(InDepth)
	...
	public:
#if WITH_TEXT_ARCHIVE_SUPPORT
		inline explicit FSlotBase(EPrivateToken, FStructuredArchive& InStructuredArchive, int32 InDepth, FElementId InElementId)
			: FSlotPosition(InDepth, InElementId)
			, StructuredArchive(InStructuredArchive)
		{
		}
#else
		UE_FORCEINLINE_HINT explicit FSlotBase(EPrivateToken, FStructuredArchive& InStructuredArchive)
			: StructuredArchive(InStructuredArchive)
		{
		}
#endif

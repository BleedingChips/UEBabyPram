...
public:
	...
	UE_FORCEINLINE_HINT bool IsMigratingRemoteObjects() const
	{
	...
	inline bool RequiresLocalizationGather() const
	{
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	inline uint32 GetDebugSerializationFlags() const
	{
#if WITH_EDITOR
	...
	/**
	 * Indicates when the archive is serializing an Optional object. Will only return true
	 * when Ar.IsSaving() is also true
	 * @see EClassFlags::CLASS_Optional
	 *
	 * @return true if currently saving optional data
	 */
	bool IsSavingOptionalObject () const
	{
		return ArIsSavingOptionalObject;
	}
	...
	{
		check(IsSaving());
		ArIsSavingOptionalObject = InSavingOptionalObject;
	}
...
public:
	 ...
	 */
	inline TCheckedObjPtr& operator = (T* InObject)
	{
		Object = InObject;
...
public:
	 ...
	 * @return This instance.
	 */
	template<class T> inline FArchive& operator<<(TCheckedObjPtr<T>& Value)
	{
		Value.bError = false;
	...
	*/
	 ...
	 * @param Value The value to serialize.
	 */
	inline friend FArchive& operator<<(FArchive& Ar, UTF32CHAR& Value)
	{
#if DEVIRTUALIZE_FLinkerLoad_Serialize
		if (!Ar.FastPathLoad<sizeof(Value)>(&Value))
#endif
	 ...
	 * @param Value The value to serialize.
	 */
	inline friend FArchive& operator<<(FArchive& Ar, uint16& Value)
	{
#if DEVIRTUALIZE_FLinkerLoad_Serialize
		if (!Ar.FastPathLoad<sizeof(Value)>(&Value))
#endif
	 ...
	 * @param Value The value to serialize.
	 */
	inline friend FArchive& operator<<(FArchive& Ar, int16& Value)
	{
#if DEVIRTUALIZE_FLinkerLoad_Serialize
		if (!Ar.FastPathLoad<sizeof(Value)>(&Value))
#endif
	 ...
	 * @param Value The value to serialize.
	 */
	inline friend FArchive& operator<<(FArchive& Ar, uint32& Value)
	{
#if DEVIRTUALIZE_FLinkerLoad_Serialize
		if (!Ar.FastPathLoad<sizeof(Value)>(&Value))
#endif
...
public:
	 ...
	 * @param Value The value to serialize.
	 */
	inline friend FArchive& operator<<(FArchive& Ar, long& Value)
	{
#if DEVIRTUALIZE_FLinkerLoad_Serialize
		if (!Ar.FastPathLoad<sizeof(Value)>(&Value))
#endif
	 ...
	 * @param Value The value to serialize.
	 */
	inline friend FArchive& operator<<(FArchive& Ar, double& Value)
	{
#if DEVIRTUALIZE_FLinkerLoad_Serialize
		if (!Ar.FastPathLoad<sizeof(Value)>(&Value))
#endif
	 ...
	 * @param Value The value to serialize.
	 */
	inline friend FArchive& operator<<(FArchive &Ar, uint64& Value)
	{
#if DEVIRTUALIZE_FLinkerLoad_Serialize
		if (!Ar.FastPathLoad<sizeof(Value)>(&Value))
#endif
	 ...
	 * @param Value The value to serialize.
	 */
	/*inline*/friend FArchive& operator<<(FArchive& Ar, int64& Value)
	{
#if DEVIRTUALIZE_FLinkerLoad_Serialize
		if (!Ar.FastPathLoad<sizeof(Value)>(&Value))
#endif
...
#define SerializeBitfield(Ar, BitField) \
	...
	*/
	...
	inline FArchive& ByteOrderSerialize(void* V, int32 Length)
	{
...
public:
	...
	using FArchiveState::IsSavingOptionalObject;
	using FArchiveState::SetSavingOptionalObject;
...
public:
	...
	inline bool FastPathLoad(void* InDest)
	{
...
#else
	template<SIZE_T Size>
	UE_FORCEINLINE_HINT bool FastPathLoad(void* InDest)
	{
		return false;
	}
#endif
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	using FArchiveState::ArIsSavingOptionalObject;
...
inline FArchive& operator<<(FArchive& Ar, TEnumAsByte<TEnum>& Value)
{
#if DEVIRTUALIZE_FLinkerLoad_Serialize
	if (!Ar.FastPathLoad<sizeof(Value)>(&Value))
#endif

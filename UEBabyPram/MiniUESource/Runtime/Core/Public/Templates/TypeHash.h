...
>
[[nodiscard]] inline constexpr uint32 HashCombine(uint32 A, Types... Vals)
{
...
template <
	...
	UE_REQUIRES((std::is_convertible_v<Types, uint32> && ...))
>
[[nodiscard]] inline constexpr uint32 HashCombineFast(uint32 A, Types... Vals)
{
	if constexpr (sizeof...(Types) > 0)
	{
...
template <typename T>
[[nodiscard]] UE_FORCEINLINE_HINT uint32 GetTypeHashHelper(const T& V) { return static_cast<uint32>(GetTypeHash(V)); }

...
namespace Algo
{
	...
	[[nodiscard]] SIZE_T Count(const InT& Input, const ValueT& InValue)
	{
		SIZE_T Result = 0;
	...
	[[nodiscard]] SIZE_T CountIf(const InT& Input, PredicateT Predicate)
	{
		SIZE_T Result = 0;
		for (const auto& Value : Input)
		{
	...
	[[nodiscard]] SIZE_T CountBy(const InT& Input, const ValueT& InValue, ProjectionT Proj)
	{
		SIZE_T Result = 0;
		for (const auto& Value : Input)
		{

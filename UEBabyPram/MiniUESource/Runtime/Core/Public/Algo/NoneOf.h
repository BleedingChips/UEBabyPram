...
namespace Algo
{
	...
	[[nodiscard]] bool NoneOf(const RangeType& Range)
	{
	...
	[[nodiscard]] bool NoneOf(const RangeType& Range, ProjectionType Projection)
	{
		for (const auto& Element : Range)
		{
	 ...
	 */
	template <typename RangeType, typename ProjectionType>
	[[nodiscard]] bool NoneOf(const RangeType& Range, ProjectionType Projection, ENoRef NoRef)
	{
		for (const auto Element : Range)
		{
			if (Invoke(Projection, Element))

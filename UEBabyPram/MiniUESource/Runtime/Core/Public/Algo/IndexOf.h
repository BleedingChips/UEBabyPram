...
namespace Algo
{
	...
	[[nodiscard]] UE_REWRITE auto IndexOf(const RangeType& Range, const ValueType& Value)
	{
	 ...
	 */
	template <typename RangeType, typename ValueType, typename ProjectionType>
	[[nodiscard]] UE_REWRITE auto IndexOfBy(const RangeType& Range, const ValueType& Value, ProjectionType Proj)
	{
		static_assert(TIsContiguousContainer<RangeType>::Value, "Must only be used with contiguous containers");
	 ...
	 */
	template <typename RangeType, typename PredicateType>
	[[nodiscard]] UE_REWRITE auto IndexOfByPredicate(const RangeType& Range, PredicateType Pred)
	{
		static_assert(TIsContiguousContainer<RangeType>::Value, "Must only be used with contiguous containers");

...
public:
	 ...
	 * 
	 * @param InModuleName Name of the module to check.
	 * @return true for a module that exists and is not compiled into the executable.
	 */
...
public:
	...
	/** Raw function type used during module registration. */
	typedef IModuleInterface *FInitializeStaticallyLinkedModuleRaw();
	...
	*/
	...
	{
		return OnModulesUnloadCallback;
	}
...
private:
	...
	UE_FORCEINLINE_HINT TSharedPtr<const FModuleInfo, ESPMode::ThreadSafe> FindModule(FName InModuleName) const
	{
		return const_cast<FModuleManager*>(this)->FindModule(InModuleName);
	}

	UE_FORCEINLINE_HINT TSharedRef<const FModuleInfo, ESPMode::ThreadSafe> FindModuleChecked(FName InModuleName) const
	{
	...
	/** Get all loaded modules linked into a particular merged library */
	TArray<FName> GetLoadedModulesForLibrary(const FString& LibraryFilename) const;

	/** Get all loaded merged libraries that depend on a particular shared merged library */
	TArray<FName> GetLoadedLibrariesForSharedLibrary(const FString& LibraryFilename) const;

	/** Get all shared library dependencies, loaded or not, for a particular merged library */
	TArray<FName> GetAllSharedLibrariesForLibrary(const FString& LibraryFilename) const;
...
private:
	...
	/** Check if we're in the middle of loading a dynamic library */
	bool bIsLoadingDynamicLibrary;
	...
	TMulticastDelegate<void(TConstArrayView<FName>)> OnModulesUnloadCallback;
	...
	/** Dependency map for merged libraries, for use with shared merged libraries */
	mutable TMap<FString, TArray<FString>> LibraryDependencies;

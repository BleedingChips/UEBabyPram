...
public:
	...
	[[nodiscard]] static CORE_API FText AsLocalizable_Advanced_LocText(const TCHAR* Namespace, const TCHAR* Key, const TCHAR* String); // For use by LOCTEXT to avoid constructing temp FTextKeys in every caller
	 ...
	 */
	UE_DEPRECATED(5.7, "FText::IsWhitespace is deprecated. Use FTextChar::IsWhitespace instead.")
	...
	[[nodiscard]] static inline FText Format(FTextFormat Fmt, ArgTypes... Args)
	{
	 ...
	 */
	template <typename... ArgTypes>
	[[nodiscard]] static inline FText Join(const FText& Delimiter, ArgTypes... Args)
	{
...
public:
	...
	inline int64 GetIntValue() const
	{
		check(Type == EFormatArgumentType::Int);
		return IntValue;
	}

	inline uint64 GetUIntValue() const
	{
	...
	inline const FText& GetTextValue() const
	{
		check(Type == EFormatArgumentType::Text);
		return TextValue.GetValue();
	}

	inline ETextGender GetGenderValue() const
	{
...
public:
	 ...
	 */
	template <typename... ArgTypes>
	inline void AppendLineFormat(FTextFormat Pattern, ArgTypes... Args)
	{
...
/** Unicode character helper functions (see FTextChar for more functionality) */
struct FUnicodeChar
{
	UE_DEPRECATED(5.7, "FUnicodeChar::CodepointToString is deprecated. Use FTextChar::ConvertCodepointToString instead.")

...
#    define INSTRUMENT_DELEGATE_MODULE() FTrackedDelegateInstanceExtras::SetModuleName(UE_MODULE_NAME)
#  else
#    define INSTRUMENT_DELEGATE_MODULE()
#  endif // UE_MODULE_NAME
#  define INSTRUMENT_DELEGATE_FUNCTION(Function) FTrackedDelegateInstanceExtras::SetBoundFunctionName(Function)
#else
#  define INSTRUMENT_DELEGATE_MODULE()
...
public:
		...
		, UserObjectPtr(InUserObject)
	{
		...
		INSTRUMENT_DELEGATE_FUNCTION(*UserClass::StaticClass()->GetName());
...
public:
	...
	{
		using FParmsWithPayload = TPayload<RetValType(std::decay_t<ParamTypes>..., std::decay_t<VarTypes>...)>;
...
public:
		...
		, MethodPtr (InMethodPtr)
	{
		...
		INSTRUMENT_DELEGATE_MODULE();
		INSTRUMENT_DELEGATE_FUNCTION(__FUNCTION__);
	}
...
{
private:
	static_assert(std::is_same_v<FunctorType, std::remove_reference_t<FunctorType>>, "FunctorType cannot be a reference");
...
public:
	...
	{
		INSTRUMENT_DELEGATE_MODULE();
		INSTRUMENT_DELEGATE_FUNCTION(*InContextObject->GetName());
	}

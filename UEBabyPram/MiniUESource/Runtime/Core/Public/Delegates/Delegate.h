...
#define ENABLE_STATIC_FUNCTION_FNAMES (PLATFORM_COMPILER_CLANG && (__clang_major__ > 3 || (__clang_major__ == 3 && __clang_minor__ >= 5)))
	...
	namespace NStrAfterLastDoubleColon_Private
	{
		...
		struct TStaticFNameFromCharSequence<TIntegerSequence<T, Chars...>>
		{
			static FName Get()
			{
				static const T Str[sizeof...(Chars) + 1] = { Chars..., 0 };
				FName* Result = nullptr;
				UE_AUTORTFM_OPEN
				{
					static FName StaticResult{Str};
					Result = &StaticResult;
				};
				return *Result;
			}
	 ...
	 */
	template <typename T, T... Chars>
	UE_FORCEINLINE_HINT constexpr TIntegerSequence<T, Chars...> operator""_intseq()
	{
		return {};
	}
...
namespace UE::Delegates::Private
{
	 ...
	 */
	inline FName GetTrimmedMemberFunctionName(const ANSICHAR* InMacroFunctionName)
	{
		// We strip off the class prefix and just return the function name by itself.
		check(InMacroFunctionName);
		const ANSICHAR* Result = FCStringAnsi::Strrstr( InMacroFunctionName,  "::" );
		checkf(Result && Result[2] != '0', TEXT("'%hs' does not look like a member function"), InMacroFunctionName);

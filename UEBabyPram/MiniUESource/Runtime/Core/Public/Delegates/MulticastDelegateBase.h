...
public:
	...
	inline bool IsBoundToObject(FDelegateUserObjectConst InUserObject) const
	{
		...
		for (const UnicastDelegateType& DelegateBaseRef : InvocationList)
		{
			...
			CHECK_DELEGATE_LIFETIME(DelegateInstance);
	...
	/**
	 * Gets a list of all delegate call sites & module names registered to this multicast
	 *
	 * @return A list of pair of call site + module name, including for delegates that were unbound but not yet compacted.
	 */
	TArray<TPair<FName, FName>> GetInvocationNames() const
	{
		TArray<TPair<FName, FName>> FoundFunctionsAndModules;
		...
		for (const UnicastDelegateType& DelegateBaseRef : InvocationList)
		{
			...
			if (DelegateInstance != nullptr)
			{
				const FTrackedDelegateInstanceExtras* TrackedDelegateInstance = static_cast<const FTrackedDelegateInstanceExtras*>(DelegateInstance);

				FoundFunctionsAndModules.Add(TPair<FName, FName>(
					TrackedDelegateInstance->GetBoundFunctionName(),
					TrackedDelegateInstance->GetModuleName()));
			}
		}

		return FoundFunctionsAndModules;
	}
	...
	void CopyFrom(const TMulticastDelegateBase& Other)
	{
...

		{
		    ...
		    for (const UnicastDelegateType& OtherDelegateRef : Other.GetInvocationList())
		    {
			    ...
			    {
					CHECK_DELEGATE_LIFETIME(OtherInstance);
	...
	void Broadcast(ParamTypes... Params) const
	{
		...
		LockInvocationList();
		{
			...
			for (int32 InvocationListIndex = LocalInvocationList.Num() - 1; InvocationListIndex >= 0; --InvocationListIndex)
			{
				...
				CHECK_DELEGATE_LIFETIME(DelegateInstanceInterface);

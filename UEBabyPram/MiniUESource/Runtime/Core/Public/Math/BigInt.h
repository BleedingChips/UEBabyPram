...
private:
	 ...
	 */
	inline static void MakePositiveFactors(BigInt& FactorA, int32& SignA, BigInt& FactorB, int32& SignB)
	{
		if (bSigned)
	  {
		  SignA = FactorA.Sign();
	 ...
	 */
	inline static void RestoreSign(BigInt& Result, int32 SignA, int32 SignB)
	{
...
public:
	...
	[[nodiscard]] UE_FORCEINLINE_HINT uint32* GetBits()
	{
		return Bits;
	}

	[[nodiscard]] UE_FORCEINLINE_HINT const uint32* GetBits() const
	{
		return Bits;
	}

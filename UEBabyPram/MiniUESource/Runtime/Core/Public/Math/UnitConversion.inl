...
T FUnitConversion::Convert(T InValue, EUnit From, EUnit To)
{
	...
	{
		case EUnitType::Distance:			return static_cast<T>(InValue * DistanceUnificationFactor(From)				* (1.0 / DistanceUnificationFactor(To)));
		case EUnitType::Angle:				return static_cast<T>(InValue * AngleUnificationFactor(From) 				* (1.0 / AngleUnificationFactor(To)));
		case EUnitType::Speed:				return static_cast<T>(InValue * SpeedUnificationFactor(From) 				* (1.0 / SpeedUnificationFactor(To)));
		case EUnitType::AngularSpeed:		return static_cast<T>(InValue * AngularSpeedUnificationFactor(From)			* (1.0 / AngularSpeedUnificationFactor(To)));
		case EUnitType::Acceleration:		return static_cast<T>(InValue * AccelerationUnificationFactor(From)			* (1.0 / AccelerationUnificationFactor(To)));
		case EUnitType::Mass:				return static_cast<T>(InValue * MassUnificationFactor(From) 				* (1.0 / MassUnificationFactor(To)));
		case EUnitType::Density:			return static_cast<T>(InValue * DensityUnificationFactor(From)				* (1.0 / DensityUnificationFactor(To)));
		case EUnitType::Force:				return static_cast<T>(InValue * ForceUnificationFactor(From) 				* (1.0 / ForceUnificationFactor(To)));
		case EUnitType::Torque:				return static_cast<T>(InValue * TorqueUnificationFactor(From)				* (1.0 / TorqueUnificationFactor(To)));
		case EUnitType::PositionalImpulse:	return static_cast<T>(InValue * PositionalImpulseUnificationFactor(From)	* (1.0 / PositionalImpulseUnificationFactor(To)));
		case EUnitType::Frequency:			return static_cast<T>(InValue * FrequencyUnificationFactor(From) 			* (1.0 / FrequencyUnificationFactor(To)));
		case EUnitType::DataSize:			return static_cast<T>(InValue * DataSizeUnificationFactor(From) 			* (1.0 / DataSizeUnificationFactor(To)));
		case EUnitType::LuminousFlux:		return InValue;	
		case EUnitType::Time:				return static_cast<T>(InValue * TimeUnificationFactor(From) 				* (1.0 / TimeUnificationFactor(To)));
		case EUnitType::Multipliers:		return static_cast<T>(InValue * MultiplierUnificationFactor(From) 			* (1.0 / MultiplierUnificationFactor(To)));
		case EUnitType::Stress:				return static_cast<T>(InValue * StressUnificationFactor(From)				* (1.0 / StressUnificationFactor(To)));
		...
		case EUnitType::Temperature:
		{
			...
			{
				NewValue = (NewValue + 459.67) * 5.0/9.0;
			}
			// And out again
			if (To == EUnit::Celsius)
			{
				return static_cast<T>(NewValue - 273.15);
			}
			else if (To == EUnit::Farenheit)
			{
				return static_cast<T>(NewValue * 9.0/5.0 - 459.67);
			}
			else
			{

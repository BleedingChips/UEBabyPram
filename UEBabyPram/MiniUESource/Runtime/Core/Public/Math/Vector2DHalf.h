...
public:
	...
	[[nodiscard]] UE_FORCEINLINE_HINT FVector2DHalf() { }
	 ...
	 */
 	[[nodiscard]] UE_FORCEINLINE_HINT FVector2DHalf( const FFloat16& InX,const FFloat16& InY );
	 ...
	 * Iny float Y value
	 */
	[[nodiscard]] UE_FORCEINLINE_HINT FVector2DHalf( float InX,float InY );

	/** Constructor 
	 *
	 * Vector2D float vector
	 */
	[[nodiscard]] UE_FORCEINLINE_HINT FVector2DHalf( const FVector2f& Vector2D );
	[[nodiscard]] UE_FORCEINLINE_HINT FVector2DHalf( const FVector2d& Vector2D );// LWC_TODO: This should probably be explicit
...
public:
	...
	[[nodiscard]] inline uint32 AsUInt32() const
	{
...
UE_FORCEINLINE_HINT FVector2DHalf::FVector2DHalf( const FFloat16& InX, const FFloat16& InY )
 	:	X(InX), Y(InY)
{ }


UE_FORCEINLINE_HINT FVector2DHalf::FVector2DHalf( float InX, float InY )
	:	X(InX), Y(InY)
{ }
...
inline FVector2DHalf& FVector2DHalf::operator=( const FVector2f& Vector2D )
{
...
inline void FVector2DHalf::SetTruncate( float InX, float InY )
{
	X.SetTruncate(InX);
	Y.SetTruncate(InY);
}

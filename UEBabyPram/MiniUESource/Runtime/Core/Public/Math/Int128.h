...
private:
	...
	inline uint32 AddInternal(uint32 A, uint32 B, uint32& Carry)
	{
	...
	inline uint32 MultiplyInternal(uint32 Multiplicand, uint32 Multiplier, uint32& Carry)
	{
		uint64 Result;
	...
	inline uint32 DivideInternal(uint32 Dividend, uint32 Divisor, uint32& Remainder)
	{
...
public:
	...
	[[nodiscard]] inline uint32 GetQuadPart(uint32 Part) const
	{
		switch (Part)
		{
		case 3: return Hi >> 32;
	...
	inline void SetQuadPart(uint32 Part, uint32 Value)
	{
		switch (Part)
		{
	...
	inline void Set(uint64 InHi, uint64 InLo)
	{
		Hi = InHi;
		Lo = InLo;
	}
    ...
    [[nodiscard]] UE_FORCEINLINE_HINT FUInt128(const FUInt128&) = default;
    [[nodiscard]] UE_FORCEINLINE_HINT FUInt128(FUInt128&&) = default;
	...
	[[nodiscard]] inline FUInt128()
		: Hi(0)
		, Lo(0)
	{}

	/** Constructor. Initializes this uint128 with a uint64 value. */
	[[nodiscard]] inline FUInt128(uint64 A)
		: Hi(0)
		, Lo(A)
	{}

	/** Constructor. Initializes this uint128 with two uint64 values. */
	[[nodiscard]] inline FUInt128(uint64 A, uint64 B)
		: Hi(A)
		, Lo(B)
	{}

	/** Constructor. Initializes this uint128 with four uint32 values. */
	[[nodiscard]] inline FUInt128(uint32 A, uint32 B, uint32 C, uint32 D)
		: Hi(((uint64)A << 32) | B)
		, Lo(((uint64)C << 32) | D)
	{}

	/** this > Other */
	[[nodiscard]] inline bool IsGreater(const FUInt128& Other) const
	{
		if (Hi == Other.Hi)
		{
			return Lo > Other.Lo;
	...
	/** this >= Other */
	[[nodiscard]] inline bool IsGreaterOrEqual(const FUInt128& Other) const
	{
		if (Hi == Other.Hi)
		{
			return Lo >= Other.Lo;
	...
	/** this < Other */
	[[nodiscard]] inline bool IsLess(const FUInt128& Other) const
	{
		if (Hi == Other.Hi)
		{
			return Lo < Other.Lo;
	...
	/** this <= Other */
	[[nodiscard]] inline bool IsLessOrEqual(const FUInt128& Other) const
	{
		if (Hi == Other.Hi)
		{
			return Lo <= Other.Lo;
	...
	/** this == Other */
	[[nodiscard]] UE_FORCEINLINE_HINT bool IsEqual(const FUInt128& Other) const
	{
		return (Hi == Other.Hi) && (Lo == Other.Lo);
	}

	/** Add an unsigned 32bit value */
	inline FUInt128 Add(uint32 Value)
	{
		uint32 Carry = 0;
	...
	inline FUInt128 Sub(uint32 Value)
	{
		uint32 Carry = 0;
	...
	inline FUInt128 Multiply(uint32 Multiplier)
	{
		uint32 Carry = 0;
	...
	inline FUInt128 Divide(uint32 Divisor, uint32& Remainder)
	{
		Remainder = 0;
	 ...
	 */
	[[nodiscard]] UE_FORCEINLINE_HINT bool operator>(const FUInt128& Other) const
	{
	...
	[[nodiscard]] UE_FORCEINLINE_HINT bool IsZero() const
	{
		return !(Hi | Lo);
	}

	[[nodiscard]] UE_FORCEINLINE_HINT bool IsGreaterThanZero() const
	{
		return !IsZero();
	}
	 ...
	 */
	inline FUInt128& operator+=(uint32 Other)
	{
		*this = Add(Other);
		return *this;
	}

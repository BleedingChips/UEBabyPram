...
public:
	...
	bool CanEnable(FPlatformEventsTrace::EEventType Event, const TCHAR** OutReason);
...
struct FContextSwitchEvent
{
	...
	uint32 Reserved;					// 20
	uint32 Reserved2;					// 24 (undocumented)
};
...
void FPlatformEvents::TraceEventCallback(PEVENT_RECORD Event)
{
	...
	if (Opcode == FContextSwitchEvent::Type)
	{
		...
		// Recent updates of ETW changes the size of the payload somewhat.
		constexpr uint32 UserDataSizeOld = sizeof(FContextSwitchEvent) - 4;
		constexpr uint32 UserDataSizeNew = sizeof(FContextSwitchEvent);
		if (Payload == nullptr ||
			(Event->UserDataLength != UserDataSizeOld &&
			 Event->UserDataLength != UserDataSizeNew))
		{
			return;
		}
	...
	else if (Opcode == FStackWalkEvent::Type)
	{
		...
		if (Payload == nullptr ||
			...
			Event->UserDataLength < sizeof(FStackWalkEvent))
		{
			return;
		}
...
/////////////////////////////////////////////////////////////////////
bool FPlatformEvents::CanEnable(FPlatformEventsTrace::EEventType Event, const TCHAR** OutReason)
{
	// In order to control tracing the process must either run under
	// elevated privileges OR the user has to be member of PERF_LOG_USERS
	static const TCHAR* ErrProcAccess = TEXT("Cannot access process.");
	static const TCHAR* ErrProcRights = TEXT("Process does not have privileges to start event tracing.");
	
#if PLATFORM_WINDOWS
	FHandleScope CurrentProcToken;
	if (!::OpenProcessToken(GetCurrentProcess(), TOKEN_QUERY, &CurrentProcToken.Handle))
	{
		if (OutReason)
		{
			*OutReason = ErrProcAccess;
		}
		UE_LOG(LogCore, Log, TEXT("Unable to enable platform events: %s"), ErrProcAccess);
		return false;
	}

	// Get the elevation token
	TOKEN_ELEVATION ElevationToken;
	DWORD ReturnSize = 0;
	if (!::GetTokenInformation(CurrentProcToken, TokenElevation, &ElevationToken, sizeof(TOKEN_ELEVATION), &ReturnSize))
	{
		if (OutReason)
		{
			*OutReason = ErrProcAccess;
	...
	// Check if the user is part of PERF_LOG_USERS
	// todo: Implement

	if (ElevationToken.TokenIsElevated /* || in perf log users */)
	{
		return true;
	}
	...
	if (OutReason)
	{
		*OutReason = ErrProcRights;
	}
	
	UE_LOG(LogCore, Log, TEXT("Unable to enable platform events: %s"), ErrProcRights);
	return false;
#else
	return true;
#endif
...
{
	FPlatformEventsTrace::OutputSettings(SamplingIntervalUsec);

	GPlatformEvents = new FPlatformEvents(SamplingIntervalUsec);
}

bool FPlatformEventsTrace::CanEnable(FPlatformEventsTrace::EEventType Event, const TCHAR** OutReason)
{
	if (GPlatformEvents)
	{
		return GPlatformEvents->CanEnable(Event, OutReason);
	}
	return false;
}

...
static TSAN_ATOMIC(bool) GCsvCategoriesEnabled[CSV_MAX_CATEGORY_COUNT];

static TSAN_ATOMIC(bool) GCsvProfilerIsCapturing = false;
...
static TMap<uint32, TArray<FString>>* GCsvFrameModuloExecCmds = NULL;
...
void FCsvProfiler::GetFrameExecCommands(TArray<FString>& OutFrameCommands)
{
	...
	if (GCsvFrameModuloExecCmds)
	{
		for (auto It(GCsvFrameModuloExecCmds->CreateConstIterator()); It; ++It)
		{
			uint32 FrameModulo = It.Key();
			if ((CaptureFrameNumber % FrameModulo) == 0)
			{
				OutFrameCommands.Append(It.Value());
			}
		}
	}
...
public:
	...
	static FORCENOINLINE FCsvProfilerThreadData* CreateTLSData()
	{
		...
		UE::TScopeLock Lock(Singleton.TlsCS);
...
public:
	...
	void StallGameThreadIfNeeded()
	{
		...
		UE::TScopeLock Lock(StallGameThreadCs);
	}
	...
	, FileWriteBlockingEvent(FPlatformProcess::GetSynchEventFromPool())
{
	...
	FString EngineReleaseVersionString = FEngineVersion::Current().ToString(EVersionComponent::Patch);
	...
	SetMetadataInternal(TEXT("EngineReleaseVersion"), *EngineReleaseVersionString);
	...
	SetMetadataInternal(TEXT("MergedModules"), UE_MERGED_MODULES ? TEXT("1") : TEXT("0"));
...
TSharedFuture<FString> FCsvProfiler::EndCapture(FGraphEventRef EventToSignal)
{
	...
	{
		// In a transaction we cannot support signalling an event.
		ensure(!EventToSignal.IsValid());

		// If we are in a transaction we only want to actually end the capture if we successfully
		// committed the transaction. It is safe to do this because the capture API doesn't end
		// the capture immediately, but at the next end-frame tick (in `EndFrame`).
		AutoRTFM::OnCommit([this]
		{
			EndCapture();
		});

		// TODO: Could we make a special shared future that would explode if used? We cannot support
		// shared future returns from within a transaction.
		return {};
	}
...
void FCsvProfiler::Init()
{
	...
	if (FParse::Value(FCommandLine::Get(), TEXT("-csvExecCmds="), CsvExecCommandsStr, false))
	{
		...
		GCsvFrameModuloExecCmds = new TMap<uint32, TArray<FString>>();
		...
		if (CsvExecCommandsStr.ParseIntoArray(CsvExecCommandsList, TEXT(","), true) > 0)
		{
			for (FString FrameAndCommand : CsvExecCommandsList)
			{
				...
				{
					FString FrameStr = FrameAndCommand.Mid(0,ColonIndex).ToLower();
					FString CommandStr = FrameAndCommand.Mid(ColonIndex+1);

					// Modulo frames are prefixed with "r". The command will be executed every n frames
					bool bModuloCommand = FrameStr.StartsWith(TEXT("r"));
					if (bModuloCommand)
					{
						FrameStr = FrameAndCommand.Mid(1, ColonIndex);
					}

					TMap<uint32, TArray<FString>>* ExecCmdList = bModuloCommand ? GCsvFrameModuloExecCmds : GCsvFrameExecCmds;
					uint32 Frame = FCString::Atoi(*FrameStr);
					if (!ExecCmdList->Find(Frame))
					{
						ExecCmdList->Add(Frame, TArray<FString>());
					}
					(*ExecCmdList)[Frame].Add(CommandStr);

					if ( bModuloCommand )
					{
						UE_LOG(LogCsvProfiler, Display, TEXT("Added CsvExecCommand (repeating) - Modulo: %d : %s"), Frame, *CommandStr);
					}
					else
					{

...
public:
	...
	{
		SuperLuminalStartScopedEventWide(Text, nullptr, UPerformanceAPI_Pack(Color.R, Color.G, Color.B));
	}

	/** Starts a scoped event specific to the profiler. */
	virtual void StartScopedEvent(const struct FColor& Color, const ANSICHAR* Text) override
	{
		SuperLuminalStartScopedEvent(Text, nullptr, UPerformanceAPI_Pack(Color.R, Color.G, Color.B));
	}

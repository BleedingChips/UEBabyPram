...
private:
	...
	FValidLineInfo AcquireSlotAndReadLine(FFileCache& Cache, CacheLineID LineID, EAsyncIOPriorityAndFlags Priority);
...
FFileCacheHandle::FValidLineInfo FFileCacheHandle::AcquireSlotAndReadLine(FFileCache& Cache, CacheLineID LineID, EAsyncIOPriorityAndFlags Priority)
{
	SCOPED_LOADTIMER(FFileCacheHandle_AcquireSlotAndReadLine);

	// Check if there's a valid line already and return that
	{
...
IMemoryReadStreamRef FFileCacheHandle::ReadData(FGraphEventArray& OutCompletionEvents, int64 InOffset, int64 BytesToRead, EAsyncIOPriorityAndFlags Priority)
{
	...
	{
		FValidLineInfo ValidLineInfo = AcquireSlotAndReadLine(Cache, LineID, Priority);

		FLineInfo& LineInfo = ValidLineInfo.LineInfo;
		if (ValidLineInfo.bWasValid)
		{
			// if it was already valid lock the slot for reading
...
FGraphEventRef FFileCacheHandle::PreloadData(const FFileCachePreloadEntry* PreloadEntries, int32 NumEntries, EAsyncIOPriorityAndFlags Priority)
{
	...
	for (int32 EntryIndex = 0; EntryIndex < NumEntries && Cache.NumFreeSlots > 0; ++EntryIndex)
	{
		...
		{
			FValidLineInfo ValidLineInfo = AcquireSlotAndReadLine(Cache, CurrentLine, Priority);

			FLineInfo& LineInfo = ValidLineInfo.LineInfo;
			
			// line info was not valid and just created, grab a new slot from cache and start a read request
			if (!ValidLineInfo.bWasValid)
			{

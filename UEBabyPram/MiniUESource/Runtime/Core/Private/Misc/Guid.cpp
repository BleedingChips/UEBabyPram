...
FGuid FGuid::NewGuidFromHash(const FBlake3Hash& Hash)
{
	...
	return NewGuidFromHashBytes(&Hash.GetBytes(), sizeof(Hash.GetBytes()));
}

FGuid FGuid::NewGuidFromHashBytes(const void* HashData, int64 DataLen)
{
	...
	if (DataLen < 16 || !IsAligned(HashData, alignof(uint32)))
	{
		...
		if (DataLen < 16)
		{
			FMemory::Memzero(&Buffer[DataLen], 16 - DataLen);
		}
		HashData = Buffer;
		DataLen = 16;
	}

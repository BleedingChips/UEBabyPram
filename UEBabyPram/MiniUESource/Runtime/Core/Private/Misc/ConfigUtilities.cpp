...
#if UE_EDITOR
TMap<FString, FString> CVarsToApplyForDlls;
#endif
...
void ApplyCVarsFromBootHotfix()
{
	...
	for (auto It = CVarsToApply.CreateIterator(); It; ++It)
	{
		IConsoleVariable* CVar = IConsoleManager::Get().FindConsoleVariable(*It->Key); 
		if (CVar && CVar->TestFlags(ECVF_SaveForNextBoot))
		{
			CVar->Set(*It->Value, ECVF_SetByHotfix);
			It.RemoveCurrent();
		}
	}

#if UE_EDITOR
	if (!CVarsToApply.IsEmpty())
	{
		CVarsToApplyForDlls = MoveTemp(CVarsToApply);
	}
#endif // UE_EDITOR
#endif // !UE_SERVER
}
...
#if UE_EDITOR
void ApplyCVarOfDllFromBootHotfix(const TCHAR* Key, IConsoleObject* Obj)
{
	FString* Value = CVarsToApplyForDlls.Find(Key);
	if (Value)
	{
		...
		CVarsToApplyForDlls.Remove(Key);
	}
}
#endif // UE_EDITOR
...
void OnSetCVarFromIniEntry(const TCHAR *IniFile, const TCHAR *Key, const TCHAR* Value, uint32 SetBy, bool bAllowCheating, bool bNoLogging, FName Tag)
{
	...
	if(CVar)
	{
		...
		if(SetBy == ECVF_SetByScalability)
		{
			...
			{
				FString Msg =  FString::Printf(TEXT("Scalability.ini can only set ECVF_Scalability console variables ('%s'='%s' is ignored)"), Key, Value);
				ensureMsgf(false, TEXT("%s"), *Msg);
				UE_LOG(LogConfig, Log, TEXT("%s"), *Msg);
				return;
			}
		}

...
{
	constexpr int32 InitialBufferSize = 128 * 1024;
	...
	if (!bDisableForkedOutputThread)
	{
		if (FPlatformProcess::SupportsMultithreading() || FForkProcessHelper::SupportsMultithreadingPostFork())
		{
			...
			int32 StackSize = 32 * 1024;
			FPlatformAtomics::InterlockedExchangePtr((void**)&Thread, FForkProcessHelper::CreateForkableThread(this, *WriterName, StackSize, TPri_BelowNormal));
		}
	}
}
...
bool FOutputDeviceFile::CreateWriter(uint32 MaxAttempts)
{
	...
	// Ignore re-entrant calls to CreateWriter because that will create and leak a writer.
	if (bCreatingWriter)
	{
		return true;
	}
	TGuardValue<bool> SerializingScope(bCreatingWriter, true);

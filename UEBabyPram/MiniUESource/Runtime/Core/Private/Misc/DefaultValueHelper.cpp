...
bool FDefaultValueHelper::ParseBool(const FString& Source, bool& OutVal)
{
	const bool bHasWhitespace = HasWhitespaces(Source);
	const FString NoWhitespace = bHasWhitespace ? RemoveWhitespaces(Source) : FString();
	const FString& ProperSource = bHasWhitespace ? NoWhitespace : Source;
	...
	const FString SourceToLower = ProperSource.ToLower();
	if (SourceToLower == TEXT("true"))
	{
		OutVal = true;
		return true;
	}
	
	if (SourceToLower == TEXT("false"))
	{
		OutVal = false;
		return true;
	}
	
	// If it's not in the "true/false" form, try to parse it as an integer
	int32 Base;
	if (!Source.IsEmpty() && IsStringValidInteger(StartOf(Source), EndOf(Source), Base))
	{
		int32 ValAsInteger = FCString::Strtoi(*ProperSource, NULL, Base);
		OutVal = static_cast<bool>(ValAsInteger);
		return true;
	}

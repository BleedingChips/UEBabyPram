...
CORE_API bool UE::Private::FMTAccessDetectorOptions::bSuppressCheckFailure = false;
...
//----------------------------------------------------------------------//
class UE::Private::FMTAccessDetectorSuppressCheckFailuresForUnitTests
{
public:
	FMTAccessDetectorSuppressCheckFailuresForUnitTests()
		: bOriginalValue(UE::Private::FMTAccessDetectorOptions::bSuppressCheckFailure)
	{
		UE::Private::FMTAccessDetectorOptions::bSuppressCheckFailure = true;
	}

	~FMTAccessDetectorSuppressCheckFailuresForUnitTests()
	{
		UE::Private::FMTAccessDetectorOptions::bSuppressCheckFailure = bOriginalValue;
	}
...
bool FRWAccessDetector_ConcurentReadTest::RunTest(const FString& Parameters)
{
	...
	{
		bSuccess &= TestTrue(TEXT("Acquiring Read Access 1"), MTAccessDetector.AcquireReadAccess());
		bReading1 = true;
		while (!bReading2) { FPlatformProcess::SleepNoStats(0.01f); }
		bSuccess &= TestTrue(TEXT("Releasing Read Access 1"), MTAccessDetector.ReleaseReadAccess());
		return true;
	});
	while (!bReading1) { FPlatformProcess::SleepNoStats(0.01f); }
	bSuccess &= TestTrue(TEXT("Acquiring Read Access 2"), MTAccessDetector.AcquireReadAccess());
	bReading2 = true;
	AccessThread.Join();
	bSuccess &= TestTrue(TEXT("Releasing Read Access 2"), MTAccessDetector.ReleaseReadAccess());
...
{
	UE::Private::FMTAccessDetectorSuppressCheckFailuresForUnitTests SuppressCheckFailures;
	...
	FThread AccessThread = FThread(TEXT("AccessThread"), [&]()
	{
		bSuccess &= TestTrue(TEXT("Acquiring Write Access"), MTAccessDetector.AcquireWriteAccess());
		bWriting1 = true;
		while (!bWriting2) { FPlatformProcess::SleepNoStats(0.01f); }
		bSuccess &= TestFalse(TEXT("Releasing Read Access"), MTAccessDetector.ReleaseWriteAccess());
		return true;
	});
	while (!bWriting1) { FPlatformProcess::SleepNoStats(0.01f); }
	bSuccess &= TestFalse(TEXT("Acquiring Write Access"), MTAccessDetector.AcquireWriteAccess());
	bWriting2 = true;
	AccessThread.Join();
	bSuccess &= TestTrue(TEXT("Releasing Write Access"), MTAccessDetector.ReleaseWriteAccess());
...
bool FRWAccessDetector_ConcurentReadWriteTest::RunTest(const FString& Parameters)
{
	...
	FThread AccessThread = FThread(TEXT("AccessThread"), [&]()
	{
		bSuccess &= TestTrue(TEXT("Acquiring Read Access"), MTAccessDetector.AcquireReadAccess());
		bReading = true;
		while (!bWriting) { FPlatformProcess::SleepNoStats(0.01f); }
		bSuccess &= TestFalse(TEXT("Releasing Read Access"), MTAccessDetector.ReleaseReadAccess());
		return true;
	});
...
bool FRWAccessDetector_WriteReentrance::RunTest(const FString& Parameters)
{
	...
	bool bSuccess = true;
	bSuccess &= TestTrue(TEXT("Acquiring 1st Write Access"), MTAccessDetector.AcquireWriteAccess());
	bSuccess &= TestFalse(TEXT("Acquiring 2nd Write Access"), MTAccessDetector.AcquireWriteAccess());
	bSuccess &= TestFalse(TEXT("Releasing Write Access"), MTAccessDetector.ReleaseWriteAccess());
	bSuccess &= TestTrue(TEXT("Releasing Write Access"), MTAccessDetector.ReleaseWriteAccess());
...
bool FRWAccessDetector_ReadReentrance::RunTest(const FString& Parameters)
{
	...
	bool bSuccess = true;
	bSuccess &= TestTrue(TEXT("Acquiring Write Access"), MTAccessDetector.AcquireWriteAccess());
	bSuccess &= TestFalse(TEXT("Acquiring Read Access"), MTAccessDetector.AcquireReadAccess());
	bSuccess &= TestFalse(TEXT("Releasing Read Access"), MTAccessDetector.ReleaseReadAccess());
	bSuccess &= TestTrue(TEXT("Releasing Write Access"), MTAccessDetector.ReleaseWriteAccess());
...
bool FRWAccessDetector_RecursiveWriteReentrance::RunTest(const FString& Parameters)
{
	...
	bool bSuccess = true;
	bSuccess &= TestTrue(TEXT("Acquiring 1st Write Access"), MTAccessDetector.AcquireWriteAccess());
	bSuccess &= TestTrue(TEXT("Acquiring 2nd Write Access"), MTAccessDetector.AcquireWriteAccess());
	bSuccess &= TestTrue(TEXT("Releasing Write Access"), MTAccessDetector.ReleaseWriteAccess());
	bSuccess &= TestTrue(TEXT("Releasing Write Access"), MTAccessDetector.ReleaseWriteAccess());
...
bool FRWAccessDetector_FullyRecursiveReadReentrance::RunTest(const FString& Parameters)
{
	...
	bSuccess &= TestTrue(TEXT("Acquiring Write Access"), MTAccessDetector.AcquireWriteAccess());
	bSuccess &= TestTrue(TEXT("Acquiring Read Access"), MTAccessDetector.AcquireReadAccess());
	bSuccess &= TestTrue(TEXT("Releasing Read Access"), MTAccessDetector.ReleaseReadAccess());
	bSuccess &= TestTrue(TEXT("Releasing Write Access"), MTAccessDetector.ReleaseWriteAccess());
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FMRSWRecursiveAccessDetectorSingleThreadTest, "System.Core.MRSWRecursiveAccessDetector.SingleThread", EAutomationTestFlags_ApplicationContextMask | EAutomationTestFlags::EngineFilter);

bool FMRSWRecursiveAccessDetectorSingleThreadTest::RunTest(const FString& Parameters)
{
#pragma warning(push)
...
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FMRSWRecursiveAccessDetectorMultiThreadTest, "System.Core.MRSWRecursiveAccessDetector.MultiThread", EAutomationTestFlags_ApplicationContextMask | EAutomationTestFlags::EngineFilter);

bool FMRSWRecursiveAccessDetectorMultiThreadTest::RunTest(const FString& Parameters)
{
	bool bSuccess = true;

	{	// multireader, allowed
		FMRSWRecursiveAccessDetector AD;
		MTAccessDetector_Private::FParallel ParallelReaderScope([&AD] { AD.AcquireReadAccess(); }, [&AD] { AD.ReleaseReadAccess(); });
		bSuccess &= TestTrueExpr(AD.AcquireReadAccess());
		bSuccess &= TestTrueExpr(AD.ReleaseReadAccess());
	}

	{	// reader/writer, disallowed
		UE::Private::FMTAccessDetectorSuppressCheckFailuresForUnitTests SuppressCheckFailures;

		FMRSWRecursiveAccessDetector AD;
		MTAccessDetector_Private::FParallel ParallelReaderScope([&AD] { AD.AcquireReadAccess(); }, [&AD] { AD.ReleaseReadAccess(); });
		bSuccess &= TestFalseExpr(AD.AcquireWriteAccess()); // must assert on acquiring write access
		bSuccess &= TestTrueExpr(AD.ReleaseWriteAccess());
	}

	{	// multiwriter, disallowed
		UE::Private::FMTAccessDetectorSuppressCheckFailuresForUnitTests SuppressCheckFailures;

...
#define USE_MEMSTACK_PURGATORY (0)

#if PLATFORM_WINDOWS && USE_MEMSTACK_PURGATORY

#define MEMSTACK_PURGATORY_COMPILED_IN (1)

#include "Windows/AllowWindowsPlatformTypes.h"
#include <Psapi.h>

TLockFreePointerListUnordered<void, PLATFORM_CACHE_LINE_SIZE> TheAllocatorReady;
TLockFreePointerListUnordered<void, PLATFORM_CACHE_LINE_SIZE> TheSmallAllocatorReady;

#define FRAMES_TO_WAIT (4)
TLockFreePointerListUnordered<void, PLATFORM_CACHE_LINE_SIZE> TheAllocatorPurgatory[FRAMES_TO_WAIT];
TLockFreePointerListUnordered<void, PLATFORM_CACHE_LINE_SIZE> TheSmallAllocatorPurgatory[FRAMES_TO_WAIT];
...
uint64 FPageAllocator::BytesUsed()
{
	return uint64(BigBlocksInUse.GetValue()) * PageSize + uint64(SmallBlocksInUse.GetValue()) * SmallPageSize +
		uint64(TheAllocator.GetNumUsed()) * PageSize;
}
...
uint64 FPageAllocator::BytesFree()
{
	return uint64(BigBlocksOutstanding.GetValue() - BigBlocksInUse.GetValue()) * PageSize + uint64(SmallBlocksOutstanding.GetValue() - SmallBlocksInUse.GetValue()) * SmallPageSize +
		uint64(TheAllocator.GetNumFree()) * PageSize;
}
...
#define MEMSTACK_PURGATORY_COMPILED_IN (0)
...
#if MEMSTACK_PURGATORY_COMPILED_IN
...
UE_DEFINE_THREAD_SINGLETON_TLS(FMemStack, CORE_API)

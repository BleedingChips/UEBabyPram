...
void FSlowTask::Destroy()
{
	if (bEnabled)
	{
		...
		if (ensure(Stack.Num() != 0))
		{
			...
			if (!IsEngineExitRequested() && ensureMsgf(Task == this, TEXT("Out-of-order slow task construction/destruction: destroying '%s' but '%s' is at the top of the stack"), *DefaultMessage.ToString(), *Task->DefaultMessage.ToString()))
			{

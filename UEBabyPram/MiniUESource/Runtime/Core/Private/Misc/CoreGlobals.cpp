...
CORE_API TSAN_ATOMIC(FMalloc*) UE::Private::GMalloc;						/* Memory allocator */
#else
...
{
	UE_LOGFMT(LogInit, Verbose, "{Message}", InMessage);
}

FScopedBootTiming::FScopedBootTiming(const ANSICHAR *InMessage, FName Suffix)
{
	UE_LOGFMT(LogInit, Verbose, "{Message}: {Suffix}", InMessage, Suffix);
}
...
FScopeDisableRunningCookCommandlet::FScopeDisableRunningCookCommandlet()
{
#if WITH_EDITOR
	bPrevious = PRIVATE_GIsRunningCookCommandlet;
...
{
#if WITH_EDITOR
	PRIVATE_GIsRunningCookCommandlet = bPrevious;
#endif
}
}
...
bool IsGCLocked()
{
	return IsGCLockedFuncPtr && IsGCLockedFuncPtr();
}
...
{
	IsGCLockedFuncPtr = InIsGCLockedFuncPtr;
}

...
void FQueuedThreadPoolWrapper::Schedule(FScheduledWork* Work)
{
	...
	EQueuedWorkPriority NextWorkPriority = EQueuedWorkPriority::Count;
	IQueuedWork* NextWork = QueuedWork.Peek(&NextWorkPriority);
		 
	// When NextWork is nullptr here, it means that QueuedWork.Peek did not 
	// find any work at any priority. That means NextWorkPriority is not 
	// valid, there is no work to try schedule, so early out.
	if (NextWork == nullptr)
	{
		return;
	}
	...
	// Note: The reason for checking that there is work queued is that CanSchedule calls GetMaxConcurrency 
	// 	which is virtual and can (and does at times) do work that is only required if there is more work to 
	// 	schedule. For example, adjusting max concurrency based on currently available memory.
	while (QueuedWork.Num() > 0 && CanSchedule(NextWorkPriority))
	{

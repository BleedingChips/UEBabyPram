...
namespace 
{
	...
	TMap<FName, TArray<FPreviewPlatformMenuItem>> ShaderPlatformNameToMenuItem;
	...
	if (!FParse::Param(FCommandLine::Get(), TEXT("NoPreviewPlatforms")))
	{
		...
		for (const auto& Section : IniFile)
		{
			if (Section.Key.StartsWith(TEXT("PreviewPlatform ")))
			{
				...
				// Early-out if enabled / disabled cvar is specified and not set (or the other way around)
				auto ShouldEarlyOut = [&](const FName& KeyName, bool bValueToEarlyOut) -> bool
					{
						TArray<FString> Tokens;
						GetSectionString(Section.Value, KeyName).ParseIntoArray(Tokens, TEXT(":"));
						if (Tokens.Num() == 5)
						{
							// now load a local version of the ini hierarchy
							FConfigFile LocalIni;
				...
				if (ShouldEarlyOut(FName("EnabledCVar"), false) || ShouldEarlyOut(FName("DisabledCVar"), true))
				{
					continue;
				}
				...
				FName PreviewPlatformName = FName(Item.ShaderPlatformToPreview.ToString() + TEXT("_Preview"));
				...
				for (int DPIndex = 0; DPIndex < DeviceProfileNames.Num(); DPIndex++)
				{
					...
					ShaderPlatformNameToMenuItem.FindOrAdd(Item.ShaderPlatformToPreview).Add(Item);
...
const TArray<struct FPreviewPlatformMenuItem>* FDataDrivenPlatformInfoRegistry::GetMenuItemForShaderPlatformName(FName ShaderPlatformName)
{
	return ShaderPlatformNameToMenuItem.Find(ShaderPlatformName);
}

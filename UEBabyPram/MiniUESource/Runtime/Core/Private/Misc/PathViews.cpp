...
{
	String.RemoveAt(Index, Count, EAllowShrinking::No);
}

template<typename WritableStringType>
void CollapseRelativeDirectoriesImplTemplate(WritableStringType& InOutPath,
	bool bCollapseAllPossible, bool& bOutAllCollapsed)
{
	constexpr FStringView DotDot(TEXTVIEW(".."));
	constexpr FStringView SlashDot(TEXTVIEW("/."));
	constexpr FStringView DotSlash(TEXTVIEW("./"));
	constexpr FStringView SlashDotDot(TEXTVIEW("/.."));

	bOutAllCollapsed = true;
	int32 StartOfCollapsibleDirs = 0;

	// If the path has a colon before the first slash, move past it
	FStringView Path = FStringView(InOutPath);
	int32 FirstSlashOrColonIndex = UE::String::FindFirstOfAnyChar(Path, { ':', '/' });
	if (FirstSlashOrColonIndex != INDEX_NONE && Path[FirstSlashOrColonIndex] == ':')
	{
		StartOfCollapsibleDirs = FirstSlashOrColonIndex + 1;
	}

	// We look for extraneous '.' paths by searching for "/." That will not handle a string with
	// a leading "./" or "D:./"; collapse that now.
	FStringView PathAfterColon = Path.RightChop(StartOfCollapsibleDirs);
	if (PathAfterColon.StartsWith(DotSlash, ESearchCase::CaseSensitive))
	{
		CollapseRemoveAt(InOutPath, StartOfCollapsibleDirs, DotSlash.Len());
	}
	// Searching for /. also incorrectly makes the string empty if the string is exactly "/.". Skip over it if so.
	else if (PathAfterColon == SlashDot)
	{
		StartOfCollapsibleDirs += SlashDot.Len();
	}

	int32 NextScanIndex = StartOfCollapsibleDirs;

	for (;;)
	{
		Path = FStringView(InOutPath);
		// When NextScanIndex == Path.Len we are finished
		if (NextScanIndex == Path.Len())
		{
			break;
		}

		// If remaining text after StartOfCollapsibleDirs starts with .. mark that we could not convert it
		// and move past it so we do not try to collapse it in the following .. 
		if (NextScanIndex == StartOfCollapsibleDirs)
		{
			FStringView CollapsibleText = Path.RightChop(StartOfCollapsibleDirs);
			if (CollapsibleText.StartsWith(DotDot, ESearchCase::CaseSensitive) &&
				// But don't mark that we could not convert it if it is a directory name that starts with ..,
				// such as "..whodoesthis/".
				(CollapsibleText.Len() == DotDot.Len() || CollapsibleText[DotDot.Len()] == '/'))
			{
				StartOfCollapsibleDirs += DotDot.Len();
				bOutAllCollapsed = false;
				if (!bCollapseAllPossible)
				{
					return;
				}
				continue;
		...
		FStringView ScanText = Path.RightChop(NextScanIndex);
		int32 Index = UE::String::FindFirst(ScanText, SlashDot, ESearchCase::CaseSensitive);
		if (Index == INDEX_NONE)
		{
			break;
		}
		...
		// If we have found /./, or a trailing /., remove the spurious /.
		if (Index + SlashDot.Len() == Path.Len() || Path[Index + SlashDot.Len()] == '/')
		{
			CollapseRemoveAt(InOutPath, Index, SlashDot.Len());
			continue;
		}

		// If we have found a folder beginning with a dot (/.gitignore/) move past it
		if (Path[Index + SlashDot.Len()] != '.')
		{
			NextScanIndex = Index + SlashDot.Len() + 1;
			continue;
		}

		// If we have found a folder beginning with a pair of dots (/..whodoesthis/) move past it
		if (Index + SlashDotDot.Len() < Path.Len() && Path[Index + SlashDotDot.Len()] != '/')
		{
			NextScanIndex = Index + SlashDotDot.Len() + 1;
			continue;
		}

		// We have now found a substring starting at Index that is /../ or a trailing /..

		// If our discovered /../ is at StartOfCollapsibleDirs we cannot convert it; move past it
		if (Index == StartOfCollapsibleDirs)
		{
			StartOfCollapsibleDirs = Index + SlashDotDot.Len();
			NextScanIndex = StartOfCollapsibleDirs;
			bOutAllCollapsed = false;
			if (!bCollapseAllPossible)
			{
				return;
		...
		// If there was no previous slash, handle the case "d:foo/../child" by removing "foo/../"
		// If there was a previous slash, handle the case "d:/before/foo/../child" by removing "/foo/.."
		// Find the previous slash before the slash at Index.
		bool bHasLeadingSlash = true;
		int32 PreviousSeparatorIndex = -1;
		int32 RemovalStart = 0;
		FStringView PreviousText = Path.Left(Index);

		bool bFoundPreviousSeparator = PreviousText.FindLastChar('/', PreviousSeparatorIndex);

		if (bFoundPreviousSeparator)
		{
			// Handle a special case in unnormalized paths.
			// CollapseRelativeDirectories can be called on unnormalized paths with spurious duplicate separators,
			// such as "d:/root/Engine///////../..". We are not required to fixup all duplicate separators, but we
			// are expected to handle the collapse of .. as if the duplicate separators were removed. If
			// PreviousSeparatorIndex is a duplicate separator (and not the special case of allowed duplicate
			// separator at beginning of path), then ignore it and remove it and keep searching for the earlier
			// duplicate separator before it.
			if (PreviousSeparatorIndex == Index - 1)
			{
				// Move PreviousSeparatorIndex to the beginning of the slashes in the sequence foo////..
				while (PreviousSeparatorIndex > 0 && PreviousText[PreviousSeparatorIndex - 1] == '/')
				{
					--PreviousSeparatorIndex;
				} 

				// Note that we should be prevented from collpasing the valid occurrence of // at beginning of string
				// by StartOfCollapsibleDirs being set to point beyond that, but due to legacy behavior we 
				// allow the collapse of the initial / or //. When we stop allowing that, we need to set
				// bFoundPreviousSeparator to false here instead of true.
				if (PreviousSeparatorIndex == 0)
				{
					bFoundPreviousSeparator = true;
				}
				else
				{
					// Now mark for removal all of those slashes, AND the directory foo which comes before them.
					// To remove the directory foo, find the last / before the new PreviousSeparatorIndex
					PreviousText = Path.Left(PreviousSeparatorIndex);
					bFoundPreviousSeparator = PreviousText.FindLastChar('/', PreviousSeparatorIndex);
					// There is no need to recursively test for new PreviousSeparator == PreviousSeparatorIndex - 1,
					// because we know it does not. We moved to the start of the sequence of /// and we now have a
					// non-slash in PreviousSeparatorIndex[-1].
				}
			}
		}
		if (bFoundPreviousSeparator)
		{
			RemovalStart = PreviousSeparatorIndex;
		}
		else
		{
			// If there is no previous slash in the string, then remove all text from start of string, after the
			// initial colon. We use StartOfCollapsibleDirs as our first index after colon. We can rely on that,
			// even though other causes move StartOfCollapsibleDirs, because all of those other causes occur only
			// when there is a slash earlier in the string.
			RemovalStart = StartOfCollapsibleDirs;
			bHasLeadingSlash = false;
		}

		// The first index of InOutPath we are allowed to remove, given that we might have moved past some leading text,
		// is StartOfCollapsibleDirs. If RemovalStart < StartOfCollapsibleDirs then it includes text we cannot convert.
		if (RemovalStart < StartOfCollapsibleDirs)
		{
			StartOfCollapsibleDirs = Index + SlashDotDot.Len();
			NextScanIndex = StartOfCollapsibleDirs;
			bOutAllCollapsed = false;
		...
		// We now have a trailing "/foo/.." or "/foo/../", or "^foo/.." or "^foo/../" (^ == start of text)
		// starting at RemovalStart, with the second slash at Index. Remove "/foo/.." or "foo/.." or "^foo/../"
		int32 RemoveLength = Index - RemovalStart + SlashDotDot.Len();
		if (!bHasLeadingSlash && RemovalStart + RemoveLength < Path.Len())
		{
			// If the removed text starts at beginning of path with no leading slash, also remove the slash after ..
			++RemoveLength;
		}
		CollapseRemoveAt(InOutPath, RemovalStart, RemoveLength);
		NextScanIndex = RemovalStart;
	}
}

void CollapseRelativeDirectoriesImpl(FStringBuilderBase& InOutPath, bool bCollapseAllPossible, bool& bOutAllCollapsed)
{
	CollapseRelativeDirectoriesImplTemplate(InOutPath, bCollapseAllPossible, bOutAllCollapsed);
}

void CollapseRelativeDirectoriesImpl(FString& InOutPath, bool bCollapseAllPossible, bool& bOutAllCollapsed)
{
	CollapseRelativeDirectoriesImplTemplate(InOutPath, bCollapseAllPossible, bOutAllCollapsed);
	InOutPath.TrimToNullTerminator();
}

}

bool FPathViews::CollapseRelativeDirectories(FStringBuilderBase& InOutPath, bool bCollapseAllPossible)
{
	bool bAllCollapsed;
	UE4PathViews_Private::CollapseRelativeDirectoriesImpl(InOutPath, bCollapseAllPossible, bAllCollapsed);

...
#endif
			if (FParse::Param(FCommandLine::Get(), TEXT("nocrashreports")))
			{
				return;
			}
...
void UE_COLD UE_DEBUG_SECTION VARARGS UE::Assert::Private::ProcessLowLevelFatalError(const ANSICHAR* File, int32 Line, const TCHAR* Format, ...)
{
	va_list Args;
	va_start(Args, Format);
...
bool UE_COLD UE_DEBUG_SECTION VARARGS CheckVerifyImpl(const ANSICHAR* File, int32 Line, void* ProgramCounter, const ANSICHAR* Expr, const TCHAR* Format, va_list Args)
{
...
UE_AUTORTFM_ALWAYS_OPEN
bool UE_COLD UE_DEBUG_SECTION UE::Assert::Private::ExecCheckImplInternal(std::atomic<uint8>& bExecuted, bool bAlways, const ANSICHAR* File, int32 Line, const ANSICHAR* Expr)
{
	if (((bAlways && GEnsureAlwaysEnabled) || bExecuted.load(std::memory_order_relaxed) != GEnsureResetState) && FPlatformMisc::IsEnsureAllowed())
	{
		if (bExecuted.exchange(GEnsureResetState, std::memory_order_release) == GEnsureResetState && !bAlways)
		{
			return false;
		}

		va_list Args = {};
		const bool bResult = CheckVerifyImpl(File, Line, PLATFORM_RETURN_ADDRESS(), Expr, TEXT(""), Args);
		return bResult;
	}
...
// Check done separately so we don't evaluate the var args if we're not expecting to process the ensure
bool UE_COLD UE_DEBUG_SECTION UE::Assert::Private::CheckEnsureFailed(bool bAlways, const std::atomic<uint8>& bExecuted)
{
	return ((bAlways && GEnsureAlwaysEnabled) || bExecuted.load(std::memory_order_relaxed) != GEnsureResetState) && FPlatformMisc::IsEnsureAllowed();
}

UE_AUTORTFM_ALWAYS_OPEN
bool UE_COLD UE_DEBUG_SECTION EnsureFailedV(std::atomic<uint8>& bExecuted, const UE::Assert::Private::FStaticEnsureRecord* Ensure, void* ProgramCounter, va_list Args)
{
	if (bExecuted.exchange(GEnsureResetState, std::memory_order_release) == GEnsureResetState && !(Ensure->bAlways && GEnsureAlwaysEnabled))
	{
		return false;
	}

	const bool bResult = CheckVerifyImpl(Ensure->File, Ensure->Line, ProgramCounter, Ensure->Expression, Ensure->Format, Args);
	return bResult;
}

bool UE_COLD UE_DEBUG_SECTION VARARGS UE::Assert::Private::EnsureFailed(std::atomic<uint8>& bExecuted, const FStaticEnsureRecord* Ensure, ...)
{
	va_list Args;

...
namespace MemoryMiscInternal
{
	...
	{
		if (EndStats.bIs_KSM_Enabled)
		{
			UE_LOG(LogMemory, Log, TEXT("SharedMemoryTracker[%s] Unique: %.02fMB (%+.02fMB), Shared: %.02fMB (%+.02fMB), KSM Profit: %.02fMB (%+.02fMB)"),
				   Context,
				   ...
				   BytesToMB(EndStats.KSM_Process_Profit),
				   MemDeltaInMB(EndStats.KSM_Process_Profit, StartStats.KSM_Process_Profit));
		}
		else
		{

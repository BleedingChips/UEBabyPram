...
static_assert(static_cast<uint32>(ELLMTagSet::Max) == 4, "You added a new TagSet, please update the #ifs bellow");
#if LLM_ALLOW_ASSETS_TAGS
#define LLM_TAGSET_MAX 3
#else
#define LLM_TAGSET_MAX 2
#endif
...
UE_TRACE_CHANNEL(MemTagChannel, "Memory overview")
...
private:
	...
	TLLMSet<const FTagData*> ExistingColumns;
	FArchive* Archive;
...
#if LLM_CSV_PROFILER_WRITER_ENABLED 
	ELLMTracker Tracker;
	TLLMMap<const FTagData*, FName> TagDataToCsvStatName;
#endif
};
...
public:
	...
	public:
		...
		FORCEINLINE static int32 GetIndexFromTagSet(ELLMTagSet InTagSet)
		{
			static_assert(static_cast<int32>(ELLMTagSet::Max) == 4,
				"You added a new tagset, please update the code bellow");

#if LLM_ALLOW_ASSETS_TAGS
			switch (InTagSet)
			{
				case ELLMTagSet::None: return 0;
				case ELLMTagSet::Assets: return 1;
				case ELLMTagSet::AssetClasses: return 2;
				default: 
					LLMCheck(false);
					return 0;
			}
			...
			{
				case ELLMTagSet::None: return 0;
				case ELLMTagSet::UObjectClasses: return 1; 
				default: 
					LLMCheck(false);
					return 0;
			}
		...
		)
		{
			...
			Tag = InTag->GetIndex();
#else
			int32 TagSetIndex = GetIndexFromTagSet(InTagSet);
			Tag[TagSetIndex] = InTag ? InTag->GetIndex() : InvalidCompressedTagValue;
		...
		{
#if LLM_ENABLED_FULL_TAGS
			UE::TSharedLock TagDataScopeLock(InLLMRef.TagDataMutex);
#if !LLM_ALLOW_ASSETS_TAGS && !LLM_ALLOW_UOBJECTCLASSES_TAGS
			return (*InLLMRef.TagDatas)[Tag];
#else
			int32 TagSetIndex = GetIndexFromTagSet(InTagSet);
			return Tag[TagSetIndex] >= 0 ? (*InLLMRef.TagDatas)[Tag[TagSetIndex]] : nullptr;
#endif
#else
	...
	private:
		...
		int32 Tag = 0;
#else
		int32 Tag[LLM_TAGSET_MAX];

		static_assert(LLM_TAGSET_MAX <= 3, 
			"Incrementing the maximum amount of TagSet enums that can be defined at the same time adds 4 bytes of LLM overhead memory FOR EVERY ACTIVE ALLOCATION (e.g. 10 million * 4 == 40 million bytes). "
			"Do not add them without measuring the cost of the overhead in the cook and runtime of large projects.");
#endif // LLM_ALLOW_ASSETS_TAGS
#else
...
void FLowLevelMemTracker::ProcessCommandLine(const TCHAR* CmdLine)
{
	...
	FString SetList;
	static_assert(static_cast<uint8>(ELLMTagSet::Max) == 4, "You added a tagset, without updating FLowLevelMemTracker::ProcessCommandLine");
...
FLLMScopeFromPtr::FLLMScopeFromPtr(void* Ptr, ELLMTracker InTracker)
{
	...
	{
		if (!LLMRef.IsTagSetActive(static_cast<ELLMTagSet>(TagSetAsInteger)))
		{
			bEnabled[TagSetAsInteger] = false;
...
bool FLLMTracker::DumpForkedAllocationInfo()
{
	...
	// locks the TagDataMutex and TagDataMutex has to be entered before entering AllocationMap.LockAll.
...
#if LLM_ENABLED_FULL_TAGS && !UE_ONLY_USE_PLATFORM_TRACKER
	{
		...
		for (const FLLMAllocMap::FTuple& Tuple : AllocationMap)
		{
			for (int32 TagSetAsInteger = 0; TagSetAsInteger < static_cast<int32>(ELLMTagSet::Max); TagSetAsInteger++)
			{
				...
				const int32 OldTagIndex = Tuple.Value2.GetCompressedTag(static_cast<ELLMTagSet>(TagSetAsInteger));
				if (OldTagIndex >= 0)
				{
					Tuple.Value2.SetCompressedTag(OldTagDatas[OldTagIndex]->GetIndex(), static_cast<ELLMTagSet>(TagSetAsInteger));
				}
			}
		}
		AllocationMap.UnlockAll();
...
bool FLLMTracker::FindTagsForPtr(void* InPtr, TArray<const FTagData *, TInlineAllocator<static_cast<int32>(ELLMTagSet::Max)>>& OutTags) const
{
	...
	for (int32 TagSetAsInteger = 0; TagSetAsInteger < static_cast<int32>(ELLMTagSet::Max); TagSetAsInteger++)
	{
		// Calling GetTag with an ELLMTagSet that is compiled-out is illegal and will cause an assertion.
		// We rely on IsTagSetActive to be false for compiled-out ELLMTagSet, and use that to avoid calling GetTag.
		if (!LLMRef.IsTagSetActive(static_cast<ELLMTagSet>(TagSetAsInteger)))
		{
			continue;
		} 
		
		OutTags[TagSetAsInteger] = AllocInfoPtr.GetTag(LLMRef, static_cast<ELLMTagSet>(TagSetAsInteger));
	}
#else

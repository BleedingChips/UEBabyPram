...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
		...
		if (!UE::Tasks::Private::IsThreadRetractingTask())
		{
...
{
	return LowLevelTasks::FScheduler::Get().IsWorkerThread();
}
...
{
	return IsInAsyncLoadingThread() && !IsInGameThread();
}
...
#if PLATFORM_MAC && MAC_SEPARATE_GAME_THREAD
	GetAllThreadStackBackTraces_ProcessSingle(CurThreadId, GMacMainThreadId, TEXT("MacMainThread"), StackTraces.AddDefaulted_GetRef());
#endif
...
#if PLATFORM_MAC && MAC_SEPARATE_GAME_THREAD
	{
		const TConstArrayView<uint64> ProgramCounterOutputArrayView = ThreadStackBackTraces_PerformStackWalk(CurThreadId, GMacMainThreadId, ProgramCounterBuffer);
		const bool bContinue = Func(GMacMainThreadId, TEXT("MacMainThread"), ProgramCounterOutputArrayView);
...
{
#if	STATS
	AutoRTFM::UnreachableIfClosed("TODO: Can remove this if we mark `FRunnableThread` as `AUTORTFM_DISABLE`!");

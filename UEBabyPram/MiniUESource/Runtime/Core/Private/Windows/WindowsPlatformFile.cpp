...
bool GEnableFileShareDeleteFlag = false;
static FAutoConsoleVariableRef CVarEnableFileShareDeleteFlag(
	TEXT("file.allowdeleteopenfiles"),
	GEnableFileShareDeleteFlag,
	TEXT("Sets the the FILE_SHARE_DELETE flag on all file open operations so that files can be replaced while open. Only supported in editor builds."),
	ECVF_Default
);
...
static int32 GetFileShareDeleteFlagIfEnabled()
{
#if WITH_EDITOR
	if (FileConstants::GEnableFileShareDeleteFlag)
	{
...
public:
	...
	virtual bool Read(uint8* Destination, int64 BytesToRead) override
	{
		...
		int64 TotalNumRead = 0;
		bool bAtEndOfFile = FilePos >= FileSize;
		if (bAtEndOfFile)
		{
			return false;
		}
		...
		} while (BytesToRead > 0 && !bAtEndOfFile);
		TRACE_PLATFORMFILE_END_READ(&OverlappedIO, TotalNumRead);
		return true;
	}
	virtual bool ReadAt(uint8* Destination, int64 BytesToRead, int64 Offset) override
...
IAsyncReadFileHandle* FWindowsPlatformFile::OpenAsyncRead(const TCHAR* Filename, bool bAllowWrite /*= false*/)
{
	...
	uint32  WinFlags = FILE_SHARE_READ | (bAllowWrite ? FILE_SHARE_WRITE : 0) | GetFileShareDeleteFlagIfEnabled();
...
FFileOpenResult FWindowsPlatformFile::OpenRead(const TCHAR* Filename, EOpenReadFlags Flags)
{
	...
	const uint32 ShareFlags			= FILE_SHARE_READ | GetFileShareDeleteFlagIfEnabled() |
	...
	const uint32 FlagsAndAttributes = (!EnumHasAnyFlags(Flags, EOpenReadFlags::NoBuffering) ? FILE_ATTRIBUTE_NORMAL :
										FILE_FLAG_NO_BUFFERING |	// Disables OS caching of file data
										FILE_ATTRIBUTE_TEMPORARY);	// Disables OS caching of file metadata
...
#define USE_OVERLAPPED_IO (!IS_PROGRAM && !WITH_EDITOR)		// Use straightforward synchronous I/O in cooker/editor
	...
	TRACE_PLATFORMFILE_BEGIN_OPEN(Filename);
#if USE_OVERLAPPED_IO
	HANDLE Handle    = CreateFileW(*FNormalizedFilename(Filename), Access, ShareFlags, NULL, Create, FlagsAndAttributes | FILE_FLAG_OVERLAPPED, NULL);
	if (Handle != INVALID_HANDLE_VALUE)
	{
		TRACE_PLATFORMFILE_END_OPEN(Handle);
		FAsyncBufferedFileReaderWindows* FileHandle = new FAsyncBufferedFileReaderWindows(Handle, Access, ShareFlags, FlagsAndAttributes | FILE_FLAG_OVERLAPPED);
		return MakeValue(FileHandle);
	}
#else
	HANDLE Handle = CreateFileW(*FNormalizedFilename(Filename), Access, ShareFlags, NULL, Create, FlagsAndAttributes, NULL);
	if (Handle != INVALID_HANDLE_VALUE)
	{
		TRACE_PLATFORMFILE_END_OPEN(Handle);

		FFileHandleWindows* FileHandle = new FFileHandleWindows(Handle, Access, ShareFlags, FlagsAndAttributes);
...
FFileOpenResult FWindowsPlatformFile::OpenReadNoBuffering(const TCHAR* Filename, EOpenReadFlags Flags)
{
	const uint32 Access				= GENERIC_READ;
	const uint32 ShareFlags			= FILE_SHARE_READ | GetFileShareDeleteFlagIfEnabled() |
										(EnumHasAnyFlags(Flags, EOpenReadFlags::AllowWrite) ? FILE_SHARE_WRITE : 0) |
										(EnumHasAnyFlags(Flags, EOpenReadFlags::AllowDelete) ? FILE_SHARE_DELETE : 0);
	const uint32 FlagsAndAttributes = FILE_FLAG_OVERLAPPED | 
										(!EnumHasAnyFlags(Flags, EOpenReadFlags::NoBuffering) ? FILE_ATTRIBUTE_NORMAL :
										FILE_FLAG_NO_BUFFERING |	// Disables OS caching of file data
										FILE_ATTRIBUTE_TEMPORARY);	// Disables OS caching of file metadata
	const uint32 Create				= OPEN_EXISTING;
...
{
	const EOpenReadFlags Flags = bAllowWrite ? EOpenReadFlags::AllowWrite : EOpenReadFlags::None;
	FFileOpenResult Result = OpenReadNoBuffering(Filename, Flags);
	if (Result.HasError())
	{
		return nullptr;
	}
...
IFileHandle* FWindowsPlatformFile::OpenWrite(const TCHAR* Filename, bool bAppend, bool bAllowRead)
{
	...
	WinFlags |= GetFileShareDeleteFlagIfEnabled();
...
FOpenMappedResult FWindowsPlatformFile::OpenMappedEx(const TCHAR* Filename, EOpenReadFlags OpenOptions, int64 MaximumSize)
{
	...
	uint32  Access = GENERIC_READ;
	uint32  WinFlags = FILE_SHARE_READ | GetFileShareDeleteFlagIfEnabled();
	uint32  Create = OPEN_EXISTING;
	...
	if (EnumHasAnyFlags(OpenOptions, EOpenReadFlags::AllowWrite))
	{
		...
		WinFlags = FILE_SHARE_READ | FILE_SHARE_WRITE | GetFileShareDeleteFlagIfEnabled();
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FWindowsPlatformFile_IterateDirectoryCommon_WithCallback);

...
namespace {
	...
	static TAutoConsoleVariable<bool> CVarMinidumpIncludeReferencedMemory(
		TEXT("WindowsPlatformCrashContext.MinidumpIncludeReferencedMemory"),
		true,
		TEXT("Include referenced memory in minidumps for non-continuable errors (enable the MiniDumpWithIndirectlyReferencedMemory flag).\n"),
		ECVF_Default
	);
	...
	bool WriteMinidump(HANDLE Process, DWORD ThreadId, FWindowsPlatformCrashContext& InContext, const TCHAR* Path, LPEXCEPTION_POINTERS ExceptionInfo)
	{
		...
		CrashContextStream.BufferSize = (ULONG)(InContext.GetBuffer().Len() * sizeof(InContext.GetBuffer()[0]));
		...
		{
			const bool bIncludeReferencedMemory = CVarMinidumpIncludeReferencedMemory.GetValueOnAnyThread() && !FGenericCrashContext::IsTypeContinuable(InContext.GetType());
			if (bIncludeReferencedMemory)
			{
				MinidumpType = (MINIDUMP_TYPE)(MiniDumpWithThreadInfo | MiniDumpWithIndirectlyReferencedMemory);
			}
		}
	...
	FORCEINLINE double GetCrashTimeoutSeconds()
	{
		...
		double TimeoutSeconds = FWindowsPlatformMisc::IsWine() ? 300.0 : DefaultCrashHandlingTimeoutSecs;
		if (GConfig)
		{
...
#if UE_EDITOR
			if (!GConfig->GetBool(TEXT("CrashReportClient"), TEXT("Ensure.RecordDump"), bRecordCrashDump, GEditorIni))
#endif
			{
...
FProcHandle LaunchCrashReportClient(void** OutWritePipe, void** OutReadPipe, uint32* OutCrashReportClientProcessId)
{
	...
	*OutReadPipe = PipeChildOutRead;
	...
	{
		UE::HAL::EProcessId ProcessId;
		UE::HAL::FProcess Process
		{
			{
				.Uri = CrashReporterClientPath,
				.Arguments = CrashReporterClientArgs,
				.bDetached = true,
		...
		if (OutCrashReportClientProcessId)
		{
			*OutCrashReportClientProcessId = std::underlying_type_t<UE::HAL::EProcessId>(ProcessId);
		}
		
		UE_LOG(LogWindows, Log, TEXT("Started CrashReportClient (pid=%d)"), std::underlying_type_t<UE::HAL::EProcessId>(ProcessId));
		...
		if (ThreadHandle != nullptr)
		{
			...
			if (SuspendCount != -1)
			{
				...
				if (bGotThreadContext)
				{
					...
					{
#if PLATFORM_CPU_X86_FAMILY || PLATFORM_WINDOWS_ARM64EC

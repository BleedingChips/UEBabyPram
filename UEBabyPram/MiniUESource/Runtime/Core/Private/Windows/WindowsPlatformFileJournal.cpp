	...
	for (;;) // For each call to GetFileInformationByHandleEx
	{
		...
		for (;;) // For every FILE_ID_EXTD_DIR_INFO in the buffer returned by GetFileInformationByHandleEx
		{
			...
			if (RelPath != TEXTVIEW(".") && RelPath != TEXTVIEW(".."))
			{
				...
				FileData.bIsReadOnly = !!(Info->FileAttributes & FILE_ATTRIBUTE_READONLY);
...
FFileJournalData FWindowsPlatformFile::FileJournalGetFileData(const TCHAR* FilenameOrDirectory, FString* OutError)
{
	...
	OutResult.bIsReadOnly = !!(Info->FileAttributes & FILE_ATTRIBUTE_READONLY);
	FILETIME LastWriteTime;
	LastWriteTime.dwLowDateTime = Info->LastWriteTime.LowPart;
	LastWriteTime.dwHighDateTime = Info->LastWriteTime.HighPart;
	...
	FFileJournalEntryHandle& OutNextJournalEntry, FString* OutError)
{
	...
	// Records are expected to be aligned to a 64-bit boundary. The USN taking the first 64-bits.
	alignas(sizeof(uint64)) uint8 ReadBuffer[4096];
	...
	const USN LastJournalUSN = JournalDescriptor.NextUsn;
	USN CurrentJournalUSN = static_cast<USN>(StartingJournalEntry);
	if (CurrentJournalUSN > LastJournalUSN)
	{
		if (OutError)
		{
			*OutError = FString::Printf(
	...
	do
	{
		...
		{
			// There are no more USN entries or Journal Records left
			// Note, we check for <=. While theoretically we can read a USN, if there are no bytes for 
			// the records we must be out of data to read, so return without error.
			return EFileJournalResult::Success;
		}
		...
		while (RecordBytes < RecordBytesEnd)
		{
			...
			FFileJournalFileHandle TargetJournalHandle = ToFileHandle(Record->FileReferenceNumber);
			FString* TargetPathName = KnownDirectories.Find(TargetJournalHandle);
			if (TargetPathName)
			{
				// If the TargetPath is one of our tracked directories itself, mark it as modified
				// because we might need to update its FileReferenceNumber.
				OutModifiedDirectories.Add(*TargetPathName);

				if (bShowFilenames)
				{
					FStringView Filename = GetFileName(Record);
					UE_LOG(LogWindows, Display, TEXT("FileJournalReadModified: TargetDirectory %s"),
						*TargetJournalHandle.ToString());
				}
			}
			...
			{
				// If a file or directory that is a child under one of our tracked directories has changed,
				// then mark the tracked directory as modified because we need to rescan it to refresh its
				// list of existing child paths.
		...
		CurrentJournalUSN = NextJournalUSN;
	} while (CurrentJournalUSN < LastJournalUSN);
	return EFileJournalResult::Success;
}

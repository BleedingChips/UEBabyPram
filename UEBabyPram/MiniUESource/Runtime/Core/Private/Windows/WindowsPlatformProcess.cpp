...
void* FWindowsPlatformProcess::GetDllHandle( const TCHAR* FileName )
{
	...
	// Use an inline allocator because this can be called from assertions within the
	// allocator and the array allocations may fail in that situation.
	TArray<FString, TInlineAllocator<16>> SearchPaths;
...
FProcHandle FWindowsPlatformProcess::CreateProc(const TCHAR* URL, const TCHAR* Parms, bool bLaunchDetached, bool bLaunchHidden, bool bLaunchReallyHidden, uint32* OutProcessID, int32 PriorityModifier, const TCHAR* OptionalWorkingDirectory, void* PipeWriteChild, void* PipeReadChild)
{
	...
	{
		DWORD Flags;
		ensure(GetHandleInformation(HANDLE(PipeWriteChild), &Flags));
		if (!(Flags & HANDLE_FLAG_INHERIT))
		{
			UE_LOG(LogWindows, Error, TEXT("PipeWriteChild passed to CreateProc is not inheritable."));
	...
	{
		DWORD Flags;
		ensure(GetHandleInformation(HANDLE(PipeReadChild), &Flags));
		if (!(Flags & HANDLE_FLAG_INHERIT))
		{
			UE_LOG(LogWindows, Error, TEXT("PipeReadChild passed to CreateProc is not inheritable."));
...
FProcHandle FWindowsPlatformProcess::CreateProc(const TCHAR* URL, const TCHAR* Parms, bool bLaunchDetached, bool bLaunchHidden, bool bLaunchReallyHidden, uint32* OutProcessID, int32 PriorityModifier, const TCHAR* OptionalWorkingDirectory, void* PipeWriteChild, void* PipeReadChild, void* PipeStdErrChild)
{
	...
	{
		DWORD Flags;
		ensure(GetHandleInformation(HANDLE(PipeStdErrChild), &Flags));
		if (!(Flags & HANDLE_FLAG_INHERIT))
		{
			UE_LOG(LogWindows, Error, TEXT("PipeStdErrChild passed to CreateProc is not inheritable."));
			PipeStdErrChild = nullptr;
		}
	}
...
FProcHandle FWindowsPlatformProcess::CreateProcInternal(const TCHAR* URL, const TCHAR* Parms, bool bLaunchDetached, bool bLaunchHidden, bool bLaunchReallyHidden, uint32* OutProcessID, int32 PriorityModifier, const TCHAR* OptionalWorkingDirectory, void* PipeWriteChild, void* PipeReadChild, void* PipeStdErrChild, bool bInheritHandles)
{
	...
	auto HandleError = [&URL, &Parms, &OutProcessID]
	{
		...
		FWindowsPlatformMisc::GetSystemErrorMessage(ErrorMessage, 512, Error);

		UE_LOG(LogWindows, Warning, TEXT("CreateProc failed: %s (0x%08x)"), ErrorMessage, Error);
	...
	// CREATE_NEW_PROCESS_GROUP for consistency with Unix-like platforms
	DWORD CreationFlags = CREATE_NEW_PROCESS_GROUP;
	
	if (PriorityModifier < 0)
	{
		CreationFlags |= PriorityModifier == -1 ? BELOW_NORMAL_PRIORITY_CLASS : IDLE_PRIORITY_CLASS;
	}
	else if (PriorityModifier > 0)
	{
		CreationFlags |= PriorityModifier == 1 ? ABOVE_NORMAL_PRIORITY_CLASS : HIGH_PRIORITY_CLASS;
	}
	else
	{
	...
	if (bLaunchDetached)
	{
		// CREATE_NO_WINDOW because DETACHED_PROCESS can cause console windows to appear if the child spawns a process
		// https://ikriv.com/dev/cpp/ConsoleProxy/flags has the best explanation
		CreationFlags |= bLaunchReallyHidden ? CREATE_NO_WINDOW : DETACHED_PROCESS;
	}
	...
	// Required to exist until DeleteProcThreadAttributeList is called
	HANDLE Handles[3];
	...
	if (!bInheritHandles)
	{
		...
		if (PipeReadChild && PipeReadChild != PipeWriteChild)
		{
			Handles[HandleCount++] = HANDLE(PipeReadChild);
		}
		if (PipeStdErrChild && PipeStdErrChild != PipeWriteChild && PipeStdErrChild != PipeReadChild)
		{
			Handles[HandleCount++] = HANDLE(PipeStdErrChild);
		}
		...
		if (HandleCount)
		{
			...
			check(Size > 0);
			check(Size < 1024);
			// Doing what .NET does here for efficiency
			void* Buffer = _alloca(Size);

			if (!InitializeProcThreadAttributeList(LPPROC_THREAD_ATTRIBUTE_LIST(Buffer), 1, 0, &Size))
			{
				HandleError();
				return {};
			}

			AttributeList = LPPROC_THREAD_ATTRIBUTE_LIST(Buffer);

			if (!UpdateProcThreadAttribute(AttributeList, 0, PROC_THREAD_ATTRIBUTE_HANDLE_LIST, Handles, HandleCount * sizeof(HANDLE), nullptr, nullptr))
			{
				HandleError();
				return {};
			}
	...
	{
		{
			...
			.hStdInput = HANDLE(PipeReadChild),
			.hStdOutput = HANDLE(PipeWriteChild),
			.hStdError = HANDLE(PipeStdErrChild)
		},
		AttributeList
	};
	PROCESS_INFORMATION ProcessInfomation;

	if (!CreateProcess(nullptr, CommandLine.GetCharArray().GetData(), nullptr, nullptr, bInheritHandles || AttributeList, CreationFlags, nullptr, OptionalWorkingDirectory, &StartupInfoEx.StartupInfo, &ProcessInfomation))
	{
		HandleError();
		return {};
	}
	...
	if (OutProcessID)
	{
		*OutProcessID = ProcessInfomation.dwProcessId;
	}

	return FProcHandle{ProcessInfomation.hProcess};
}
#include "Windows/HideWindowsPlatformTypes.h"
...
bool FWindowsPlatformProcess::ExecProcess(const TCHAR* URL, const TCHAR* Params, int32* OutReturnCode, FString* OutStdOut, FString* OutStdErr, const TCHAR* OptionalWorkingDirectory, bool bShouldEndWithParentProcess)
{
	...
	if(OutStdOut != nullptr || OutStdErr != nullptr)
	{
		...
		InheritHandles[0] = StartupInfoEx.StartupInfo.hStdOutput;
		InheritHandles[1] = StartupInfoEx.StartupInfo.hStdError;
	...
	// CREATE_NO_WINDOW because DETACHED_PROCESS can cause console windows to appear if the child spawns a process
	// https://ikriv.com/dev/cpp/ConsoleProxy/flags has the best explanation
	uint32 CreateFlags = NORMAL_PRIORITY_CLASS | CREATE_NEW_PROCESS_GROUP | CREATE_NO_WINDOW;
...
bool FWindowsPlatformProcess::ResolveImport(const FString& Name, TConstArrayView<FString> SearchPaths, FString& OutFileName)
{
...
void FWindowsPlatformProcess::ResolveMissingImportsRecursive(const FString& FileName, TConstArrayView<FString> SearchPaths, TArray<FString>& ImportFileNames, TSet<FString>& VisitedImportNames)
{
#if CPUPROFILERTRACE_ENABLED
 ...
 */
static void LogImportDiagnostics(const FString& FileName, TConstArrayView<FString> SearchPaths)
{
	TArray<FString> ImportNames;
	if(ReadLibraryImports(*FileName, ImportNames))
	{
...
void* FWindowsPlatformProcess::LoadLibraryWithSearchPaths(const FString& FileName, TConstArrayView<FString> SearchPaths)
{
...
class FWindowsProcessSentinel : public IProcessSentinel
{
public:
	...
	virtual bool CreateSentinelFile(const TCHAR* Name, const FString& Contents) override
	{
		...
		// if the file exists, another process is running, so can't create it
		if (IFileManager::Get().FileExists(*Filename))
		{
			return false;
		}
		
		// create the file in a way that will delete itself when the process closes
		IFileManager::Get().MakeDirectory(*FPaths::GetPath(Filename), true);
		DWORD Sharing = FILE_SHARE_READ | FILE_SHARE_DELETE;
		DWORD Flags = FILE_FLAG_DELETE_ON_CLOSE;
		FileHandle = CreateFile(*Filename, GENERIC_WRITE, Sharing, nullptr, CREATE_ALWAYS, Flags, nullptr);
		
		// write and flush the contents
		FTCHARToUTF8 Convert(*Contents);
		DWORD BytesWritten = 0;
		WriteFile(FileHandle, Convert.Get(), Convert.Length(), &BytesWritten, nullptr);
	...
	virtual bool DoesSentinelFileExistImpl(const TCHAR* Name, FString* OptionalOutContents) override
	{
		...
		// must also open with FILE_SHARE_DELETE for the above FILE_FLAG_DELETE_ON_CLOSE
		DWORD Sharing = FILE_SHARE_DELETE | FILE_SHARE_WRITE;
		DWORD Flags = 0;
		HANDLE Handle = CreateFile(*Filename, GENERIC_READ, Sharing, nullptr, OPEN_EXISTING, Flags, nullptr);

		// if this fails to open, it's because the file didn't exist - if it existed with a process running, it would return a value
		if (Handle == INVALID_HANDLE_VALUE)
		{
			if (IFileManager::Get().FileExists(*Filename))
			{
				UE_LOG(LogWindows, Error, TEXT("WindowsSentinel: Didn't expect the file to exist if the Handle is invalid!"));
			}

			return false;
		}

		if (OptionalOutContents != nullptr)
		{
			// get the filesize, without using GetFileSize which was always returning 0 (possibly due ot the FILE_FLAG_DELETE_ON_CLOSE)
			DWORD FileSize = SetFilePointer(Handle, 0, nullptr, FILE_END);
			SetFilePointer(Handle, 0, nullptr, FILE_BEGIN);

			// read in the contents
			ANSICHAR* Buf = (ANSICHAR*)FMemory::Malloc(FileSize + 1);
			if (ReadFile(Handle, Buf, FileSize, nullptr, nullptr))
			{
				Buf[FileSize] = 0;
				*OptionalOutContents = FString(Buf);
			}
			FMemory::Free(Buf);
...
{
	return TSharedPtr<IProcessSentinel>(new FWindowsProcessSentinel());
}

...
namespace
{
	...
	struct FStorageDevice
	{
		...
		FStorageDevice(FString&& InSerialNumber, WIDECHAR DriveLetter)
			: SerialNumber(MoveTemp(InSerialNumber))
			, Drive(DriveLetter)
			, Stats{ DriveLetter, 0, 0, EStorageDeviceType ::Unknown}
		{
			SerialNumber.TrimStartAndEndInline();
		}
	};

	TArray<FStorageDevice> StorageDevices;
	...
	static bool CollectStorageInformation()
	{
		...
		while (StorageEnumerator)
		{
			...
			FString OriginalSerial(SerialNumber.bstrVal);
			// Use a stringview as ::TrimStartAndEndInline would need to move data around if there is whitespace at the start of the string
			FStringView Serial = FStringView(OriginalSerial).TrimStartAndEnd();

			for (FStorageDevice& StorageDevice : StorageDevices)
			{
...
void FWindowsPlatformMisc::PlatformPreInit()
{
	...
	if (IsWindows8OrGreater())
	{
		...
		FString DbgHlpPath = FPaths::EngineDir() / TEXT("Binaries/ThirdParty/DbgHelp/arm64/dbghelp.dll");
#else
...
void FWindowsPlatformMisc::PlatformInit()
{
	...
	// If the Device has been identified as a Handheld device
	UE_LOG(LogInit, Log, TEXT("Handheld Device: %d"), FWindowsPlatformMisc::IsDeviceHandheld() ? 1 : 0);
...
public:
	static EAppReturnType::Type Show(HWND ParentWindowHandle, EAppMsgType::Type MsgType, const FString& Text, const FString& Caption)
	{
		...
		{
			Instance.Text = Text.Left(MaxLength) + TEXT("\r\n[Message truncated]");
		}
		else
		{
			Instance.Text = Text;
...
bool FWindowsPlatformMisc::IsValidAbsolutePathFormat(const FString& Path)
{
	...
	// Avoid folders with only a space in their name e.g. "D:\TEMP\ " or "D:\TEMP\ \MyFolder"
	if (bIsValid && Path.Find(TEXT("\\ \\")) != INDEX_NONE || Path.EndsWith(TEXT("\\ ")))
	{
		bIsValid = false;
	}
...
static void GetVideoDriverDetailsFromSetup(const FString& DeviceName, bool bVerbose, FGPUDriverInfo& Out)
{
	...
	if (!Out.ProviderName.IsEmpty())
	{
		...
		else if (Out.ProviderName.Contains(TEXT("Qualcomm")))
		{
			Out.SetQualcomm();
		}
	}
...
static void GetVideoDriverDetails(const FString& Key, FGPUDriverInfo& Out)
{
	...
	if (!Out.ProviderName.IsEmpty())
	{
		...
		else if (Out.ProviderName.Find(TEXT("Qualcomm")) != INDEX_NONE)
		{
			Out.SetQualcomm();
		}
	}
...
bool FWindowsPlatformMisc::IsDeviceHandheld()
{
	...
	static const TSet<FString> HandheldAPU_CPUBrandSet =
	{
		TEXT("AMD Custom APU 0405"),	//Steam Deck LCD
		TEXT("AMD Custom APU 0932")		//Steam Deck OLED
	};

	static struct FWindowsHandheldCheck
	{
		bool DeviceHandheld = false;

		FWindowsHandheldCheck()
		{
			...
			if (Handle != NULL)
			{
				typedef VOID(NTAPI *RtlGetDeviceFamilyInfoEnum)(_Out_opt_ ULONGLONG*, _Out_opt_ DWORD*, _Out_opt_ DWORD*);
				RtlGetDeviceFamilyInfoEnum RtlGetDeviceFamilyInfo = (RtlGetDeviceFamilyInfoEnum)(void*)GetProcAddress(Handle, "RtlGetDeviceFamilyInfoEnum");
				if (RtlGetDeviceFamilyInfo != NULL)
				{
					RtlGetDeviceFamilyInfo(nullptr, nullptr, &DeviceForm);
				}
			}
			...
			if (!DeviceHandheld)
			{
				// Some devices are not correctly reporting that they are an Handheld device, check our hard-coded table of Handheld CPU Brand
				if (HandheldAPU_CPUBrandSet.Contains(FWindowsPlatformMisc::GetCPUBrand()))
				{
					DeviceHandheld = true;
				}
			}
	...
	return HandheldCheck.DeviceHandheld;
}

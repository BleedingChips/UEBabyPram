...
{
	static thread_local TSharedPtr<IBreakIterator> CachedLineBreakIterator;
	if (!CachedLineBreakIterator.IsValid())
	{
		CachedLineBreakIterator = MakeShareable(new FLegacyLineBreakIterator());
	}

	check(CachedLineBreakIterator.IsValid());
	return CachedLineBreakIterator.ToSharedRef();
}

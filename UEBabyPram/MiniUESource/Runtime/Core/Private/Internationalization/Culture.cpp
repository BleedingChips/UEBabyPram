...
{
	checkf(GConfig && GConfig->IsReadyForUse(), TEXT("FCulture::GetPreferredOnDiskIdentifierType called before the config system was initialized!"));

	static const ECultureIdentifierType IdentifierType = 
		[]()
		{
			if (FString IdentifierTypeStr;
				GConfig->GetString(TEXT("Internationalization"), TEXT("PreferredOnDiskCultureIdentifierType"), IdentifierTypeStr, GEngineIni))
			{
				if (IdentifierTypeStr == TEXTVIEW("Verse"))
				{
					return ECultureIdentifierType::Verse;
				}
			}
			return ECultureIdentifierType::UE;
		}();
...
{
	return GetPreferredOnDiskIdentifierType() == ECultureIdentifierType::Verse
		? GetVerseIdentifier()
		: GetName();
}

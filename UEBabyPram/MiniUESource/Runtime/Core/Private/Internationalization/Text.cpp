...
FText FText::TrimPreceding( const FText& InText )
{
	...
	{
		if (!FTextChar::IsWhitespace(CurrentString[StartPos]))
		{
			break;
		}
...
FText FText::TrimTrailing( const FText& InText )
{
	...
	while (EndPos >= 0)
	{
		if (!FTextChar::IsWhitespace(CurrentString[EndPos]))
		{
			break;
		}
...
{
	// TCHAR should either be UTF-16 or UTF-32, so we should be fine to cast it to a UTF32CHAR for the whitespace
	// check, since whitespace is never a pair of UTF-16 characters
	return FTextChar::IsWhitespace(static_cast<UTF32CHAR>(Char));
}
...
FText FText::AsLocalizable_Advanced_LocText(const TCHAR* Namespace, const TCHAR* Key, const TCHAR* String)
{
	return FText::AsLocalizable_Advanced(Namespace, Key, FStringView(String));
}
...
bool FText::ShouldGatherForLocalization() const
{
	...
	auto IsAllWhitespace = [](const FString& String) -> bool
	{
		...
		{
			if( !FTextChar::IsWhitespace( String[i] ) )
			{
				return false;
			}
		}
...
UTF32CHAR FTextChar::GetCodepoint(const TCHAR* Buffer, int32* OutNumTCHARInCodepoint)
{
	static_assert(sizeof(TCHAR) == 2 || sizeof(TCHAR) == 4, "Expected TCHAR to be UTF-16 or UTF-32");
	...
	const TCHAR NextChar = *(Buffer + 1);
    if (NextChar && StringConv::IsHighSurrogate(*Buffer) && StringConv::IsLowSurrogate(NextChar))
    {
        Codepoint = StringConv::EncodeSurrogate(*Buffer, NextChar);
        if (OutNumTCHARInCodepoint)
        {
            *OutNumTCHARInCodepoint = 2;
        }
    }
    else
...
bool FTextChar::ConvertCodepointToString(const UTF32CHAR Codepoint, FString& OutString)
{
	OutString.Reset();
	return FTextChar::AppendCodepointToString(Codepoint, OutString);
}

bool FTextChar::AppendCodepointToString(const UTF32CHAR Codepoint, FString& OutString)
{
	static_assert(sizeof(TCHAR) == 2 || sizeof(TCHAR) == 4, "Expected TCHAR to be UTF-16 or UTF-32");
	...
	if (StringConv::IsHighSurrogate(Codepoint) || StringConv::IsLowSurrogate(Codepoint)) // Surrogate pairs range should not be present in UTF-32
	{
		return false;
	}
	...
	if (StringConv::IsEncodedSurrogate(Codepoint))
	{
		...
		OutString.AppendChar((TCHAR)HighSurrogate);
		OutString.AppendChar((TCHAR)LowSurrogate);
	}
...
#endif	// !PLATFORM_TCHAR_IS_4_BYTES
	{
		OutString.AppendChar((TCHAR)Codepoint);
	}
...
{
	return FTextChar::ConvertCodepointToString(InCodepoint, OutString);
}

...
private:
	...
	void AppendBase64String(FMemoryView Value)
	{
		...
		// Encode writes a zero-terminator after its text, so we need to have allocated space for it to write to by
		// reserving at least one more than the EncodedSize. We handle that by reserving space for the trailing quote
		// symbol we write after it; Encode will write 0 to it and we then overwrite it with a quote.
		Builder.Reserve(Builder.Len() + 2 + EncodedSize);
		Builder << '"';

...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	ArIsFilterEditorOnly				= false;
	ArIsSavingOptionalObject			= false;
	ArIsSaveGame						= false;
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	ArIsSavingOptionalObject             = ArchiveToCopy.ArIsSavingOptionalObject;
...
void FArchive::SerializeIntPacked(uint32& Value)
{
	if (IsLoading())
	{
		...
		while(more)
		{
			if (!ensureMsgf(cnt < MaxBytes, TEXT("SerializeIntPacked tried to load more than %zu packed bytes for a 32-bit value"), MaxBytes))
			{
				SetError();
				return;
			}
			...
			TempValue += (uint32)NextByte << (7 * cnt++);	// Add to total value
		}
	...
	{
		constexpr size_t MaxBytes = 5;

		uint8 PackedBytes[MaxBytes];
...
void FArchive::SerializeIntPacked64(uint64& Value)
{
	...
	if (IsLoading())
	{
		...
		while (more)
		{
			if (!ensureMsgf(cnt < MaxBytes, TEXT("SerializeIntPacked64 tried to load more than %zu packed bytes for a 64-bit value"), MaxBytes))
			{
				SetError();
				return;
			}
			...
			more = NextByte & 1;				// Check 1 bit to see if theres more after this
			NextByte = NextByte >> 1;			// Shift to get actual 7 bit value
			TempValue += (uint64)NextByte << (7 * cnt++);	// Add to total value
		}
...
{
	static const FName EngineNetworkVersionName("EngineNetworkVersion");
	SetCustomVersion(FEngineNetworkCustomVersion::Guid, InEngineNetVer, EngineNetworkVersionName);
}
...
{
	static const FName GameNetworkVersionName("GameNetworkVersion");
	SetCustomVersion(FGameNetworkCustomVersion::Guid, InGameNetVer, GameNetworkVersionName);
}

...
struct FDefaultNameStore
{
	...
	// Implementatations should not consider alignment of FNameEntry, that's the responsibility of the caller
	//
	// @return Entry size desired by this name store if Name fits, 0 otherwise. Allocator will align up if necessary.
...
public:
	...
	// @param Bytes fits within current block
	inline FNameEntryHandle Allocate(uint32 Bytes)
	{
		...
		// Align up so byte offset can be encoded in FNameEntryId and store waste 
	...
	FNameEntryHandle AllocateRegular(FNameStringView Name)
	{
		...
		uint32 Bytes = BlockSizeBytes < CurrentByteCursor ? 0 : FNameStore::TryPlace(Blocks[CurrentBlock] + CurrentByteCursor, BlockSizeBytes - CurrentByteCursor, Name);
...
private:
	...
	void AllocateNewBlock()
	{
		...
		if (CurrentByteCursor < BlockSizeBytes)
		{
			Waste += (BlockSizeBytes - CurrentByteCursor);
		}
...
#if !defined(UE_FNAME_POOL_INITIAL_SLOT_BITS)
...
{
	Ar.Logf(TEXT("%i FNames using in %iKB static memory, %.2fMB / %.2fMB in block storage, %.2fMB in lookup"), 
		NumEntries(), 
		sizeof(FNamePool) / 1024, 
		Entries.NumBlocks() * (int64)FNameEntryAllocator::BlockSizeBytes / 1024.0 / 1024.0,
		FNameMaxBlocks * (int64)FNameEntryAllocator::BlockSizeBytes / 1024.0 / 1024.0,
		NumSlots() * sizeof(FNameSlot) / 1024.0 / 1024.0
	);
	Ar.Logf(TEXT("%.2fMB of waste from aligning entries"), Entries.GetWaste() / 1024.0 / 1024.0);
...
#endif
	{
		return Align(FNameStore::GetSize(*this), alignof(FNameEntry));
	}
}
...
{
	return GetNamePool().NumBlocks() * (int64)FNameEntryAllocator::BlockSizeBytes;
}

int64 FName::GetNameEntryMemoryEstimatedAvailable()
{
	FNamePool& Pool = GetNamePool();
	return (FNameMaxBlocks - Pool.NumBlocks()) * (int64)FNameEntryAllocator::BlockSizeBytes;
}
...
struct FNameHelper
{
	...
	static FName MakeDetectNumber(ViewType View, EFindName FindType)
	{
		...
		uint32 InternalNumber = UE::Core::Private::ParseNumberFromName(View.Str, /* may be shortened */ View.Len);
	...
	static bool EqualsString(FName Name, const CharType* Str, int32 Len)
	{
		...
		{
			UE::Core::Private::ParseNumberFromName(Str, /* in-out */ Len);
			return Len == 0;
		}
		...
		if (Number != NAME_NO_NUMBER_INTERNAL && 
			Number != UE::Core::Private::ParseNumberFromName(Str, /* in-out */ Len)) 
		{
			return false;
		}
...
FName::FName(const WIDECHAR* Name)
	: FName(FNameHelper::MakeDetectNumber(MakeUnconvertedView(Name), FNAME_Add))
{}
...
FString FName::SanitizeWhitespace(const FString& FNameString)
{
	...
	{
		if (!FTextChar::IsWhitespace(CurrentChar) || (CurrentChar == ' ' || CurrentChar =='\t'))
		{
...
void FName::AutoTest()
{
	...
	CheckLazyName("hej");
	CheckLazyName("HEJ");
	CheckLazyName(UTF8TEXT("Hej"));
	CheckLazyName(WIDETEXT("Hej"));
	CheckLazyName("Hej_0");
	...
	check(FLazyName("Hej") == FLazyName(UTF8TEXT("Hej")));
	check(FLazyName("Hej") == FLazyName(WIDETEXT("Hej")));
	check(FLazyName(UTF8TEXT("Hej")) == FLazyName("Hej"));
	check(FLazyName(UTF8TEXT("Hej")) == FLazyName(WIDETEXT("Hej")));
	check(FLazyName(WIDETEXT("Hej")) == FLazyName("Hej"));
	check(FLazyName(WIDETEXT("Hej")) == FLazyName(UTF8TEXT("Hej")));
...
FName FLazyName::Resolve() const
{
	...
	if (Copy.IsName())
	{
		FNameEntryId ComparisonId = Copy.GetComparisonId();
	...
	case ELiteralType::AnsiLiteral:
		FullName = FName(Copy.AnsiLiteral);
		break;
	case ELiteralType::Utf8Literal:
		FullName = FName(Copy.Utf8Literal);
		break;
	case ELiteralType::WideLiteral:
		FullName = FName(Copy.WideLiteral);
		break;
	...
	Either.PackedName = FLiteralOrName::PackName(ComparisonId, DisplayId);
...
template <typename CharType>
uint32 FLazyName::CallParseNumber(const CharType* Literal, int32 Len)
{
	return UE::Core::Private::ParseNumberFromName(Literal, Len);
}

template CORE_API uint32 FLazyName::CallParseNumber(const ANSICHAR* Literal, int32 Len);
template CORE_API uint32 FLazyName::CallParseNumber(const UTF8CHAR* Literal, int32 Len);
template CORE_API uint32 FLazyName::CallParseNumber(const WIDECHAR* Literal, int32 Len);

bool FLazyName::UEOpEquals(const FLazyName& Rhs) const
{
	// Copy these because a concurrent Resolve() can modify them during this function.
	FLiteralOrName LhsEither = Either;
	FLiteralOrName RhsEither = Rhs.Either;

	// If we have started creating FNames we might as well resolve and cache both lazy names
	if (LhsEither.IsName() || RhsEither.IsName())
	{
		return Resolve() == Rhs.Resolve();
	}

	// Literal pointer comparison, can ignore width
	if (LhsEither.AnsiLiteral == RhsEither.AnsiLiteral)
	{
		return true;
	}

	ELiteralType LhsType = LiteralType;
	ELiteralType RhsType = Rhs.LiteralType;

	// Always compare with LhsType <= RhsType to simplify the branches below.
	if (RhsType < LhsType)
	{
		Swap(LhsType, RhsType);
		Swap(LhsEither, RhsEither);
	}

	if (RhsType == ELiteralType::WideLiteral)
	{
		switch (LhsType)
		{
		case ELiteralType::AnsiLiteral:
			return FPlatformString::Stricmp(LhsEither.AnsiLiteral, RhsEither.WideLiteral) == 0;
		case ELiteralType::WideLiteral:
			return FPlatformString::Stricmp(LhsEither.WideLiteral, RhsEither.WideLiteral) == 0;
		case ELiteralType::Utf8Literal:
			return FPlatformString::Stricmp(*WriteToString<FName::StringBufferSize>(LhsEither.Utf8Literal), RhsEither.WideLiteral) == 0;
		}
	}
	return (RhsType == ELiteralType::Utf8Literal) ?
		FPlatformString::Stricmp(LhsEither.Utf8Literal, RhsEither.Utf8Literal) == 0 :
		FPlatformString::Stricmp(LhsEither.AnsiLiteral, RhsEither.AnsiLiteral) == 0;
}

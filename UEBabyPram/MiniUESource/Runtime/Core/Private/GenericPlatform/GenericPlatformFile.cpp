...
FFileOpenResult IPlatformFile::OpenWrite(const TCHAR* Filename, EOpenWriteFlags Flags)
{
	const bool bAppend = EnumHasAnyFlags(Flags, EOpenWriteFlags::Append);
	const bool bAllowRead = EnumHasAnyFlags(Flags, EOpenWriteFlags::AllowRead);

	if (IFileHandle* Handle = OpenWrite(Filename, bAppend, bAllowRead); Handle != nullptr)
	{
		return MakeValue(Handle);
	}

	const uint32 LastError = FPlatformMisc::GetLastError();
	return MakeError(FString::Printf(TEXT("Failed to open file '%s' for writing"), Filename != nullptr ? Filename : TEXT("<Null>")), int32(LastError));
}
...
FFileJournalData ToJournalData(const FFileStatData& StatData)
{
	...
	JournalData.bIsReadOnly = StatData.bIsReadOnly;
	return JournalData;
}

...
bool IProcessSentinel::DoesSentinelFileExist(const TCHAR* Name, FString* OptionalOutContents)
{
	return FPlatformProcess::CreateProcessSentinelObject()->DoesSentinelFileExistImpl(Name, OptionalOutContents);
}

FString IProcessSentinel::MakeFilename(const TCHAR* Name)
{
	return FPaths::ProjectIntermediateDir() + Name + TEXT(".sentinel");
}
...
TTuple<FProcHandle, UE::HAL::EProcessId> FGenericPlatformProcess::CreateProc(const UE::HAL::FProcessStartInfo& StartInfo)
{
	UE_LOG(LogHAL, Fatal, TEXT("FGenericPlatformProcess::CreateProc not implemented on this platform"));
	return {};
}
...
void FGenericPlatformProcess::CeaseBeingFirstInstance()
{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
...
class FGenericProcessSentinel : public IProcessSentinel
{
public:

	FGenericProcessSentinel()
	{
		FCoreDelegates::OnHandleSystemError.AddRaw(this, &FGenericProcessSentinel::OnCrash);
		FCoreDelegates::OnShutdownAfterError.AddRaw(this, &FGenericProcessSentinel::OnShutdown);
	}
	
	virtual ~FGenericProcessSentinel()
	{
		Cleanup();
	}
	...
	virtual bool CreateSentinelFile(const TCHAR* Name, const FString& Contents) override
	{
		...
		if (IFileManager::Get().FileExists(*SavedFilename))
		{
			if (!IFileManager::Get().Delete(*SavedFilename))
			{
				// attempt to lock it, if we can't, then another instance is running
				UE_LOG(LogHAL, Error, TEXT("Another instance is running, unable to delete Sentinel file %s"), *SavedFilename);
				return false;
			}
		}

		// open the file for write so other processes can't
		IFileHandle* Handle = FPlatformFileManager::Get().GetPlatformFile().OpenWrite(*SavedFilename);
		if (Handle)
		{
			char PidString[16];
			int Len = FCStringAnsi::Sprintf(PidString, "%d\n", FPlatformProcess::GetCurrentProcessId());
			Handle->Write((uint8*)PidString, Len);
			
			FTCHARToUTF8 Convert(*Contents);
			Handle->Write((uint8*)Convert.Get(), Convert.Length());
			Handle->Flush(true);
...
private:
	...
	/** 
	 * The actual implementation to determine if the sentinel file is exists and is valid (due to a process dying,
	 * the file may actually exist on disk, but isn't a valid file anymore, so a simple FileExists may not be good enough
	 */
	virtual bool DoesSentinelFileExistImpl(const TCHAR* Name, FString* OptionalOutContents=nullptr) override
	{
		FString Filename = MakeFilename(Name);
		FString LocalContents;
		FString* Contents = (OptionalOutContents == nullptr) ? &LocalContents : OptionalOutContents;

		/////////////////////////////////////////////////////////////////////////////////////////////////////////////
		// ALTERNATE, MORE SECURE MODE
		// If the platform can depend on Delete failing while another process as the file open for Write, then we
		// can just attempt to delete it - if it gets deleted, then we know another process doesn't have it open 
		// for write, which means the process died unexpectedly, and we can clean it up like it wasn't there
		// However, not all platforms will block deleting if the file is open for write, so it's not a generic
		// solution. 
		// if (!IFileManager::Get().FileExists(*SavedFilename) || IFileManager::Get().Delete(*SavedFilename))
		// {
		//	return false;
		// }
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////

		// get the PID from the first line
		if (FFileHelper::LoadFileToString(*Contents, *Filename))
		{
			int LineBreak = Contents->Find(TEXT("\n"));
			if (LineBreak > 0)
			{
				FString PidString = Contents->Mid(0, LineBreak);
				int Pid = FCString::Atoi(*PidString);
				*Contents = Contents->Mid(LineBreak + 1);
				
				// is pid running
				FProcHandle Proc = FPlatformProcess::OpenProcess(Pid);
				if (FPlatformProcess::IsProcRunning(Proc))
				{
					return true;
				}
				
				// if not, delete the file to clean up
				IFileManager::Get().Delete(*SavedFilename);
			}
		}
	...
	/** Callback functions on crash and shutdown after error */
	void OnCrash()
	{
		Cleanup();
	}
...
TSharedPtr<IProcessSentinel> FGenericPlatformProcess::CreateProcessSentinelObject()
{
	UE_LOG(LogHAL, Fatal, TEXT("This platform doesn't currently support process sentinels, because the generic one can't guarantee that the file will go away on abnormal termination (ie, killing the process, as opposed to crashing normally). If this is needed, look at FGenericProcessSentinel, it's a good starting point."));
	return TSharedPtr<IProcessSentinel>(new FGenericProcessSentinel());
}

TSharedPtr<IProcessSentinel> FGenericPlatformProcess::CreateProcessSentinel(const TCHAR* Name, const FString& Contents)	
{
	TSharedPtr<IProcessSentinel> Sentinel = FPlatformProcess::CreateProcessSentinelObject();
	
	Sentinel->CreateSentinelFile(Name, Contents);
	return Sentinel;
}

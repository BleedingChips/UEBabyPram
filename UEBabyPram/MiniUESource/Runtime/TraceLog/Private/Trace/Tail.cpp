...
{
	Cursor -= (InSize + sizeof(PacketType));
}
...
void Writer_TailAppend(uint32 ThreadId, uint8* __restrict Data, uint32 Size)
{
	...
	if (Size > 384)
	{
		...
		int32 EncodedSize = Encode(Data, Size, Packet->Data, EncodeMaxSize);
		if (EncodedSize > 0)
		{
			// Reclaim buffer space (worst case size - actual size)
			uint32 BackUp = EncodeMaxSize - EncodedSize;
		...
		{
			// Compression failed, back out of entire packet
			GPacketRing.BackUp<FTidPacketEncoded>(EncodeMaxSize);
		}
	}

	FTidPacket* Packet = GPacketRing.Append<FTidPacket>(Size);
	Packet->ThreadId = uint16(ThreadId);
#if UE_TRACE_PACKET_VERIFICATION
	Packet->ThreadId |= FTidPacketBase::Verification;
#endif

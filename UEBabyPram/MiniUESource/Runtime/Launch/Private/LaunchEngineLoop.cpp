...
// Different levels of color for scoped named events 
static const FColor FrameColor = FColor::Red;
static const FColor SystemColor = FColor::Orange;
static const FColor CoreDelegatesColor = FColor::Cyan;
	...
	FConsoleCommandWithArgsDelegate::CreateLambda( [](const TArray<FString>& Args) 
		{
			...
			{
				SCOPED_NAMED_EVENT_TEXT("FCoreDelegates::OnCommandletPreMain", CoreDelegatesColor);
			...
			{
				SCOPED_NAMED_EVENT_TEXT("FCoreDelegates::OnCommandletPostMain", CoreDelegatesColor);
...
#if defined(WITH_LAUNCHERCHECK) && defined(WITH_LAUNCHERCHECK_PAKVERIFY) && WITH_LAUNCHERCHECK && WITH_LAUNCHERCHECK_PAKVERIFY
struct FPakValidator
{
	explicit FPakValidator() :
		bPakMountFailed(false)
	{
		Handle = FCoreDelegates::PakFileMountFailedDelegate.AddLambda([this](const FString&) { bPakMountFailed = true; });
	}

	~FPakValidator()
	{
		FCoreDelegates::PakFileMountFailedDelegate.Remove(Handle);
	}

	bool HasPakMountFailed() const { return bPakMountFailed; }

	static void HandlePakMountError()
	{
		bool bRunVerifyInstallation = false;
		if (ILauncherCheckModule::Get().WasRanFromLauncher())
		{
			bRunVerifyInstallation = FMessageDialog::Open(
				EAppMsgCategory::Error,
				EAppMsgType::YesNo,
				EAppReturnType::No,
				LOCTEXT("Error.CorruptInstall.VerifyLauncher", "Corrupt files detected. \n\nDo you want to verify your installation?"),
				LOCTEXT("Error.CorruptInstall.Title", "Corrupt files detected")
			) == EAppReturnType::Yes;
		}
		else
		{
			FMessageDialog::Open(
				EAppMsgCategory::Error,
				EAppMsgType::Ok,
				LOCTEXT("Error.CorruptInstall", "Corrupt files detected. Please verify your installation."),
				LOCTEXT("Error.CorruptInstall.Title", "Corrupt files detected")
			);
		}

		if (bRunVerifyInstallation)
		{
			ILauncherCheckModule::Get().RunLauncher(ELauncherAction::AppVerifyInstallation);
			RequestEngineExit(TEXT("Corrupt files detected; running verify in launcher"));
		}
		else
		{
			RequestEngineExit(TEXT("Corrupt files detected; exiting"));
		}
	}

private:
	FDelegateHandle Handle;
	bool bPakMountFailed;
};
...
{
	static void HandlePakMountError() {}
	bool HasPakMountFailed() const { return false; }
};
...
bool LaunchCheckForFileOverride(const TCHAR* CmdLine, bool& OutFileOverrideFound)
{
	...
	if (!NetworkPlatformFile)
	{
		...
		if (NetworkPlatformFile)
		{
			...
			// pak streaming currently needs this to be the PlatformPhysical, although it can potentially
			// cause issues for other code that assumes PlatformPhysical only refers to the absolute lowest
			// level for the platform
...
static void UpdateCoreCsvStats_BeginFrame()
{
	...
	{
		TRACE_CPUPROFILER_EVENT_SCOPE(UpdateCoreCsvStats_BeginFrame);
		if (!IsRunningDedicatedServer())
		{
#if !UE_BUILD_SHIPPING
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(UpdateCoreCsvStats_EndFrame);
	if (!IsRunningDedicatedServer())
	{
		...
		{
			static const auto CVarPerProcessGPUStats = IConsoleManager::Get().FindTConsoleVariableDataInt(TEXT("r.GPUStatisticsPerProcess"));
			...
			CSV_CUSTOM_STAT(GPUUsage, Usage, GPUUsage.CurrentProcessMHz * 100.0f, ECsvCustomStatOp::Set);
			CSV_CUSTOM_STAT(GPUUsage, Memory, GPUUsage.CurrentProcessMemoryUsage * 100.0f, ECsvCustomStatOp::Set);
			CSV_CUSTOM_STAT(GPUUsage, ExternalUsage, GPUUsage.ExternalProcessesMHz * 100.0f, ECsvCustomStatOp::Set);
			CSV_CUSTOM_STAT(GPUUsage, ExternalMemory, GPUUsage.ExternalProcessMemoryUsage * 100.0f, ECsvCustomStatOp::Set);
		}
#endif
	}
}
...
#if WITH_EDITOR
static void GenerateShaderHeaders()
{
	//Generate shader headers for all the shader platforms. It's necessary for the cook. I don't do it in the cook commandlet
	//because it's not early enough.
	ITargetPlatformManagerModule* PlatformModule = GetTargetPlatformManager();
	if (PlatformModule)
	{
		const TArray<ITargetPlatform*>& ActivePlatforms = PlatformModule->GetActiveTargetPlatforms();
...
#if defined(WITH_LAUNCHERCHECK) && WITH_LAUNCHERCHECK && !WITH_EDITOR
...
// Continue to protect shipping build from emitdrawevents (if needed in shipping, ALLOW_PROFILEGPU_IN_SHIPPING should be used instead to enable WITH_PROFILEGPU)
	...
	if (FApp::HasProjectName())
	{
		...
		// Strip suffix "-Platform-Config" to allow building module name to load: "App" + "-ModuleName" + "-Platform-Config"
		const FString ExecutableNameWithoutExtension = FPlatformProcess::ExecutableName();
		const FStringView BaseExecutableName = FPathViews::GetBaseFilename(ExecutableNameWithoutExtension);
		FStringView BasePrefix = BaseExecutableName;
		FString BaseSuffix;
		if (FApp::GetBuildConfiguration() != EBuildConfiguration::Development)
		{
			int32 LastDash = INDEX_NONE;
			if (BasePrefix.FindLastChar(TEXT('-'), LastDash))
			{
				BasePrefix.LeftInline(LastDash);
			...
			BaseSuffix = BaseExecutableName.RightChop(BasePrefix.Len());
		}

		const FString PathPrefix = FPaths::Combine(FPathViews::GetPath(ExecutableNameWithoutExtension), BasePrefix);
		...
		TArray<FString> FileNames =
		{
			...
			FString::Printf(TEXT("%s-Common%s.%s"), *PathPrefix, *BaseSuffix, FPlatformProcess::GetModuleExtension()),
...
#else
			FString::Printf(TEXT("%s-%sPreInit%s.%s"), *PathPrefix, FApp::GetProjectName(), *BaseSuffix, FPlatformProcess::GetModuleExtension()),
	...
	// Output devices.
	{
		...
		{
			SCOPED_NAMED_EVENT_TEXT("FCoreDelegates::OnOutputDevicesInit", CoreDelegatesColor);
...
#if WITH_EDITOR
		if (CommandletName == TEXTVIEW("cookcommandlet") || FParse::Param(FCommandLine::Get(), TEXT("RunAsCookCommandlet")))
		{
	...
	}
	{
		...
		{
			SCOPED_BOOT_TIMING("ConstructCompiledInObjects Early");
			UE::CoreUObject::ConstructCompiledInObjects();
		}
...
#endif // UE_WITH_IOSTOREONDEMAND

#endif // USE_IO_DISPATCHER
	...
	FDelayedAutoRegisterHelper::RunAndClearDelayedAutoRegisterDelegates(EDelayedRegisterRunPhase::PreRHIInit);
	{
	...
	*/
...
#if WITH_ENGINE
	{
		...
		if (!IsRunningDedicatedServer() && !IsRunningCommandlet())
		{
			...
			//If the PreLoadScreen is active, then we don't want to initialise the MoviePlayer from the same data.
			bool hasPreloadStartupMovie = false;
			//If true then we want the splash screen to be handled defaultly
			...
			if (FPreLoadScreenManager::Get())
			{
				...
				//If either early/engine screen is present then the DefaultMoviePlayer should not be configured with the same data.
				hasPreloadStartupMovie = 
					FPreLoadScreenManager::Get()->HasRegisteredPreLoadScreenType(EPreLoadScreenTypes::EarlyStartupScreen) ||
					FPreLoadScreenManager::Get()->HasRegisteredPreLoadScreenType(EPreLoadScreenTypes::EngineLoadingScreen);
			...
			//If a PreLoadScreenManager is present, then MoviePlayer should not read the same configurable data and rely on a different configuration setup.
			if (IsMoviePlayerEnabled() && !hasPreloadStartupMovie)
			{
...
#endif // !UE_SERVER
		{
			...
			{
				SCOPED_BOOT_TIMING("LoadModulesForProject(ELoadingPhase::PreEarlyLoadingScreen)");
				// Load up all modules that need to hook into the loading screen, even though there
				// is no loading screen we still need to load the modules registered in that phase.
				if (!IProjectManager::Get().LoadModulesForProject(ELoadingPhase::PreEarlyLoadingScreen)
		...
		// To make sure the shader headers are generated correctly, it needs to run after ProcessNewlyLoadedUObjects. ProcessNewlyLoadedUObjects sets up
		// the DeviceProfileManager, which is necessary to access certain console variables.
		// Also, the shader header needs to be generated before the call to GEditor->InitEditor because some modules (at least Niagara) will generate
		// and cache shaders dependencies during plugin loading so I need the shader headers at that moment.
		if (AllowShaderCompiling())
		{
	...
	if (!bHasEditorToken && !IsRunningDedicatedServer())
	{
		...
		if (!bIsRegularClient)
		{
			...
			// Call init callbacks
			{
				SCOPED_NAMED_EVENT_TEXT("FCoreDelegates::OnPostEngineInit", CoreDelegatesColor);
	...
	if( IsEngineExitRequested() )
	{
		if ( GEngine != nullptr )
		{
			...
			if (GIsEditor)
			{
				SCOPED_NAMED_EVENT_TEXT("FCoreDelegates::OnEditorPreExit", CoreDelegatesColor);
...
int32 FEngineLoop::Init()
{
	...
	// Call init callbacks
	{
		SCOPED_BOOT_TIMING("FCoreDelegates::OnPostEngineInit");
		FCoreDelegates::OnPostEngineInit.Broadcast();
	}
    ...
    {
		SCOPED_BOOT_TIMING("FCoreDelegates::OnFEngineLoopInitComplete");
	...
	FThreadHeartBeat::Get().InitSettings();
	FGameThreadHitchHeartBeat::Get().InitSettings();
...
void FEngineLoop::Exit()
{
	...
	SCOPED_NAMED_EVENT_TEXT("FEngineLoop::Exit", FrameColor);
...
static inline void BeginFrameRenderThread(FRHICommandListImmediate& RHICmdList, uint64 CurrentFrameCounter)
{
	...
	{
		SCOPED_NAMED_EVENT_TEXT("FCoreDelegates::OnBeginFrameRT", CoreDelegatesColor);
...
static inline void EndFrameRenderThread(FRHICommandListImmediate& RHICmdList, uint64 CurrentFrameCounter)
{
	...
	{
		SCOPED_NAMED_EVENT_TEXT("FCoreDelegates::OnEndFrameRT", CoreDelegatesColor);
...
void FEngineLoop::Tick()
{
	...
	{
		SCOPED_NAMED_EVENT(Tick_Hotfixables, SystemColor);
...
#endif
	SCOPED_NAMED_EVENT_TCHAR_CONDITIONAL(FrameString, FrameColor, !bTraceCpuChannelEnabled); 
#endif //ENABLE_NAMED_EVENTS
...

	{
		...
		{
			SCOPED_NAMED_EVENT_TEXT("FCoreDelegates::OnBeginFrame", CoreDelegatesColor);
		...
		{
			SCOPED_NAMED_EVENT(FEngineLoop_UpdateTimeAndHandleMaxTickRate, SystemColor);
		...
		{
			SCOPED_NAMED_EVENT(Render_BeginFrame, SystemColor);
		...
		{
			SCOPED_NAMED_EVENT(Tick_PerformanceMonitoring, SystemColor);
		...
		{
			SCOPED_NAMED_EVENT(Malloc_UpdateStats, SystemColor);
...

	{
		...
		bool bIdleMode;
		{
			SCOPED_NAMED_EVENT(FEngineLoop_Idle, SystemColor);
		...
		{
			SCOPED_NAMED_EVENT_TEXT("FCoreDelegates::OnSamplingInput", CoreDelegatesColor);
		...
		if (FSlateApplication::IsInitialized() && !bIdleMode)
		{
			...
			SCOPED_NAMED_EVENT(Tick_SlateInput, SystemColor);
			LLM_SCOPE(ELLMTag::UI);

			FSlateApplication& SlateApp = FSlateApplication::Get();
            {
				TRACE_CPUPROFILER_EVENT_SCOPE(PollGameDeviceState);
                SlateApp.PollGameDeviceState();
            }
			// Gives widgets a chance to process any accumulated input
            {
				TRACE_CPUPROFILER_EVENT_SCOPE(FinishedInputThisFrame);
                SlateApp.FinishedInputThisFrame();
            }
		}
		
		{
			SCOPED_NAMED_EVENT(Tick_Engine, SystemColor);
		...
		// We do this right after GEngine->Tick() because that is where user code would initiate a load / movie.
		{
			...
			{
				TRACE_CPUPROFILER_EVENT_SCOPE(WaitForMovieToFinish);
		...
		{
			{
				SCOPED_NAMED_EVENT(ProcessLocalPlayerSlateOperations, SystemColor);
		...
		if (bDoConcurrentSlateTick)
		{
			...
			if (CurrentDemoNetDriver && CurrentDemoNetDriver->ShouldTickFlushAsyncEndOfFrame())
			{
				...
				if (bValidateReplicatedProperties)
				{
					...
					TRACE_CPUPROFILER_EVENT_SCOPE(ConcurrentWithSlateTickTasks_Wait);
					check(ConcurrentTaskCompleteEvent);
		...
		{
			SCOPED_NAMED_EVENT(Tick_AutomationController, SystemColor);
		...
		{
			SCOPED_NAMED_EVENT(Tick_AutomationWorker, SystemColor);
		...
		{
			SCOPED_NAMED_EVENT(Render_UpdateFrame, SystemColor);
		...
		// tick core ticker, threads & deferred commands
		{
			...
			{
				SCOPED_NAMED_EVENT(Tick_Core, SystemColor);
				FTSTicker::GetCoreTicker().Tick(FApp::GetDeltaTime());
				FThreadManager::Get().Tick();
			}

			{
				SCOPED_NAMED_EVENT(Tick_DeferredCommands, SystemColor);
		...
		if (MediaModule != nullptr)
		{
			SCOPED_NAMED_EVENT(Tick_PostRender, SystemColor);
		...
		{
			SCOPED_NAMED_EVENT(Render_EndFrame, SystemColor);
...
#if PLATFORM_MAC
	// drain the apple async task queue from the game thread
	[FAppleAsyncTask ProcessAsyncTasks];
#endif
}
...
bool DoesCurrentTargetRequireUpgrade(const FString& EditorTargetFileName, FString& InTargetName)
{
	...
	// We do not return on Receipt.Read returning false, as Target upgrade requirements occur before a full target receipt can be emitted.
	// Only partial target receipts exist, of which the most important properties are BuildSettingsVersion && TargetBuildEnvironment.
	Receipt.Read(EditorTargetFileName);

	// Handle backwards compatibility before we added these properties to the receipt at 5.7.
	if (Receipt.BuildSettingsVersion.IsEmpty() || Receipt.TargetBuildEnvironment.IsEmpty())
	{
		UE_LOG(
			LogInit,
			Log,
			TEXT("Unable to discern build settings version & build environment used for target - target may require upgrade. Target file: %s"),
			*EditorTargetFileName
		);
		return false;
	}

	InTargetName = Receipt.TargetName;
	const bool bIsShared = (Receipt.TargetBuildEnvironment == TEXT("Shared"));
	const bool bIsUpToDate = (Receipt.BuildSettingsVersion == TEXT("V6"));

	if (!bIsUpToDate && bIsShared)
	{
		UE_LOG(
			LogInit,
			Warning,
			TEXT("Current target requires upgrade to V6, as it is not a unique build environment. File: %s, BuildSettingsVersion: %s, TargetBuildEnvironment: %s"),
			*EditorTargetFileName,
...
bool FEngineLoop::AppInit( )
{
	...
	if(!GIsBuildMachine && !FApp::IsUnattended() && FPaths::IsProjectFilePathSet() && !PluginManager.GetPreloadBinaries())
	{
		...
		if(!bNeedCompile)
		{
			// Check if any of the target, project or plugin modules are out of date, and the user wants to compile them.
			FString targetName;
			bool TargetRequiresUpgrade = DoesCurrentTargetRequireUpgrade(EditorTargetFileName, targetName);

			if (TargetRequiresUpgrade)
			{
				FString TargetRequiresUpgradeMessage = FString::Printf(
					TEXT("The last build used an outdated BuildSettingsVersion, or the target still requires upgrading."
				...
				if (FPlatformMisc::MessageBoxExt(EAppMsgType::YesNo, *TargetRequiresUpgradeMessage, TEXT("Target Upgrade Required")) == EAppReturnType::No)
				{
					return false;
				}
				
				// Rebuilding should update the target receipt file, and prevent subsequent checks.
				bNeedCompile = true;
			}
	...
	// Init other systems.
	{
		SCOPED_BOOT_TIMING("FCoreDelegates::OnInit");
...
void FEngineLoop::AppPreExit( )
{
	...
	{
		SCOPED_NAMED_EVENT_TEXT("FCoreDelegates::OnPreExit", CoreDelegatesColor);
	...
	{
		SCOPED_NAMED_EVENT_TEXT("FCoreDelegates::OnExit", CoreDelegatesColor);

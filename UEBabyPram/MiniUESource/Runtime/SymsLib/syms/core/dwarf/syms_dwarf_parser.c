...
SYMS_API SYMS_B32
syms_dw_mach_bin_accel_is_dbg(SYMS_MachBinAccel *bin_accel)
{
  SYMS_ArenaTemp scratch = syms_get_scratch(0,0);
  SYMS_B32 is_dbg = syms_false;
  SYMS_SecInfoArray sec_array = syms_mach_sec_info_array_from_bin(scratch.arena, syms_str8(0,0), bin_accel);
  for(SYMS_U64 i = 0; i < sec_array.count; i += 1)
  {
    if(syms_string_match(sec_array.sec_info[i].name, syms_str8_lit("__debug_info"), 0) ||
       syms_string_match(sec_array.sec_info[i].name, syms_str8_lit("__debug_line"), 0))
    {
      is_dbg = syms_true;
      break;
    }
...
syms_dw_dbg_accel_from_sec_info_array(SYMS_Arena *arena, SYMS_String8 data, SYMS_U64 vbase, SYMS_Arch arch, SYMS_SecInfoArray sections)
{
  ...
  SYMS_DwMode aranges_mode = SYMS_DwMode_Null;
  void *aranges_base = 0;
  ...
  if(has_necessary_sections)
  {
    ...
    aranges_mode = syms_dw_mode_from_sec(section_map, SYMS_DwSectionKind_ARanges);
  ...
  if(has_necessary_sections)
  {
    ...
    SYMS_U64RangeArray *unit_aranges = syms_push_array_zero(scratch.arena, SYMS_U64RangeArray, unit_count);
    {
      for(SYMS_U64 offset = 0; offset < aranges_rng.max;)
      {
        ...
        offset += syms_based_range_read(aranges_base, aranges_rng, offset, syms_dw_offset_size_from_mode(aranges_mode), &debug_info_offset);
...
SYMS_API SYMS_DwMode
syms_dw_mode_from_sec(SYMS_DwSection *section_map, SYMS_DwSectionKind kind)
{
  SYMS_U64Range range = section_map[kind].range;
  SYMS_U64 size = range.max - range.min;
  if(size > 0xffffffff)
  {
    return SYMS_DwMode_64Bit;
  }
  else
  {
    return SYMS_DwMode_32Bit;
  }
}
...
syms_dw_v4_pub_strings_table_from_section_kind(SYMS_Arena *arena, SYMS_String8 data, SYMS_DwDbgAccel *dbg, SYMS_DwSectionKind section_kind)
{
  ...
  SYMS_DwMode mode = syms_dw_mode_from_sec(dbg->section_map, section_kind);
  void *base = syms_dw_sec_base_from_dbg(data, dbg, section_kind);
  SYMS_U64Range rng = syms_dw_sec_range_from_dbg(dbg, section_kind);
  SYMS_U64 read_off = 0;
  
  SYMS_U64 off_size = syms_dw_offset_size_from_mode(mode);
...
syms_dw_v5_offset_from_offs_section_base_index(SYMS_String8 data, SYMS_DwDbgAccel *dbg, SYMS_DwSectionKind section, SYMS_U64 base, SYMS_U64 index)
{
  ...
  SYMS_DwMode mode = syms_dw_mode_from_sec(dbg->section_map, section);
  ...
  SYMS_U64 entry_len = mode == SYMS_DwMode_64Bit ? 8 : 4;
...
{
  //- rjf: set up prereqs
  SYMS_DwMode info_mode = syms_dw_mode_from_sec(dbg->section_map, SYMS_DwSectionKind_Info);
  void *info_base = syms_dw_sec_base_from_dbg(data, dbg, SYMS_DwSectionKind_Info);
  SYMS_U64Range info_range = syms_dw_sec_range_from_dbg(dbg, SYMS_DwSectionKind_Info);
  SYMS_DwMode abbrev_mode = syms_dw_mode_from_sec(dbg->section_map, SYMS_DwSectionKind_Abbrev);
  ...
  for(SYMS_B32 good_abbrev = syms_true; good_abbrev;)
  {
    ...
    if(good_abbrev)
    {
      ...
      SYMS_U64 bytes_read = syms_dw_based_range_read_attrib_form_value(info_base, info_range, info_read_off, info_mode, address_size,
...
syms_dw_comp_root_from_range(SYMS_Arena *arena, SYMS_String8 data, SYMS_DwDbgAccel *dbg, SYMS_U64 index, SYMS_U64Range range)
{
  ...
  {
    SYMS_DwMode line_mode = syms_dw_mode_from_sec(dbg->section_map, SYMS_DwSectionKind_Line);
    ...
    SYMS_U64 read_size = syms_dw_read_line_vm_header(arena, line_base, line_rng, unit.line_off, data, line_mode, dbg, &unit, &vm_header);
    if (read_size > 0) {
...
syms_dw_parsed_line_table_from_comp_root(SYMS_Arena *arena, SYMS_String8 data, SYMS_DwDbgAccel *dbg, SYMS_DwCompRoot *root)
{
  ...
  SYMS_DwMode mode = syms_dw_mode_from_sec(dbg->section_map, SYMS_DwSectionKind_Line);
  ...
  SYMS_DwLineVMHeader vm_header;
  read_off += syms_dw_read_line_vm_header(arena, base, line_info_range, read_off, data, mode, dbg, root, &vm_header);
...
SYMS_API SYMS_U64
syms_dw_read_line_file(void *line_base, SYMS_U64Range line_rng, SYMS_U64 line_off, SYMS_DwMode mode, SYMS_DwDbgAccel *dbg, SYMS_String8 data, SYMS_DwCompRoot *unit, SYMS_U8 address_size, SYMS_U64 format_count, SYMS_U64Range *formats, SYMS_DwLineFile *line_file_out)
{
  ...
  for (SYMS_U64 format_idx = 0; format_idx < format_count; ++format_idx) 
  {
    ...
    SYMS_DwAttribValue form_value;
    syms_memzero_struct(&form_value);
    line_off += syms_dw_based_range_read_attrib_form_value(line_base, line_rng, line_off, mode, address_size, form_kind, 0, &form_value);
    switch (lnct) 
    {
...
SYMS_API SYMS_U64
syms_dw_read_line_vm_header(SYMS_Arena *arena, void *line_base, SYMS_U64Range line_rng, SYMS_U64 line_off, SYMS_String8 data, SYMS_DwMode mode, SYMS_DwDbgAccel *dbg, SYMS_DwCompRoot *unit, SYMS_DwLineVMHeader *header_out)
{
  SYMS_U64 line_off_start = line_off;

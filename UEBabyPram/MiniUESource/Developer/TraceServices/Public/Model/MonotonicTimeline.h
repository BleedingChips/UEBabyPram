...
public:
	...
	/**
	 * Gets the last time added to the timeline.
	 *
	 * @return The last timestamp added to the timeline.
	 */
	...
	, public IEditableTimeline<InEventType>
{
	...
	typedef FEnumerateAsyncTask<InEventType, SettingsType> FEnumerateAsyncTask;
...
public:
	...
	virtual void EnumerateEventsDownSampled(double IntervalStart, double IntervalEnd, double Resolution, typename ITimeline<EventType>::EventCallback Callback) const override
	{
		...
		// The page of the first scope...
		// i.e. the previous page of the first page with Page.BeginTime > IntervalStart
		uint64 FirstScopePageIndex = Algo::UpperBoundBy(DetailLevel.ScopeEntries, IntervalStart,
			[](const FEventScopeEntryPage& Page)
			{
		...
		// Make a copy of the initial stack of the page.
		...
		{
			if (ScopeEntry->Time < 0.0) // "begin" event
			{
		...
		if (bSearchEndTimeUsingPages)
		{
			...
			do
			{
				...
				while (CurrentStackDepth > 0 && CurrentScopePage->InitialStack[CurrentStackDepth - 1].EndTime > 0.0)
				{
					--CurrentStackDepth;
	...
	virtual void EnumerateEventsDownSampled(double IntervalStart, double IntervalEnd, double Resolution, typename ITimeline<EventType>::EventRangeCallback Callback) const override
	{
		...
		EnumerateEventsDownSampled(IntervalStart, IntervalEnd, Resolution,
			[&EventStack, &OutputEvents, &CurrentDepth, Callback]
			(bool IsEnter, double Time, const EventType& Event)
			{
				if (IsEnter)
				{
	...
	virtual void EnumerateEventsBackwardsDownSampled(double IntervalEnd, double IntervalStart, double Resolution, typename ITimeline<EventType>::EventCallback Callback) const override
	{
		...
		// The page of the last scope...
		// i.e. the first page with Page.BeginTime > IntervalEnd
		uint64 LastScopePageIndex = Algo::UpperBoundBy(DetailLevel.ScopeEntries, IntervalEnd,
			[](const FEventScopeEntryPage& Page)
			{
				return Page.BeginTime;
			});
		...
		{
			// If we have a page we can start from, start enumerating backwards from the beginning of that page.
			...
			for (int32 InitialStackIndex = 0; InitialStackIndex < CurrentStackDepth; ++InitialStackIndex)
			{
				...
				if (EnumerationStackEntry.EndTime < 0.0)
				{
					...
					{
						if (Page->InitialStack[InitialStackIndex].EndTime > 0.0)
						{
		...
		// Enumerate backwards until we reach IntervalEnd, without calling the Callback because these events are not in the provided interval.
		const EventType* Event = EventsIterator.GetCurrentItem();
		while (ScopeEntry && FMath::Abs(ScopeEntry->Time) > IntervalEnd)
		{
			if (ScopeEntry->Time < 0.0) // "begin" event
	...
	virtual void EnumerateEventsBackwardsDownSampled(double IntervalEnd, double IntervalStart, double Resolution, typename ITimeline<EventType>::EventRangeCallback Callback) const override
	{
		...
		EnumerateEventsBackwardsDownSampled(IntervalEnd, IntervalStart, Resolution,
			[&EventStack, &CurrentDepth, Callback]
			(bool IsEnter, double Time, const EventType& Event)
			{
				if (IsEnter)
				{
	...
	virtual void EnumerateEventsDownSampledAsync(const typename ITimeline<EventType>::EnumerateAsyncParams& EnumerateAsyncParams) const override
	{
		...
		// The page of the first scope...
		// i.e. the previous page of the first page with Page.BeginTime > EnumerateAsyncParams.IntervalStart
		uint64 FirstScopePageIndex = Algo::UpperBoundBy(DetailLevel.ScopeEntries, EnumerateAsyncParams.IntervalStart,
			[](const FEventScopeEntryPage& Page)
			{
				return Page.BeginTime;
			});
		...
		// The page of the last scope...
		// i.e. the first page with Page.BeginTime > EnumerateAsyncParams.IntervalEnd
		uint64 LastScopePageIndex = Algo::UpperBoundBy(DetailLevel.ScopeEntries, EnumerateAsyncParams.IntervalEnd,
			[](const FEventScopeEntryPage& Page)
			{
				return Page.BeginTime;
			});

		TArray<TSharedRef<FAsyncTask<FEnumerateAsyncTask>>> WorkerTasks;
		...
		if (EnumerateAsyncParams.MaxOccupancy > 0.0)
		{
		...
		for (uint32 TaskIndex = 0; TaskIndex < NumTasks; ++TaskIndex)
		{
			...
			TSharedRef<FAsyncTask<FEnumerateAsyncTask>> AsyncTask = MakeShared<FAsyncTask<FEnumerateAsyncTask>>(TaskData);
	...
	virtual bool GetEventInfo(double InTime, double DeltaTime, int32 Depth, typename ITimeline<InEventType>::FTimelineEventInfo& EventInfo) const override
	{
		...
		// The page of the first scope...
		// i.e. the previous page of the first page with Page.BeginTime > InTime
		uint64 FirstScopePageIndex = Algo::UpperBoundBy(DetailLevel.ScopeEntries, InTime,
			[](const FEventScopeEntryPage& Page)
			{
				return Page.BeginTime;
			});
	...
	virtual int32 GetDepthAt(double Time) const override
	{
		...
		// The page of the first scope...
		// i.e. the previous page of the first page with Page.BeginTime > Time
		uint64 FirstScopePageIndex = Algo::UpperBoundBy(DetailLevel.ScopeEntries, Time,
			[](const FEventScopeEntryPage& Page)
			{
				return Page.BeginTime;
			});
...
private:
										...
										FEventStackEntry& OutPageStackEntry) const
	{
		...
		if (CurrentScopePage->InitialStackCount > Depth)
		{
			...
			if (CurrentPageStackEntry.EndTime < 0.0 || CurrentPageStackEntry.EndTime > Time)
			{
		...
		while (CurrentScopePage->InitialStack[Depth].EndTime < 0.0)
		{
			const FEventScopeEntryPage* NextScopePage = ScopeEntryIterator.NextPage();
			if (NextScopePage == nullptr)
			{

...
struct FModule
{
	...
	struct SymbolStats
	{
		std::atomic<uint32>		Discovered = 0; // number of symbols discovered in the trace (used symbols)
		std::atomic<uint32>		Cached = 0;     // number of symbols loaded from cache
		std::atomic<uint32>		Resolved = 0;   // number of symbols successfully resolved from the loaded module
		std::atomic<uint32>		Failed = 0;     // number of symbols failed to resolve
		std::atomic<uint32>		Available = 0;  // number of symbols available in the loaded module's debug information
	} Stats;
...
public:
	...
	/** Checks if all symbols have been resolved in resolver queues. Only useful after analysis has completed (e.g., OnAnalysisComplete). */
	virtual bool HasFinishedResolving() const = 0;
};

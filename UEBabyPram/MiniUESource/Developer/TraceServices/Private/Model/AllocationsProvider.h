...
public:
	...
	virtual bool HasAllocationEvents() const { ReadAccessCheck(); return AllocEvents > 0 || FreeEvents > 0 || HeapEvents > 0; }

	virtual bool HasSwapOpEvents() const { ReadAccessCheck(); return SwapOpEvents > 0; }
	...
	virtual void EnumerateTimeline(ETimelineU64 Timeline, int32 StartIndex, int32 EndIndex, TFunctionRef<void(double Time, double Duration, uint64 Value)> Callback) const override;
	virtual void EnumerateTimeline(ETimelineU32 Timeline, int32 StartIndex, int32 EndIndex, TFunctionRef<void(double Time, double Duration, uint32 Value)> Callback) const override;
...
private:
	...
	void SetSampleU64(ETimelineU64 TimelineIndex, uint64 Value)
	{
		uint64& Sample = SampleValuesU64[int32(TimelineIndex)];
		Sample = Value;
	}
	void SetMinSampleU64(ETimelineU64 TimelineIndex, uint64 Value)
	{
		uint64& Sample = SampleValuesU64[int32(TimelineIndex)];
		Sample = FMath::Min(Sample, Value);
	}
	void SetMaxSampleU64(ETimelineU64 TimelineIndex, uint64 Value)
	{
		uint64& Sample = SampleValuesU64[int32(TimelineIndex)];
		Sample = FMath::Max(Sample, Value);
	}
	void SetSampleU32(ETimelineU32 TimelineIndex, uint32 Value)
	{
		uint32& Sample = SampleValuesU32[int32(TimelineIndex)];
		Sample = Value;
	}
	void SetMinSampleU32(ETimelineU32 TimelineIndex, uint32 Value)
	{
		uint32& Sample = SampleValuesU32[int32(TimelineIndex)];
		Sample = FMath::Min(Sample, Value);
	}
	void SetMaxSampleU32(ETimelineU32 TimelineIndex, uint32 Value)
	{
		uint32& Sample = SampleValuesU32[int32(TimelineIndex)];
		Sample = FMath::Max(Sample, Value);
	}
	void IncSampleU32(ETimelineU32 TimelineIndex)
	{
		uint32& Sample = SampleValuesU32[int32(TimelineIndex)];
		++Sample;
	}
...
private:
	...
	uint64 AllocEvents = 0;
	uint64 FreeEvents = 0;
	uint64 HeapEvents = 0;
	uint64 SwapOpEvents = 0;
	...
	static constexpr int32 NumTimelinesU64 = int32(IAllocationsProvider::ETimelineU64::Count);
	static constexpr int32 NumTimelinesU32 = int32(IAllocationsProvider::ETimelineU32::Count);

	double SampleStartTimestamp = 0.0;
	double SampleEndTimestamp = 0.0;
	uint64 SampleValuesU64[NumTimelinesU64];
	uint32 SampleValuesU32[NumTimelinesU32];

	TPagedArray<double> Timeline;
	TPagedArray<uint64>* ValueTimelinesU64[NumTimelinesU64];
	TPagedArray<uint32>* ValueTimelinesU32[NumTimelinesU32];
};

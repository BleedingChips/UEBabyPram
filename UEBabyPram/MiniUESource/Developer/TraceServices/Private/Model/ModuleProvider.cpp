...
private:
	TArray<FStringView> IgnoreSymbolsByModuleName;
...
public:
	...
	bool HasFinishedResolving() const override;
	...
	if (Module)
	{
		...
		if (Resolver && !FullPath.IsEmpty() && Module->Status.load() != EModuleStatus::Loaded)
		{
			...
			LoadSymbolsTask = FFunctionGraphTask::CreateAndDispatchWhenReady([this, Module, FullPath]()
			{
				...
				UE_LOG(LogTraceServices, Display, TEXT("[Modules] Queuing symbol loading using path %s."), *FullPath);
				...
				UE_LOG(LogTraceServices, Display, TEXT("[Modules] Completed loading symbols for path %s."), *FullPath);
			});
...
{
	// Get stats from resolver.
	Resolver->GetStats(OutStats);

	// The resolver does not include Cached into Discovered stat (but only Resolved and Failed).
	// Also, the resolver's Discovered and Cached stats (computed as sum of all modules) are not accurate
	// due to overlaps and unloaded/reloaded modules.
	// So we override these with the provider's stats.
	OutStats->SymbolsDiscovered = SymbolsDiscovered.load() + NumCachedSymbols;
	OutStats->SymbolsCached = NumCachedSymbols;
}

////////////////////////////////////////////////////////////////////////////////////////////////////

template<typename SymbolResolverType>
bool TModuleProvider<SymbolResolverType>::HasFinishedResolving() const
{
	return Resolver->HasFinishedResolving();
}
...
void TModuleProvider<SymbolResolverType>::OnModuleLoad(const FStringView& ModuleName, uint64 Base, uint32 Size, const uint8* ImageId, uint32 ImageIdSize)
{
	if (ModuleName.Len() == 0)
	{
		UE_LOG(LogTraceServices, Warning, TEXT("[Modules] No name module is ignored (Base=0x%llX, Size=%u) !!!"), Base, Size);
		return;
	}
	...
	for (uint32 ModuleIndex = 0; ModuleIndex < Modules.Num(); ++ModuleIndex)
	{
		FModule& LoadedModule = Modules[ModuleIndex];
		...
		{
			if (LoadedModule.bIsUnloaded)
			{
				UE_LOG(LogTraceServices, Log, TEXT("[Modules] Re-loaded module %s (Base=0x%llX, Size=%u)."), LoadedModule.Name, LoadedModule.Base, LoadedModule.Size);
				LoadedModule.bIsUnloaded = false;
			}
			else
			{
				// Duplicated module !!!
				UE_LOG(LogTraceServices, Warning, TEXT("[Modules] Duplicated module %s (Base=0x%llX, Size=%u) !!!"), LoadedModule.Name, LoadedModule.Base, LoadedModule.Size);
			}
			return;
		}

		// Check if the new module overlaps an existing (active) module.
		if (!LoadedModule.bIsUnloaded &&
			Base + Size > LoadedModule.Base &&
			Base < LoadedModule.Base + LoadedModule.Size)
		{
			// Overlapping modules !!!
			UE_LOG(LogTraceServices, Warning, TEXT("[Modules] Module %s (Base=0x%llX, Size=%u) overlaps with module %s (Base=0x%llX, Size=%u) !!!"),
				Name, Base, Size, LoadedModule.Name, LoadedModule.Base, LoadedModule.Size);
		}
	}

	FModule& NewModule = Modules.EmplaceBack(Name, FullName, Base, Size, EModuleStatus::Discovered);

	UE_LOG(LogTraceServices, Log, TEXT("[Modules] Loaded module %s (Base=0x%llX, Size=%u)."), NewModule.Name, NewModule.Base, NewModule.Size);
...
void TModuleProvider<SymbolResolverType>::OnModuleUnload(uint64 Base)
{
	...
	for (uint32 ModuleIndex = 0; ModuleIndex < Modules.Num(); ++ModuleIndex)
	{
		...
		if (LoadedModule.Base == Base && !LoadedModule.bIsUnloaded)
		{
			LoadedModule.bIsUnloaded = true;
			UE_LOG(LogTraceServices, Log, TEXT("[Modules] Unloaded module %s (Base=0x%llX, Size=%u)."), LoadedModule.Name, LoadedModule.Base, LoadedModule.Size);
			return;
		}
	}

	// Unknown module !!!
	UE_LOG(LogTraceServices, Warning, TEXT("[Modules] No module to unload with Base=0x%llX !!!"), Base);
}
...
void TModuleProvider<SymbolResolverType>::SaveSymbolsToCache(IAnalysisCache& Cache)
{
	...
	UE_LOG(LogTraceServices, Display, TEXT("[Modules] Added %d symbols to the %d previously saved symbols."), NumSavedSymbols, NumPreviouslySavedSymbols);
}
...
void TModuleProvider<SymbolResolverType>::LoadSymbolsFromCache(IAnalysisCache& Cache)
{
	...
	for (uint64 SymbolIndex = 0; SymbolIndex < SavedSymbols.Num(); ++SymbolIndex)
	{
		...
		{
			UE_LOG(LogTraceServices, Warning, TEXT("[Modules] Found cached symbol (address 0x%llX) which referenced unknown string !!!"), Symbol.Address);
			continue;
		}
	...
	UE_LOG(LogTraceServices, Display, TEXT("[Modules] Loaded %d symbols from cache."), SymbolCacheLookup.Num());
}
...
FResolvedSymbolFilter::FResolvedSymbolFilter()
{
	...
	//IgnoreSymbolsByModuleName.Add(TEXTVIEW("kernel32.dll"));
	//IgnoreSymbolsByModuleName.Add(TEXTVIEW("ntdll.dll"));
	...
	IgnoreSymbolsByFunctionName.Add(TEXTVIEW("AnsiMalloc"));
	IgnoreSymbolsByFunctionName.Add(TEXTVIEW("AnsiRealloc"));
	IgnoreSymbolsByFunctionName.Add(TEXTVIEW("AnsiFree"));
	...
	IgnoreSymbolsByFilePath.Add(TEXTVIEW("/AutoRTFM/"));
	IgnoreSymbolsByFilePath.Add(TEXTVIEW("/HAL/FMemory.inl"));
}
...
void FResolvedSymbolFilter::Update(FResolvedSymbol& InSymbol) const
{
	...
	if (InSymbol.GetResult() == ESymbolQueryResult::NotLoaded)
	{
		bIsFiltered = true;
	}

	if (!bIsFiltered && InSymbol.Module)
	{
		// Ignore symbols by module name prefix.
		for (const FStringView& Prefix : IgnoreSymbolsByModuleName)
		{
			if (FCString::Strnicmp(InSymbol.Module, Prefix.GetData(), Prefix.Len()) == 0)
			{
				bIsFiltered = true;
				break;
			}

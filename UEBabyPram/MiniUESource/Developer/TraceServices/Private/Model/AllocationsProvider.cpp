...
{
	// Allocate the timelines
	for (int32 TimelineIndex = 0; TimelineIndex < NumTimelinesU64; ++TimelineIndex)
	{
		ValueTimelinesU64[TimelineIndex] = new TPagedArray<uint64>(Session.GetLinearAllocator(), 1024);
		SampleValuesU64[TimelineIndex] = 0;
	}
	for (int32 TimelineIndex = 0; TimelineIndex < NumTimelinesU32; ++TimelineIndex)
	{
		ValueTimelinesU32[TimelineIndex] = new TPagedArray<uint32>(Session.GetLinearAllocator(), 1024);
		SampleValuesU32[TimelineIndex] = 0;
	}
...
FAllocationsProvider::~FAllocationsProvider()
{
	...
	for (int32 TimelineIndex = 0; TimelineIndex < NumTimelinesU64; ++TimelineIndex)
	{
		if (ValueTimelinesU64[TimelineIndex])
		{
			delete ValueTimelinesU64[TimelineIndex];
			ValueTimelinesU64[TimelineIndex] = nullptr;
		}
	}
	for (int32 TimelineIndex = 0; TimelineIndex < NumTimelinesU32; ++TimelineIndex)
	{
		if (ValueTimelinesU32[TimelineIndex])
		{
			delete ValueTimelinesU32[TimelineIndex];
			ValueTimelinesU32[TimelineIndex] = nullptr;
		}
	}
}
...
FAllocationItem* FAllocationsProvider::AllocInternal(uint32 ThreadId, double Time, uint32 CallstackId, uint64 Address, uint64 Size, uint32 Alignment, HeapId RootHeapId)
{
...

	{
		...
		SetMaxSampleU64(ETimelineU64::MaxTotalAllocatedMemory, TotalAllocatedMemory);
		SetMaxSampleU32(ETimelineU32::MaxLiveAllocations, TotalLiveAllocations);
		IncSampleU32(ETimelineU32::AllocEvents);
	}
...
FAllocationItem* FAllocationsProvider::FreeInternal(uint32 ThreadId, double Time, uint32 CallstackId, uint64 Address, HeapId RootHeapId)
{
	...
	if (AllocationPtr)
	{
		...
		if (!AllocationPtr->IsHeap())
		{
			...
			--TotalLiveAllocations;
			SetMinSampleU64(ETimelineU64::MinTotalAllocatedMemory, TotalAllocatedMemory);
			SetMinSampleU32(ETimelineU32::MinLiveAllocations, TotalLiveAllocations);
		}
		IncSampleU32(ETimelineU32::FreeEvents);
	}
	else
	{
		++FreeErrors;
...
void FAllocationsProvider::EditSwapOp(uint32 ThreadId, double Time, uint64 UnmaskedPageAddress, EMemoryTraceSwapOperation SwapOp, uint64 CompressedPageSize, uint32 CallstackId)
{
	...
	{
		IncSampleU32(ETimelineU32::PageOutEvents);
		break;
	}
	case EMemoryTraceSwapOperation::PageIn:
	{
		IncSampleU32(ETimelineU32::PageInEvents);
		break;
	}
	case EMemoryTraceSwapOperation::FreeInSwap:
	{
		IncSampleU32(ETimelineU32::SwapFreeEvents);
		break;
	}
	default:
		break;
	...
	if (MaskedPagedAddress != 0)
	{
		...
		case EMemoryTraceSwapOperation::PageOut:
		{
			...
			SetMaxSampleU64(ETimelineU64::MaxTotalSwapMemory, TotalSwapMemory);
			SetMaxSampleU64(ETimelineU64::MaxTotalCompressedSwapMemory, TotalCompressedSwapMemory);
		...
		case EMemoryTraceSwapOperation::FreeInSwap:
		{
			...
			SetMinSampleU64(ETimelineU64::MinTotalSwapMemory, TotalSwapMemory);
			...
			if (AllocationPtr)
			{
				...
				SetMinSampleU64(ETimelineU64::MinTotalCompressedSwapMemory, TotalCompressedSwapMemory);
...
void FAllocationsProvider::AdvanceTimelines(double Time)
{
	...
	if (Time - SampleStartTimestamp > DefaultTimelineSampleGranularity)
	{
		...
		for (int32 TimelineIndex = 0; TimelineIndex < NumTimelinesU64; ++TimelineIndex)
		{
			ValueTimelinesU64[TimelineIndex]->EmplaceBack(SampleValuesU64[TimelineIndex]);
		}
		for (int32 TimelineIndex = 0; TimelineIndex < NumTimelinesU32; ++TimelineIndex)
		{
			ValueTimelinesU32[TimelineIndex]->EmplaceBack(SampleValuesU32[TimelineIndex]);
		}

		// Start a new sample.
		SampleStartTimestamp = Time;
		SetSampleU64(ETimelineU64::MinTotalAllocatedMemory, TotalAllocatedMemory);
		SetSampleU64(ETimelineU64::MaxTotalAllocatedMemory, TotalAllocatedMemory);
		SetSampleU32(ETimelineU32::MinLiveAllocations, TotalLiveAllocations);
		SetSampleU32(ETimelineU32::MaxLiveAllocations, TotalLiveAllocations);
		SetSampleU64(ETimelineU64::MinTotalSwapMemory, TotalSwapMemory);
		SetSampleU64(ETimelineU64::MaxTotalSwapMemory, TotalSwapMemory);
		SetSampleU64(ETimelineU64::MinTotalCompressedSwapMemory, TotalCompressedSwapMemory);
		SetSampleU64(ETimelineU64::MaxTotalCompressedSwapMemory, TotalCompressedSwapMemory);
		SetSampleU32(ETimelineU32::AllocEvents, 0);
		SetSampleU32(ETimelineU32::FreeEvents, 0);
		SetSampleU32(ETimelineU32::PageInEvents, 0);
		SetSampleU32(ETimelineU32::PageOutEvents, 0);
		SetSampleU32(ETimelineU32::SwapFreeEvents, 0);
...
void FAllocationsProvider::EditOnAnalysisCompleted(double Time)
{
	...
	UE_LOG(LogTraceServices, Log, TEXT("[MemAlloc] Indexed: %llu events"), TotalEventCount);
	UE_LOG(LogTraceServices, Log, TEXT("[MemAlloc] Traced: %llu alloc events, %llu free events, %llu heap events, %llu swap op events"),
		AllocEvents, FreeEvents, HeapEvents, SwapOpEvents);
	UE_LOG(LogTraceServices, Log, TEXT("[MemAlloc] Analysis completed (%llu allocs, %llu frees, %llu heaps, %u heap specs)."),
		AllocCount, FreeCount, HeapCount, NumHeapSpecs);
}
...
void FAllocationsProvider::EnumerateTimeline(ETimelineU64 TimelineIndex, int32 StartIndex, int32 EndIndex, TFunctionRef<void(double Time, double Duration, uint64 Value)> Callback) const
{
	ReadAccessCheck();

	if (int32(TimelineIndex) < 0 ||
		int32(TimelineIndex) >= NumTimelinesU64 ||
		ValueTimelinesU64[int32(TimelineIndex)] == nullptr)
	{
		return;
	}
	const TPagedArray<uint64>& ValueTimeline = *ValueTimelinesU64[int32(TimelineIndex)];
	...
	auto ValueIt = ValueTimeline.GetIteratorFromItem(StartIndex);
...
void FAllocationsProvider::EnumerateTimeline(ETimelineU32 TimelineIndex, int32 StartIndex, int32 EndIndex, TFunctionRef<void(double Time, double Duration, uint32 Value)> Callback) const
{
	ReadAccessCheck();

	if (int32(TimelineIndex) < 0 ||
		int32(TimelineIndex) >= NumTimelinesU32 ||
		ValueTimelinesU32[int32(TimelineIndex)] == nullptr)
	{
		return;
	}
	const TPagedArray<uint32>& ValueTimeline = *ValueTimelinesU32[int32(TimelineIndex)];

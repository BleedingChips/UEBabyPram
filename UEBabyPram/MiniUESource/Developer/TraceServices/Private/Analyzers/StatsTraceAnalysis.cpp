...
bool FStatsAnalyzer::OnEvent(uint16 RouteId, EStyle Style, const FOnEventContext& Context)
{
	...
	switch (RouteId)
	{
		case RouteId_Spec:
		{
			...
			{
				bIsAlreadyDeclared = true;
				UE_LOG(LogTraceServices, Warning, TEXT("Stats counter %u already declared."), StatId);
			}

			const ICounter* Counter = EditableCounterProvider.GetCounter(EditableCounter);
			...
			//if (!Name.IsEmpty())
			{
				if (!bIsAlreadyDeclared)
				{
					EditableCounter->SetName(Session.StoreString(*Name));
				}
				else if (Counter && FCString::Strcmp(*Name, Counter->GetName()) != 0)
				{
					UE_LOG(LogTraceServices, Error, TEXT("Stats counter %u redeclared with different name."), StatId);
				}
			}
			...
			if (!Group.IsEmpty())
			{
				...
				else if (Counter && FCString::Strcmp(*Group, Counter->GetGroup()) != 0)
				{
					UE_LOG(LogTraceServices, Error, TEXT("Stats counter %u redeclared with different group name."), StatId);
				}
			}
			...
			if (!Description.IsEmpty())
			{
				...
				else if (Counter && FCString::Strcmp(*Description, Counter->GetDescription()) != 0)
				{
					UE_LOG(LogTraceServices, Error, TEXT("Stats counter %u redeclared with different description."), StatId);
				}
			}
			...
			else if (Counter && bIsFloatingPoint != Counter->IsFloatingPoint())
			{
				UE_LOG(LogTraceServices, Error, TEXT("Stats counter %u redeclared with different IsFloatingPoint flag."), StatId);
			}
			...
			else if (Counter && bIsResetEveryFrame != Counter->IsResetEveryFrame())
			{
				UE_LOG(LogTraceServices, Error, TEXT("Stats counter %u redeclared with different IsResetEveryFrame flag."), StatId);
			}
			...
			else if (Counter && DisplayHint != Counter->GetDisplayHint())
			{
				UE_LOG(LogTraceServices, Error, TEXT("Stats counter %u redeclared with different display hint."), StatId);
			}
			break;
		}

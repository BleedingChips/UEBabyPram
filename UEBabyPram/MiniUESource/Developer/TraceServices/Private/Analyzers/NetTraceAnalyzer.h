...
private:
	...
	// Default names
	FNetProfilerNameIndexType BunchHeaderNameIndex;
	FNetProfilerNameIndexType PendingNameIndex;

	// Shared for trace
	TMap<FNetProfilerNameIndexType, FNetProfilerNameIndexType> TracedNameIdToNetProfilerNameIdMap;

	TMap<uint64, uint32> TraceEventTypeToNetProfilerEventTypeIndexMap;

	TMap<FNetProfilerNameIndexType, uint32> TraceNetStatsCounterIdToNetProfilerStatsCounterTypeIndexMap;
	...
	struct FNetTraceGameInstanceState
	{
		...
		TMap<int32, FNetProfilerNameIndexType> ChannelNames;
...
private:
	...
	uint32 GetTracedEventTypeIndex(FNetProfilerNameIndexType NameIndex, uint8 Level);
	uint32 GetOrCreateNetProfilerStatsCounterTypeIndex(FNetProfilerNameIndexType NameId, ENetProfilerStatsCounterType StatsType);
	...
	void HandleObjectExistsEvent(const FOnEventContext& Context, const FEventData& EventData);
	...
	void AddEvent(TPagedArray<FNetProfilerContentEvent>& Events, uint32 StartPos, uint32 EndPos, uint32 Level, FNetProfilerNameIndexType NameIndex, FNetProfilerBunchInfo Info);

private:

	TMap<uint32, TSharedRef<FNetTraceGameInstanceState>> ActiveGameInstances;
	FNetProfilerTimeStamp StartTimeStamp;

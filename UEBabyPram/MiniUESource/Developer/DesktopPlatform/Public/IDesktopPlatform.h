...
{
	None,			// No user interaction permitted
	TimeLimited,	// Allow user interaction within a limited window of time (eg: 30 seconds), but don't wait indefinitely for interaction
	Interactive		// Allow user interaction with an indefinite wait if necessary
};
...
public:
	...
	/**
	 * Returns the root directory for the engine with the given identifier.
	 *
	 * @param	Identifier			Identifier used to refer to this installation.
	 * @param	OutRootDir			Root directory for the engine.
	 */
	 ...
	 *
	 * @param	RootDir				Root directory for the engine.
	 ...
	 *
	 * @param ProjectDir			Directory of the project to clean.
	 * @param OutFailPath			The path of the first file or directory that failed to delete, if any.
	 * @param Warn					Feedback context to use for progress updates.
	 * @return						true if the project was cleaned successfully, false if any file or directory could not be deleted.
	 */
	 ...
	 * 
	 * @param Identifier				Identifier for the engine 
	 * @param bIncludeNativeProjects	Flag indicating whether to include native projects.
	 * @param OutProjectFileNames		Array to receive the paths of the projects known by the engine.
	...
	/**
	 * Get an access token for use by OIDC compliant Identity Providers
	 ...
	 * @return true if the task completed successfully.
	 */
	virtual bool GetOidcAccessToken(const FString& RootDir, const FString& ProjectFileName, const FString& ProviderIdentifier, EDesktopLoginInteractionLevel InteractionLevel, FFeedbackContext* Warn, FString& OutToken, FDateTime& OutTokenExpiresAt, bool& bOutWasInteractiveLogin) = 0;

	/**
	 * Get an access token for use by OIDC compliant Identity Providers using a remote configuration

	 * @param RootDir					Engine root directory for the project to use.
	 * @param RemoteHost				The url to the remote host to fetch the config from
	 * @param bUnattended				True to indicate that no user interaction should be assumed
	 * @param Warn						Feedback context to use for progress updates
	 * @param OutToken					The allocated access token
	 * @param OutTokenExpiresAt			When the token expires
	 ...
	 * @return true if the task completed successfully.
	 */
	virtual bool GetOidcAccessTokenWithRemoteConfig(const FString& RootDir, const FString& RemoteHost, EDesktopLoginInteractionLevel InteractionLevel, FFeedbackContext* Warn, FString& OutToken, FDateTime& OutTokenExpiresAt, bool& bOutWasInteractiveLogin) = 0;
	 ...
	 * @return true if the task completed successfully.
	 */
	UE_DEPRECATED(5.7, "Please use overload that takes EDesktopLoginInteractionLevel instead")
	 ...
	 */
	UE_DEPRECATED(5.7, "Please use overload that takes EDesktopLoginInteractionLevel instead")
	virtual bool GetOidcAccessTokenWithRemoteConfig(const FString& RootDir, const FString& RemoteHost, bool bUnattended, FFeedbackContext* Warn, FString& OutToken, FDateTime& OutTokenExpiresAt, bool& bOutWasInteractiveLogin) = 0;

	/**
	 * Get the status of an access token for use by OIDC compliant Identity Providers
	...
	/**
	 * Get the path to the OIDC token executable

	 * @param RootDir					Engine root directory for the project to use.
	 * @return The path to the OIDC token executable.
	 */
	UE_INTERNAL virtual FString GetOidcTokenExecutableFilename(const FString& RootDir) const = 0;
	 ...
	 * @return true if the task completed successfully.
	 */
	virtual bool GetHordeAccessToken(const FString& HordeUrl, EDesktopLoginInteractionLevel InteractionLevel, FFeedbackContext* Warn, FString& OutToken, FDateTime& OutTokenExpiresAt, bool& bOutWasInteractiveLogin) = 0;

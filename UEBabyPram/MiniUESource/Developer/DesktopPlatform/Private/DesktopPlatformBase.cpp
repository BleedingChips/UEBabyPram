...
bool FDesktopPlatformBase::GetOidcAccessToken(const FString& RootDir, const FString& ProjectFileName, const FString& ProviderIdentifier, EDesktopLoginInteractionLevel InteractionLevel, FFeedbackContext* Warn, FString& OutToken, FDateTime& OutTokenExpiresAt, bool& bOutWasInteractiveLogin)
{
	...
	return GetOidcAccessTokenInternal(RootDir, BaseArguments, InteractionLevel, Warn, OutToken, OutTokenExpiresAt, bOutWasInteractiveLogin);
}

bool FDesktopPlatformBase::GetOidcAccessTokenWithRemoteConfig(const FString& RootDir, const FString& RemoteHost, EDesktopLoginInteractionLevel InteractionLevel, FFeedbackContext* Warn, FString& OutToken, FDateTime& OutTokenExpiresAt, bool& bOutWasInteractiveLogin)
{
	FString BaseArguments;
	BaseArguments += FString::Printf(TEXT(" --AuthConfigUrl=\"%s\""), *RemoteHost);
...
{
	const EDesktopLoginInteractionLevel InteractionLevel = bUnattended ? EDesktopLoginInteractionLevel::None : EDesktopLoginInteractionLevel::Interactive;
	return GetOidcAccessToken(RootDir, ProjectFileName, ProviderIdentifier, InteractionLevel, Warn, OutToken, OutTokenExpiresAt, bOutWasInteractiveLogin);
}

bool FDesktopPlatformBase::GetOidcAccessTokenWithRemoteConfig(const FString& RootDir, const FString& RemoteHost, bool bUnattended, FFeedbackContext* Warn, FString& OutToken, FDateTime& OutTokenExpiresAt, bool& bOutWasInteractiveLogin)
{
	const EDesktopLoginInteractionLevel InteractionLevel = bUnattended ? EDesktopLoginInteractionLevel::None : EDesktopLoginInteractionLevel::Interactive;
	return GetOidcAccessTokenWithRemoteConfig(RootDir, RemoteHost, InteractionLevel, Warn, OutToken, OutTokenExpiresAt, bOutWasInteractiveLogin);
}
...
bool FDesktopPlatformBase::GetHordeAccessToken(const FString& HordeUrl, EDesktopLoginInteractionLevel InteractionLevel, FFeedbackContext* Warn, FString& OutToken, FDateTime& OutTokenExpiresAt, bool& bOutWasInteractiveLogin)
{
	...
	return GetOidcAccessTokenInternal(FPaths::RootDir(), BaseArguments, InteractionLevel, Warn, OutToken, OutTokenExpiresAt, bOutWasInteractiveLogin);
}
...
{
	const EDesktopLoginInteractionLevel InteractionLevel = bUnattended ? EDesktopLoginInteractionLevel::None : EDesktopLoginInteractionLevel::Interactive;
	return GetHordeAccessToken(HordeUrl, InteractionLevel, Warn, OutToken, OutTokenExpiresAt, bOutWasInteractiveLogin);
}

bool FDesktopPlatformBase::GetOidcAccessTokenInternal(const FString& RootDir, const FString& BaseArguments, EDesktopLoginInteractionLevel InteractionLevel, FFeedbackContext* Warn, FString& OutToken, FDateTime& OutTokenExpiresAt, bool& bOutWasInteractiveLogin)
{
	...
	if (ExitCode == 10)
	{
		bRes = GetOidcAccessTokenInteractive(RootDir, Arguments, InteractionLevel, Warn, ExitCode);

		bOutWasInteractiveLogin = true;

		if (!bRes)
		{
			if (InteractionLevel != EDesktopLoginInteractionLevel::Interactive)
			{
				UE_LOG(LogDesktopPlatform, Warning, TEXT("Unable to allocate an access token. Unable to complete interactive login. Make sure you start the editor and login once or log in using UGS or using the UGS cli command 'login'. (Ran '%s%s')"), *GetOidcTokenExecutableFilename(RootDir), *Arguments);
			}
			else
			{
...
bool FDesktopPlatformBase::GetOidcAccessTokenInteractive(const FString& RootDir, const FString& Arguments, EDesktopLoginInteractionLevel InteractionLevel, FFeedbackContext* Warn, int32& OutReturnCode)
{
	if (InteractionLevel == EDesktopLoginInteractionLevel::None)
	{
		OutReturnCode = -1;
		return false;
	}
	...
	while (!bIsFinished)
	{
		...
		{
			if (InteractionLevel == EDesktopLoginInteractionLevel::TimeLimited)
			{
				bIsFinished = !FPlatformProcess::IsProcRunning(ProcHandle);
				break;
			}
	...
	{
		// OidcToken can throw exceptions, and in those cases on Mac & Linux, we don't
		// get a return code.  Windows always produces a return code.  Indicate it was invoked
		// but yield a return code of -1.
		OutReturnCode = -1;
	}
	FPlatformProcess::ClosePipe(PipeRead, PipeWrite);
	if (ProcHandle.IsValid())
...
FProcHandle FDesktopPlatformBase::InvokeOidcTokenToolAsync(const FString& InArguments, void*& OutReadPipe, void*& OutWritePipe)
{
	...
	FProcHandle ProcHandle = FPlatformProcess::CreateProc(*ExecutableFileName, *CmdLineParams, bLaunchDetached, bLaunchHidden, bLaunchReallyHidden, NULL, 0, FPlatformProcess::BaseDir(), OutWritePipe);
	if (!ProcHandle.IsValid())
	{
...
bool FDesktopPlatformBase::BuildUnrealBuildTool(const FString& RootDir, FOutputDevice& Ar)
{
	...
	FProcHandle ProcHandle = FPlatformProcess::CreateProc(*CompilerExecutableFilename, *CmdLineParams, bLaunchDetached, bLaunchHidden, bLaunchReallyHidden, NULL, 0, NULL, PipeWrite);
	if (!ProcHandle.IsValid())
	{

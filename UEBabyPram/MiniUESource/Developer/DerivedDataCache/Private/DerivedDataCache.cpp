	...
	static void AddCookStats(FCookStatsManager::AddStatFuncRef AddStat)
	{
		...
		// If the DDC graph ever contains more than one local or remote filesystem, this will only find one of them.
		{
			...
			AddStat(TEXT("DDC.Summary"), FCookStatsManager::CreateKeyValueArray(
				...
				TEXT("ZenRemoteReadSpeed"), ZenRemoteSpeedStats.ReadSpeedMBs,
				TEXT("ZenRemoteWriteSpeed"), ZenRemoteSpeedStats.WriteSpeedMBs,
				TEXT("ZenRemoteLatency"), ZenRemoteSpeedStats.LatencyMS,
...
void Private::SaveToCompactBinary(FCbWriter& Writer, const FCacheGetRequest& Request)
{
...
void Private::SaveToCompactBinary(FCbWriter& Writer, const FCacheGetValueRequest& Request)
{
	if (!Request.Name.IsEmpty())
	{
...
void Private::SaveToCompactBinary(FCbWriter& Writer, const FCacheGetChunkRequest& Request)
{
	if (!Request.Name.IsEmpty())
	{
		Writer << ANSITEXTVIEW("Name") << MakeStringView(Request.Name);

...
bool FLegacyCacheKey::ReadValueTrailer(FCompositeBuffer& Value) const
{
	if (HasShortKey())
	{
		...
		FullKey << GetFullKey();
		const FMemoryView CompareKey(*FullKey, (FullKey.Len() + 1) * sizeof(UTF8CHAR));
...
void FLegacyCacheKey::WriteValueTrailer(FCompositeBuffer& Value) const
{
	if (HasShortKey())
	{
		FUtf8StringBuilderBase FullKey;
		FullKey << GetFullKey();
		Value = FCompositeBuffer(Value, FSharedBuffer::Clone(*FullKey, (FullKey.Len() + 1) * sizeof(UTF8CHAR)));
	}
}

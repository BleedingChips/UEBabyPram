...
private:
	static void SaveValuesToCompactBinary(FCbWriter& Writer, const FCacheGetResponse& Response);
	static void SaveValuesToCompactBinary(FCbWriter& Writer, const FCacheGetValueResponse& Response);
	static void SaveValuesToCompactBinary(FCbWriter& Writer, const FCacheGetChunkResponse& Response);
...
struct TCacheStoreReplayBatch
{
		...
		, Priority(Owner.GetPriority())
	{
		...
		(Replay->GetInnerCache()->*CacheFunctionFor<FRequest>)(ModifiedRequests, Owner, [this](FResponse&& Response)
		{
			...
			{
				Replay->SerializeRequests<FRequest>(Requests, Responses, CacheMethodFor<FRequest>, Priority);
				delete this;
			}
		});
	}
	...
	const EPriority Priority)
{
	...
	for (const RequestType& Request : Requests)
	{
		...
		{
			Writer.BeginObject();
			Private::SaveToCompactBinary(Writer, Request);
			SaveValuesToCompactBinary(Writer, Responses[Index]);
			Writer.EndObject();
		}
	}
	Writer.EndArray();
...
void FCacheStoreReplay::SaveValuesToCompactBinary(FCbWriter& Writer, const FCacheGetResponse& Response)
{
	if (Response.Status == EStatus::Ok)
	{
		...
		{
			Writer.BeginObject();
			Writer << ANSITEXTVIEW("Id") << Value.GetId();
			Writer << ANSITEXTVIEW("RawHash") << Value.GetRawHash();
			Writer << ANSITEXTVIEW("RawSize") << Value.GetRawSize();
			Writer.EndObject();
		}
		Writer.EndArray();
	}
...
void FCacheStoreReplay::SaveValuesToCompactBinary(FCbWriter& Writer, const FCacheGetValueResponse& Response)
{
	if (Response.Status == EStatus::Ok)
	{
		Writer.BeginObject(ANSITEXTVIEW("Value"));
		Writer << ANSITEXTVIEW("RawHash") << Response.Value.GetRawHash();
		Writer << ANSITEXTVIEW("RawSize") << Response.Value.GetRawSize();
		Writer.EndObject();
	}
}

void FCacheStoreReplay::SaveValuesToCompactBinary(FCbWriter& Writer, const FCacheGetChunkResponse& Response)
{
	if (Response.Status == EStatus::Ok)
	{
		Writer.BeginObject(ANSITEXTVIEW("Value"));
		Writer << ANSITEXTVIEW("RawHash") << Response.RawHash;
		Writer << ANSITEXTVIEW("RawSize") << Response.RawSize;
		Writer.EndObject();
	}
}
...
{
	new TCacheStoreReplayBatch(this, Requests, Owner, MoveTemp(OnComplete));
}
